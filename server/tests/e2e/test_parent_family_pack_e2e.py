...
def test_pack_create_duplicate_name_returns_409(
    ...
    first = http.post("/api/v1/parent/family-packs", json={"name": name})
    assert first.status_code == 201, first.text

    second = http.post("/api/v1/parent/family-packs", json={"name": name})
...
def test_pack_list_excludes_archived_by_default(
    ...
    arch = http.post(f"/api/v1/parent/family-packs/{pack_id}/archive")
    assert arch.status_code == 200
    assert arch.json()["state"] == "archived"

    default = http.get("/api/v1/parent/family-packs")
...
def test_pack_publish_then_rollback_lifecycle(
    ...
    # Publish v1.
    http.put(
        f"/api/v1/parent/family-packs/{pack_id}/draft/words/{prefix}{run_id[:6]}-w1",
    ...
    http.put(
        f"/api/v1/parent/family-packs/{pack_id}/draft/words/{prefix}{run_id[:6]}-w2",
    ...
    rb = http.post(f"/api/v1/parent/family-packs/{pack_id}/rollback")
    ...
    versions = http.get(f"/api/v1/parent/family-packs/{pack_id}/versions")
