"""V0.6.3 — HTTP behaviour contracts for /api/v1/family/_/family-packs/* and
...
async def test_create_duplicate_active_name_409(db: object) -> None:
    ...
    async with ac:
        r = await ac.post(
            "/api/v1/family/_/family-packs", json={"name": "Fruit"}
        )
...
async def test_list_default_excludes_archived(db: object) -> None:
    ac, _ = await _make_parent_client()
    async with ac:
        await ac.post("/api/v1/family/_/family-packs", json={"name": "Keep"})
        rdrop = await ac.post(
            "/api/v1/family/_/family-packs", json={"name": "Drop"}
        )
        pack_to_drop = rdrop.json()["pack_id"]
        await ac.post(
            f"/api/v1/family/_/family-packs/{pack_to_drop}/archive"
        )

        r = await ac.get("/api/v1/family/_/family-packs")
        assert r.status_code == 200
        names = sorted(item["definition"]["name"] for item in r.json()["items"])
        assert names == ["Keep"]

        rall = await ac.get(
            "/api/v1/family/_/family-packs?include_archived=true"
        )
        names_all = sorted(
...
async def test_get_pack_other_family_404(db: object) -> None:
    ...
    async with ac_a, ac_b:
        r = await ac_a.post(
            "/api/v1/family/_/family-packs", json={"name": "PrivateA"}
        )
        pack_id = r.json()["pack_id"]
        r = await ac_b.get(f"/api/v1/family/_/family-packs/{pack_id}")
...
async def test_admin_token_blocked_on_parent_endpoint(db: object) -> None:
    ...
    async with AsyncClient(
        ...
        r = await ac.post(
            "/api/v1/family/_/family-packs", json={"name": "X"}
        )
...
async def test_full_publish_flow_via_http(db: object) -> None:
    ...
    async with ac:
        r = await ac.post(
            "/api/v1/family/_/family-packs", json={"name": "Pack1"}
        )
        pack_id = r.json()["pack_id"]

        r = await ac.put(
            f"/api/v1/family/_/family-packs/{pack_id}/draft/words/apple",
            json={"source": "global"},
        )
        assert r.status_code == 200
        ...
        r = await ac.post(
            f"/api/v1/family/_/family-packs/{pack_id}/publish", json={}
        )
        assert r.status_code == 201
        assert r.json()["version"] == 1

        r = await ac.get(
            f"/api/v1/family/_/family-packs/{pack_id}/versions"
        )
        assert r.status_code == 200
...
async def test_draft_pack_full_409(db: object) -> None:
    ...
    async with ac:
        r = await ac.post(
            "/api/v1/family/_/family-packs", json={"name": "Big"}
        )
        pack_id = r.json()["pack_id"]
        for i in range(50):
            r = await ac.put(
                f"/api/v1/family/_/family-packs/{pack_id}/draft/words/g-{i}",
                json={"source": "global"},
            )
            assert r.status_code == 200
        r = await ac.put(
            f"/api/v1/family/_/family-packs/{pack_id}/draft/words/g-50",
            json={"source": "global"},
        )
    assert r.status_code == 409
...
async def test_publish_empty_409(db: object) -> None:
    ...
    async with ac:
        r = await ac.post(
            "/api/v1/family/_/family-packs", json={"name": "Empty"}
        )
        pack_id = r.json()["pack_id"]
        r = await ac.post(
            f"/api/v1/family/_/family-packs/{pack_id}/publish", json={}
...
async def test_custom_word_id_must_have_family_prefix(db: object) -> None:
    ...
    async with ac:
        r = await ac.post(
            "/api/v1/family/_/family-packs", json={"name": "P"}
        )
        pack_id = r.json()["pack_id"]
        r = await ac.put(
            f"/api/v1/family/_/family-packs/{pack_id}/draft/words/wrong-id",
            json={
                "source": "custom",
                "word": "x",
                "meaning_zh": "y",
...
async def test_child_merged_json_204_when_no_packs(db: object) -> None:
    ...
    async with ac_d:
        r = await ac_d.get("/api/v1/family/_/family-packs/latest.json")
...
async def test_child_merged_json_200_with_published(db: object) -> None:
    ...
    async with ac_p:
        r = await ac_p.post(
            "/api/v1/family/_/family-packs", json={"name": "Pack"}
        )
        pack_id = r.json()["pack_id"]
        await ac_p.put(
            f"/api/v1/family/_/family-packs/{pack_id}/draft/words/apple",
...
async def test_child_merged_json_head_returns_etag_only(db: object) -> None:
    ...
    ac_d, _ = await _make_device_client(family_id=family_id)
    async with ac_d:
        r = await ac_d.head("/api/v1/family/_/family-packs/latest.json")
    assert r.status_code == 200
