...
Endpoints:
- POST `/api/v1/family/{family_id}/pair/create` (cookie) — issue a PairToken
- GET `/api/v1/family/{family_id}/pair/status/{token}` (cookie) — poll status
- DELETE `/api/v1/family/{family_id}/pair/{token}` (cookie) — cancel pending token
- POST `/api/v1/public/pair/redeem` (no auth) — client posts { token | short_code,
...
family_router = APIRouter(prefix="/api/v1/family", tags=["pair"])
public_router = APIRouter(prefix="/api/v1/public", tags=["pair"])
landing_router = APIRouter(tags=["pair-landing"])
...
@landing_router.get("/p/{token_short}", response_class=HTMLResponse)
...
__all__ = ["family_router", "public_router", "landing_router"]
