...
_LESSON_SYSTEM_PROMPT = (
    ...
    "  * `story_en`: one short English fairy-tale intro sentence for "
    "    the pack's HomePage card (8-18 words). Required even if the "
    "    page does not print a story.\n"
    "  * `story_zh`: one short Chinese fairy-tale intro sentence for "
    "    the same pack (20-60 characters). Required even if the page "
    "    does not print a story.\n"
    "    The server stores these as `scene.storyEn` and `scene.storyZh`.\n"
    "  * `words`: array of objects, one per memorisable vocabulary item. "
    "    Each object MUST have these fields, plus `source_no` when "
    "    the page prints an item number:\n"
    "      - `source_no`: integer item number printed before the row, "
    "        e.g. 101. Use it to audit that no visible numbered row "
    "        is skipped.\n"
    "      - `word`: the COMPLETE English vocabulary item as printed "
    "        in the numbered/listed row. It may be a single word, "
    "        multi-word phrase, fixed expression, phrasal verb, "
    "        collocation, acronym phrase, or ellipsis pattern. Do NOT "
    "        lemmatize, split, or shorten phrases: `listen to music` "
    "        must stay `listen to music`, `watch TV` must stay "
    "        `watch TV`, `feed the cows` must stay `feed the cows`, "
    "        and `take...to...` must stay `take...to...`. Preserve "
    "        spaces, hyphens, apostrophes, ellipses, and acronym "
    "        capitalization when printed; otherwise use lowercase.\n"
    ...
    "        words, primary-school grammar) using the vocabulary "
    "        item naturally. For literal words or phrases, the full "
    "        `word` value MUST appear exactly once in the sentence so "
    "        it can be replaced with a blank for a cloze "
    "        (fill-in-the-blank) exercise. For ellipsis patterns such "
    "        as `take...to...`, write a natural sentence demonstrating "
    "        the pattern. Write the FULL sentence including the target "
    "        item; do NOT pre-blank it. Avoid sentences whose context "
    "        is ambiguous without the target item.\n"
    ...
    "    Include every numbered/listed memorisable vocabulary item "
    "    from THIS page, including multi-word phrases and fixed "
    "    expressions. Before returning, compare the `source_no` "
    "    values against the visible printed numbers; if the page "
    "    shows 101-150, return 50 word objects with source_no 101 "
    "    through 150. Do not drop function words like a/the/to/of. "
    "    Ignore unit titles, grammar topics, page numbers, and "
    "    sentence examples.\n"
