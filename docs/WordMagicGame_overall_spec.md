...
> 文档状态：当前版本基线（Monorepo 四模块 + **三端 bootstrap parity V0.7.1** + 服务端 **V0.8.1 / V0.8.2+** 后台 + V0.8.3 polish parity code + V0.8.4 client core）
> 适用版本：V0.1 原型 → V0.7.1 三端对齐 → V0.8 家长词库与系统管理员控制台（已交付）→ V0.8.3 / V0.8.4 battle gate cleanup
...
- **HarmonyOS 客户端**（`harmonyos/`，app `0.8.4` / `1008004`）：完整闭环，17 页路由；战斗、复习、每日冒险、愿望单、家长 PIN、按词包本地学习报告；V0.6.5+ 三层词包（built-in / global / family）+ 设备级 pack 选择器；ohosTest 为自动化权威参考；V0.8.3 polish + V0.8.4 scheduler core 已在本端落地。
- **iOS 客户端**（`ios/WordMagicGame/`，`0.8.4` / build `1008006`）：与 Harmony **bootstrap 矩阵对齐**（17 页、V0.6.7.8 语义）；V0.8.4 scheduler / HP / Spell penalty core 已复制；V0.8.3 polish（10 槽、auto-rotate、MonsterLevel、bonus、HP-2、稳定 ID）已补到代码层。索引见 [`docs/ios-replica/00-index.md`](ios-replica/00-index.md)、发布门禁 [`ios/release-pre.md`](../ios/release-pre.md)。
- **Android 客户端**（`android/`，app `0.8.4` / `1008004`）：与 Harmony **bootstrap 矩阵对齐**（17 页均已实现）；V0.8.4 scheduler / HP / Spell penalty core 已复制；V0.8.3 polish（10 槽、auto-rotate、MonsterLevel、bonus、HP-2、稳定 ID）已补到代码层。索引见 [`docs/android-replica/00-index.md`](android-replica/00-index.md)、Phase 5 门禁 [`docs/android-replica/07-release-readiness-checklist.md`](android-replica/07-release-readiness-checklist.md)。
- **服务端**：FastAPI（Vercel），词库 / 词包 / 类目 / 家长账户 / 设备绑定 / OpenAI 辅助；**V0.8.1** 家长 Web 词库工作台 `/family/{family_id}/packs/*` + 子端 **global+family 合并** `GET /api/v1/family/{family_id}/packs/latest.json`；**V0.8.2** 系统管理员 HTML `/admin/*`（会话 cookie + `admin_audit_service`）；**2026-05-22 follow-up** 增加 admin global pack audited hard delete（API + HTML）。自 V0.6.5 起 `FamilyPackDefinition` + `GLOBAL_PACK_FAMILY_ID` 承载 global / family 两层。
...
| V0.8.3 | 战斗与词包体验小优化（三端代码补齐 / iOS verification pending） | 三端具备词包激活 10 槽 + 满额自动轮换、`MonsterLevel` + bonus ✨ + 50% HP-2 重击、`DamageFloaterLabel` 与设计 §5 稳定 ID；Android targeted tests 已通过，iOS 仍需 Xcode 开发机复核。详见 feature `2026-05-18-battle-polish-v0-8-3`。 |
| V0.8.4 | 战斗平衡与题型节奏（三端 core 已实现 / gate cleanup open） | HarmonyOS / Android metadata 到 `0.8.4` / `1008004`，iOS 到 `0.8.4` / `1008006`；魔法师默认 HP **10**；`Spell` 错点字母每次 **-1 HP**；今日战斗 intro（每词最多 1× Choice + 1× FillLetter）→ challenge（FillLetterMedium / Spell 各 **50%**）。Gate 剩余 targeted tests 与 release checklist。详见 feature `2026-05-18-battle-balance-v0-8-4`。 |
...
```text
baseCoins = result.stars
coinsEarned = won && bonusKillCount > 0 ? ceil(baseCoins * 1.3) : baseCoins
```
...
- `DAILY_CAP = 20`，V0.8.3 bonus 怪会让胜利局金币按 1.3 向上取整；正常使用仍不容易触顶。
...
| 模块 | 当前状态 | 技术方向 | 职责边界 |
| ---- | -------- | -------- | -------- |
| `harmonyos/` | 已实现（权威参考） | HarmonyOS NEXT / ArkTS / ArkUI | 17 页运行时 + ohosTest；`app.json5` **0.8.4 / 1008004**；V0.8.3 polish + V0.8.4 scheduler core 已落地。 |
| `ios/` | 已实现（bootstrap parity + V0.8.4 core） | Native Swift / SwiftUI | `ios/WordMagicGame/`；17 页与 Harmony 对齐；0.8.4 / 1008006；V0.8.3 polish parity 已补代码；见 `docs/ios-replica/00-index.md`。 |
| `android/` | 已实现（bootstrap parity + V0.8.4 core） | Native Kotlin / Jetpack Compose | `android/` 全页落地；0.8.4 / 1008004；V0.8.3 polish parity 已补代码；见 `docs/android-replica/00-index.md`。 |
