...
| `parent_pages.py` / `parent_inbox.py`（HTML） / `parent_packs_pages.py` | `/family` | `/family/{family_id}/**` 家长 Web shell（登录、设备绑定、兑换审批、inbox、词库工作台 `/packs` 等） |
...
- 课本扫描 → 自动建议 family pack 草稿（接 V0.5 已有的 `family_lessons` 草稿流）。
