...
> 文档状态：当前版本基线（Monorepo 四模块结构 + HarmonyOS 运行时 V0.6.7.8）
> 适用版本：V0.1 原型 → V0.7.0 Monorepo 原生多客户端重排
> 客户端目标平台：HarmonyOS NEXT（当前完整实现）、iOS（原生 Swift / SwiftUI 规划）、Android（原生 Kotlin / Jetpack Compose 规划）
...
经过 V0.1 → V0.7.0 的迭代，产品已经从一个本地原型演进为 monorepo 产品工程：

- HarmonyOS 客户端：当前完整闭环的可运行客户端，含战斗、复习、每日冒险、愿望单、家长 PIN、本地学习报告，以及 V0.6.5+ 的三层词包系统（built-in / global / family）+ 设备级 pack 选择器。
- iOS 客户端：根目录 `ios/` 已作为原生客户端模块预留；后续按 Swift / SwiftUI 实现同一产品体验，并通过 `shared/` fixtures 与服务端契约对齐。
- Android 客户端：根目录 `android/` 已作为原生客户端模块预留；后续按 Kotlin / Jetpack Compose 实现同一产品体验，并通过 `shared/` fixtures 与服务端契约对齐。
...
- 共享契约：`shared/` 只保存 OpenAPI / JSON Schema / 错误码 / 同步协议 / golden fixtures，不承载跨平台客户端 runtime；三端各自用原生代码实现业务逻辑。
- 端云协同：各原生客户端按用户主动操作从服务端拉最新 global / family 词包到本地缓存，下次冷启动从缓存重建本地词包库；HarmonyOS 的实现名为 `PackLibrary`，iOS / Android 后续可按本端命名复刻同一语义。
...
- 手机横屏补充练习（当前以 HarmonyOS / Mate 60 Pro 等验证；iOS / Android 后续按同一横屏体验补齐）。
...
- 服务端发布新词包后，任一已接入的原生客户端下次入主页能看到新区域 / 新词，全程不需要重装应用；当前已落地验证路径为 HarmonyOS。
...
| V0.7.0 | Monorepo 原生多客户端重排 | 根目录升级为 monorepo；HarmonyOS DevEco 工程整体迁入 `harmonyos/`；根目录并列预留 `ios/`、`android/` 原生客户端模块；`server/` 保持后端一级模块；`shared/` 只保存 contracts / schemas / fixtures。V0.7.0 不实现 iOS / Android 产品功能，不改变服务端 API 行为，不把 `shared/` 做成跨平台 SDK。 |
...
`harmonyos/entry/src/main/resources/base/profile/main_pages.json` 注册了 16 个页面（顺序）：
...
| 模块 | 当前状态 | 技术方向 | 职责边界 |
| ---- | -------- | -------- | -------- |
| `harmonyos/` | 已实现完整运行时 | HarmonyOS NEXT / ArkTS / ArkUI / DevEco Studio | 当前可运行客户端；承载现有页面、战斗逻辑、本地缓存、设备绑定、三层词包 UI 与 ohosTest。 |
| `ios/` | 预留模块 | Native Swift / SwiftUI | 后续实现 iOS 原生客户端；复用服务端 API 与 `shared/` fixtures，不引入跨平台 runtime。 |
| `android/` | 预留模块 | Native Kotlin / Jetpack Compose | 后续实现 Android 原生客户端；复用服务端 API 与 `shared/` fixtures，不引入跨平台 runtime。 |
| `server/` | 已实现内容与家长后台能力 | Python / FastAPI / MongoDB / Vercel | 词库、词包、家长账户、设备绑定、family/global pack、LLM 草稿与发布链路。 |
