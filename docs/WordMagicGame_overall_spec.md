...
> 文档状态：当前版本基线（V0.6.7.6 — 三层词包 UI 自动化测试覆盖）
> 适用版本：V0.1 原型 → V0.6.7.6
...
- 设备端：完整闭环的 HarmonyOS 应用，含战斗、复习、每日冒险、愿望单、家长 PIN、本地学习报告，以及 V0.6.5+ 的三层词包系统（built-in / global / family）+ 设备级 pack 选择器。
- 服务端：FastAPI 应用（部署在 Vercel），管理词库 / 词包 / 类目 / 兑换池 / 家长账户 / OpenAI 辅助内容生成；自 V0.6.5 起以 `FamilyPackDefinition` 集合为底，通过 `GLOBAL_PACK_FAMILY_ID` 哨兵同时承载 global 和 family 两层。
- 协同：客户端按用户主动操作从服务端拉最新 global / family 词包到本地缓存，下次冷启动从缓存重建本地 `PackLibrary`。
...
| `family`  | 家长在家长后台上传 | 是 | `/api/v1/family/{family_id}/family-packs/latest.json` | `FamilyPackDefinition` 普通记录 |
...
- `Pack` + `SceneMetadata` 模型（`entry/src/main/ets/models/Pack.ets`）— 统一描述三层词包；scene 包含 `bgPrimary` / `bgAccent` / `bossName` / `bossCandidates` / `monsterPlan`。
...
1. **built-in rawfiles**：`entry/src/main/resources/rawfile/data/builtin/{fruit-forest,school-castle,home-cottage,animal-safari,ocean-realm}.json`（每包独立 + scene 元数据）。这是离线兜底，App 安装即可用。
...
`entry/src/main/resources/base/profile/main_pages.json` 注册了 16 个页面（顺序）：

```
pages/HomePage              入口
pages/BattlePage            战斗
pages/ResultPage            结算
pages/ConfigPage            设置（含「📦 我的词包」picker）
...
pages/BypassSecretPage      preview 部署的 bypass token 配置页（V0.6.x debug 工具）
pages/ScanBindingPage       V0.6 家长扫码绑定（含从相册选 QR 解码）
pages/BoundDeviceInfoPage   V0.6 已绑定设备信息查看 / 解绑入口
pages/DevMenuPage           V0.6.x debug 后端环境切换菜单
```
...
| ConfigPage          | 战斗参数（HP / 怪物数 / 倒计时） + 自动发音开关 + 「📦 我的词包」picker（V0.6.5.1）+ 家长 PIN 入口 + 家长账户绑定状态 + 旧版 `word_pack_cache` 同步入口 + 家长管理后台入口。V0.6.6 起不再有「词库类别」chip 行 / 自定义词入口 / ConfigValidationHint。 |
...
| `public_global_packs.py`（V0.6.5）  | `/api/v1/public/global-packs` | `GET /latest.json` + `HEAD /latest.json`（匿名，ETag/304） |
...
| `admin_global_packs.py`（V0.6.5） | `/api/v1/admin/global-packs` | `GET ""` / `GET /{pack_id}` / `POST ""` / `PUT /{pack_id}` / `POST /{pack_id}/publish` / `DELETE /{pack_id}` |
...
> 旧 `WordPackSyncService` 的 `SyncOutcome / SyncStatus` 枚举（`Updated / UpToDate / HttpError / RejectedTooSmall / NetworkError`）单元测试仍在 `entry/src/test`，启动期 bootstrap 流程没有变。
...
不起 mock 直接跑 ohosTest，`ParentAdminFlowV058` 会因为 127.0.0.1:8123 不可达而失败（这是有意的：测试不允许偷偷 fallback 到 prod）。
...
- 家长账户绑定（设备 token + family_id + 短码 / QR 兑换 + 解绑），`@cursor/rules/api-route-pattern.mdc` 工程级路由约束。
- 三层词包模型上线（V0.6.5 server + foundation / V0.6.5.1 UI 全量集成 / V0.6.6 GameConfig 旧字段清理）。
- 家长「📦 我的词包」picker，最多 5 包 + pin + perfect-rotation 自动轮换。
- 家长 admin 路由统一 mount 到 `/api/v1/admin/**`，匿名公共数据全部走 `/api/v1/public/**`，family-scoped 数据全部走 `/api/v1/family/{family_id}/**`。

### V0.6.7+ 候选

- admin 路由的鉴权重接入（家长 / 系统管理员 token），把 V0.5.8 的"全局共享视图"改为行级隔离。
- `legacy_route_aliases.py` 退役（所有客户端切到新 URL 后删除旧别名）。
- 服务端发起 push（推送式新词包），客户端 silent refresh。
- 家长 PIN 哈希存储，跨设备共用一个 family code。
- 课本扫描 → 自动建议 family pack 草稿（接 V0.5 已有的 `admin_lessons` 草稿流）。
