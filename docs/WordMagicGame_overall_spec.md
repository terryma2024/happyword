...
> 文档状态：当前版本基线（V0.6.6 — 三层词包模型 UI 全量集成 + GameConfig 旧词库字段清理）
> 适用版本：V0.1 原型 → V0.6.6
...
- `WordPackBootstrapper.bootstrapPackLibrary(ctx)`（V0.6.5.1）— 冷启动从 prefs 缓存重建 PackLibrary，不发网络；网络刷新由 ConfigPage「🔄 同步词包」按钮显式触发。
...
- HomePage：`@State chipPacks: Pack[]` + `@State todayPack: Pack`，`onPageShow` 调 `loadHomeIntegration` + `applyActivePacks`；点 chip 即切换今日冒险词源；进入战斗前把 `todayPack` 写入 AppStorage `TODAY_ACTIVE_PACK_KEY`。
...
**远端老词包缓存**（`WordPackCache.read()`，namespace `word_pack_cache`）仍保留，是 V0.5 时代的全局 schema-versioned 词包格式，专供 ConfigPage 的「词库同步」按钮和老 `RemoteWordPackService` / `WordPackSyncService` 链路。当前版本它仅替换 BuiltinPackLoader 的 word entry 字段（rich metadata 注入），不影响 PackLibrary 的 union 结果。
...
HomePage ─┬─ HomeStartButton (todayPack) ─→ BattlePage(today, AppStorage TODAY_ACTIVE_PACK_KEY) ─→ ResultPage ─┬─ HomePage
          ...
          └─ HomeConfigButton ─→ ConfigPage ─┬─ 📦 我的词包 picker（最多 5 个 ✓ + 📌 pin + 🔄 同步词包）
                                              │     └─ 🔄 同步：GlobalPackService + FamilyPackService 并行 ensureLatest
                                              ├─ 家长密码 ─→ ParentPinSetupPage
                                              ├─ 立即同步（旧 word_pack_cache，保留兼容）
                                              ├─ 家长账户：未绑定 ─→ ScanBindingPage / 已绑定 ─→ BoundDeviceInfoPage
...
> 同步流程见 §13.5。冷启动绝不发网络（`bootstrapPackLibrary` 只读 prefs 缓存）；网络刷新由 ConfigPage「🔄 同步词包」按钮显式触发。
...
V0.6.5+ 客户端有两条独立的同步链路：

1. **三层 pack 同步**（V0.6.5+ 主链路）—— ConfigPage「📦 我的词包」→「🔄 同步词包」按钮（id `ConfigPackSyncButton`）：并行调 `GlobalPackService.ensureLatest()` + `FamilyPackService.ensureLatest(deviceToken)`（仅在已绑定时），然后 `loadHomeIntegration` 重建 `PackLibrary`，最后弹中文 toast（id `ConfigPackSyncToast`）：「✅ 已同步全局 + 家庭词包」「✅ 已同步全局词包」「✅ 已同步家庭词包」「已是最新」「已是最新 (仅全局可同步，未绑定家长账号)」。HomePage / ConfigPage 的 chip / picker 在同步成功后立即反映新词包；今日冒险下次进入战斗才用新词源，避免战斗中途换底。
2. **V0.5 旧链路（保留兼容）**—— ConfigPage「词库同步」行 →「同步词包」按钮（id `ConfigSyncButton`）：
...
- 家长账户绑定：`ParentBindingFlowV06`（V0.6 + V0.6.x 增量），覆盖 `unboundConfigPageRendersBindEntryOnly` / `tapBindOpensScanBindingPage`（含 `ScanBindingScannerButton.isEnabled()` polling 防止 `bootstrapService` 异步链路退化为永久禁用）/ `shortCodeRedeemFlipsConfigToBound` / `unbindWithPinFlipsBackToUnbound`，加上 V0.6.x 新增的 `pickQrFromGalleryRedeemsAndFlipsToBound`：沿用 V0.5.8 lesson-fixture 的「ohosTest rawfile 预打包 → tempDir 拷贝 → AppStorage 双 override key（`SCAN_BINDING_PHOTO_PICKER_OVERRIDE_URI_KEY` 喂 picker 路径，`SCAN_BINDING_BARCODE_DECODER_OVERRIDE_PAYLOAD_KEY` 喂 ScanKit decode payload）」机制，端到端验证从图库选 QR → 解码 → redeem → 绑定成功的链路，固件 `scan_binding_qr_fixture.png` 由 `tools/generate_scan_binding_qr_fixture.py` 用 `qrcode[pil]` 生成并入仓。
