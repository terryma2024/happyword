...
> 最近更新：2026-05-10（新增 V0.6.7.8 学习报告按词包统计：用户反馈 V0.4.5 LearningReportPage 仍按 category（fruit / place / animal / ocean / home）维度分组，与 V0.6.5+ 的三层词包模型 mental model 不一致 —— 家长在 PackManagerPage 看到的、ConfigPage 选的、HomePage chip 显示的、BattlePage today-mode 关卡套用的，全部是 `Pack` 维度，但学习报告仍要求家长把 5 个 region 的 chip 与 `place` / `home` 这种从 V0.6.6 起已经下线的 category 概念在脑中映射。改动一：`services/LearningReportBuilder.ets` 删 `CategoryReport` / `describeCategory` / `pickWeakCategories`，新增 `PackReport(packId, name, totalSeen, totalCorrect, accuracyPct, active)` 与 `LearningReport.packs: PackReport[]`；`build()` 从 `(repo, recorder, nowMs)` 改成 `(library: PackLibrary, activeIds: string[], recorder, nowMs)`。聚合规则：top-level 总计 / 4 态计数 / 复习完成率走 stats 列表一次性扫描（per-stat-record 唯一性由 LearningRecorder 保证 → 跨包共享词不会被双计），孤儿 stats（wordId 不在任何 pack 里）跳过；per-pack 行各自独立扫包内 `pack.words[]`，所以同一个词在 N 个激活包里会各计一次（per-pack independent counting，符合家长把同一行视为「这个包当前掌握度」的语义）。改动二：行渲染规则 = 激活包按 selection 顺序在前 + 未激活但 totalSeen > 0 的包按 accuracyPct 升序在后 + 未激活且零作答的包不渲染；对未激活包不再额外打 tag，避免「激活 2 / 5 + 未激活 8」混排里 tag 满天飞的视觉噪声。`pages/LearningReportPage.ets` 删 weakCard / categoryRow，新增 `packRow` + `LearningReportPackSection` 容器 id；`packRow` 给每条 `Row` 加 `.id(\`pack-${p.packId}\`)`，让 UI 自动化能锚定具体某个包行。`packBreakdown` 标题从「分类详情」改成「词包详情」。改动三：`aboutToAppear` 改为 `Promise.all([WordPackBootstrapper.forContext(ctx), loadHomeIntegration(ctx)])` 拿到 `HomeIntegrationBundle.library` + `selection.getActiveIds()` 后再驱动 `populateView`，与 HomePage / PackManagerPage / BattlePage 走同一条 PackLibrary 装载路径。**测试**：① `harmonyos/entry/src/test/LearningReportBuilder.test.ets` 整文件重写：删 categoryReport fixtures，新建 `makePack` / `makeLibrary` 三包 fixture（fruit-forest / school-castle 内置 + space-station 全局），覆盖 12 cases —— zero-state、aggregatesAccuracyPerPack、inactivePackWithSeenWordsStillAppears、inactivePackWithNoStatsIsExcluded、wordSharedBetweenActivePacksCountsTowardBoth、topLevelTotalsDedupeSharedWordsAcrossPacks、activePacksSortBeforeInactiveOnes（验证 selection 顺序而非字母）、inactivePacksSortByAccuracyAscending、accuracyRoundsToNearestInteger、countsMasteredAndFamiliarViaScheduler、reviewCompletionTracksTodaysCorrectAnswers、reviewCompletionStays0WhenNothingReviewedToday。② `harmonyos/entry/src/ohosTest/ets/test/LearningReportFlow.ui.test.ets` 把 `LearningReportCategorySection` id 改成 `LearningReportPackSection`，新增「首发激活集种 5 个 builtin → `pack-fruit-forest` 行必在 a11y 树」的 builtin-row smoke 检查；其余顶部 4 个 state pill / accuracy / review bar / 返回链路用例不变。`harmonyos/AppScope/app.json5` 推到 `0.6.7.8` (versionCode 1006016)。`docs/WordMagicGame_overall_spec.md` §6 路由表 / §10.3 `LearningReportBuilder.build` 签名同步刷新。**未做**：未把「按 pack 分组」回填到 `RemoteWordPackConfig` / 服务端学习汇总 —— 报告仍是 device-local，云端 stats 同步只走 `WordStat` 维度，与本次重构正交。先前更新：2026-05-10（新增 V0.6.7.7 ConfigPage 倒计时自定义 + 我的词包行精简：用户反馈 ConfigPage UI 不一致 + 倒计时 chip 太多。改动一：`pages/ConfigPage.ets` 把「我的词包」行 label 从 `📦 我的词包` 改成 `我的词包`，与同页「玩家血量 / 怪物血量 / 怪物数量 / 倒计时 / 发音播放 / 家长密码 / 家长账户 / 学习记录 / 家长」其它 9 行保持「无 leading emoji」一致。改动二：`models/GameConfig.ets` 把 `TIMER_CHOICES` 从 `[3, 15, 30, 60, 120, 300, 600]` 砍到 `[30, 180, 300, 600]`（30s / 3m / 5m / 10m），去掉对家长视角无意义的 3s/15s/1m/2m chip；新增 `TIMER_CUSTOM_MIN = 1` / `TIMER_CUSTOM_MAX = 3600` 边界常量。改动三：新组件 `components/CustomTimerDialog.ets`（`@CustomDialog` + 数字 `TextInput`，导出纯函数 `validateCustomTimerSeconds(input)`：trim → 仅数字 → 在 `[1, 3600]` 内），`ConfigPage` 在 chip 行尾增加 `ConfigTimerCustom` 按钮 → 未激活态显示 `自定义`，激活态显示 `✓自定义 (Xs)` 与 chip 选中样式（`#FFB400` 背景），点击打开 dialog。这条「自定义任意秒数」入口同时给家长更细粒度控制 + 给 UI 自动化测试一个把超时压到 3s 快速触发的便利方案。改动四：`services/GameConfigPersistence.ets` 的 `sanitizeTimer` 不再 snap-round 到最近 chip（旧逻辑会把 `3` 这种自定义值在 rehydrate 时偷偷改回 `30`），改为只 clamp 到 `[TIMER_CUSTOM_MIN, TIMER_CUSTOM_MAX]`，让自定义值能真实持久化。**测试**：① `harmonyos/entry/src/test/CustomTimerDialog.test.ets` 新增 10 个用例覆盖 `validateCustomTimerSeconds` 的 empty / whitespace / non-digit / negative / float / zero / aboveCeiling / 3s / MIN / MAX 全 case。② `harmonyos/entry/src/test/LocalUnit.test.ets::defaultsMatchEngineDefaults` 把 `TIMER_CHOICES.indexOf(3) >= 0` 旧断言换成 `length === 4` + `[30, 180, 300, 600]` 都在 / `[3, 15, 60, 120]` 都不在的精确对比。③ `harmonyos/entry/src/ohosTest/ets/test/ConfigFlow.ui.test.ets` 新增 `customTimerDialogAcceptsThreeSecondsAndUpdatesChip`（开 dialog → 输入 `3` → 确定 → 断言 `ConfigTimerCustom` 文案含 `✓自定义` 和 `3s` + `ConfigTimer30s` 不带 `✓` → 保存 → 重开 ConfigPage 验证 3s 不被 sanitizeTimer snap 回 30s → finally `restoreDefaultTimerChip` 还原 5m 防设备 prefs 污染）和 `customTimerDialogRejectsZeroAndKeepsDialogOpen`（输入 `0` → 确定 → 断言 dialog 仍打开 + `CustomTimerDialogError` 显示 → 取消）。④ 修复历史 flakiness：`scrollToParentPinButton` 从「`findComponent` 非空就退出」加固为「button 报告高度 ≥ 100px 才退出」，旧版会在 `ConfigParentPinButton` 处于 `Y∈[1235,1260]` 这种 25px clipped slice 时立即退出，导致 `clickByIdShared` 落在 viewport 边缘被系统当作非 hit-test，三个 PIN setup 用例反复 fail；新版强制 swipe 到按钮全可见。`harmonyos/AppScope/app.json5` 推到 `0.6.7.7` (versionCode 1006015)；`docs/WordMagicGame_overall_spec.md` 同步刷新 §1 / §15.2。**未做**：`CustomTimerDialog` 的 dialog UI 本身没加 `@Builder` 化的进阶交互（步进按钮 / 滑动选择），目前是单 TextInput + 数字键盘 + 确定/取消，因为 V0.6.7.7 的目标是让「家长 + 测试」拿到「任意秒数」入口，外形可以晚一版迭代。先前更新：2026-05-10（新增 V0.6.7.6 三层词包 UI 自动化测试覆盖：之前 `harmonyos/entry/src/ohosTest/ets/test/PackManagerFlow.ui.test.ets` 只有 builtin 包的 toggle / pin 验证，缺 global / family 这两层在 PackManagerPage 同步、激活、HomePage chip row 传播的 round-trip。本版本扩 `server/mock_ui_server.py` 支撑这条链路：① 新增 `FIXTURE_GLOBAL_PACK_PAYLOAD`（`space-station` / "Space Station"，3 词）+ 匿名只读端点 `GET /api/v1/public/global-packs/latest.json`（含 `If-None-Match` 304 + `HEAD`，匹配 `services/GlobalPackService.ets` 的 ETag 协议）；② 把旧 `GET /api/v1/family/{family_id}/family-packs/active.json`（V0.6.3 桩，固定返回空 packs）替换成与生产一致的 `GET /api/v1/family/{family_id}/family-packs/latest.json`（Bearer token 闸 + ETag + HEAD），返回 `FIXTURE_FAMILY_PACK_PAYLOAD`（`family-snacks` / "Family Snacks"，3 词）。两个 fixture 的 `pack_id` 都刻意避开 5 个 builtin 名字，让 chip row 在「激活 1 个 builtin off + 1 个新包 on」时能客观表现「row composition 变了」。**测试**：`PackManagerFlow.ui.test.ets` 新增 `syncedGlobalPackAppearsInListAndActivatingItGrowsHomeChipRow` 与 `boundDeviceSyncPullsFamilyPackAndAffectsHomeChipRow` 两个用例；前者匿名 sync → 验证 `PackSourceTag_space-station == 官方` + `PackLabel_space-station == Space Station` → 关 `home-cottage` 开 `space-station` → 回 HomePage 验证 `RegionChip_space-station` 出现且文本为英文 `Space Station`、`RegionChip_home-cottage` 消失；后者先 `seedMockBinding()` 在 mock 侧 `_active_bindings` 里挂上 deterministic JWT，再在客户端走 `bindViaShortCode(driver)` 经 `ConfigBindParentButton → ScanBindingPage → ScanBindingManualSubmit → ScanBindingSuccessBack` 完成 UI 级 redeem（确保 `CloudCredentials` 在 live ability 上被实际写入，避开「测试进程预 seed prefs 跟 PackManagerPage `aboutToAppear` 里 in-flight 的 `init()` 抢」的竞态），随后 sync → `PackSourceTag_family-snacks == 家庭` + `PackLabel_family-snacks == Family Snacks` → 关 `animal-safari` 开 `family-snacks` → HomePage 验证 `RegionChip_family-snacks` 出现且文本 `Family Snacks`、`RegionChip_animal-safari` 消失；`finally` 里把激活集还原到 5 个 builtin 默认，再 `unbindMockBinding()` + `wipeBoundDeviceState()` 清 `wordmagic_cloud` 全部绑定 keys（不删 prefs bag —— 测试进程跟 app 进程共享 prefs 缓存，`deletePreferences` 在持有 handle 的情况下会卡住），让紧随其后的 `ParentBindingFlowV06.unboundConfigPageRendersBindEntryOnly` 还能从未绑定状态起步。新增了 `mockHttp` / `seedMockBinding` / `unbindMockBinding` / `bindViaShortCode` / `wipeBoundDeviceState` / `tapPackSyncAndWaitForToast` / `togglePackActiveInPlace` 一组 helper（`@ohos.net.http` + `@ohos.data.preferences`），可被未来的 family / global 测试复用。`harmonyos/AppScope/app.json5` 推到 `0.6.7.6` (versionCode 1006014)；`docs/WordMagicGame_overall_spec.md` §15.2 ohosTest 索引同步加这两个用例。**未做**：服务端真路径仍是 mock-only 覆盖；e2e（真 Vercel preview + 真 family pack publish + 真 device 绑定）暂未跑，因为 V0.6.5 family pack 端点契约已被 server 端 `tests/test_child_family_pack.py` 单测覆盖，client 端的 `services/FamilyPackService.test.ets` + `services/GlobalPackService.test.ets` 单测也覆盖了 200/304 / ETag 协议。先前更新：2026-05-10（新增 V0.6.7.5 HomePage 标签英文统一 + 词包激活变更跨页传播修复：用户报「在词包激活切换以后，标题变成了中文」—— HomePage chip row 默认显示 `pack.labelZh`（builtins 是「水果森林」「校园城堡」等中文），但若同 id 的 global pack override 有空 `description`，`PackLibrary.allPacks` 的 `applySceneFallback` 只 fallback `scene` 不 fallback `labelZh`，导致那两个 pack 的 `labelZh = ''` → chip 兜底到英文 `p.name`，于是出现 5 个 chip 里 3 中 + 2 英文的混排状态。同时 V0.6.7.4 把 HomePage `onPageShow` 加了 `loadHomeIntegration` 重载，让 PackManagerPage 的开关变化能正确传播到 chip row（之前 `homeIntegration.selection` 是 `aboutToAppear` 缓存的 `PackSelectionService` 实例，PackManagerPage 改的是它自己 `loadHomeIntegration` 出来的另一份实例，所以 `applyActivePacks` 看不到变化）。修复方向二选一后用户选了 `all_english`：让 HomePage chip / `AdventureCardTitle` 都跟「我的词包」管理页保持英文命名一致。`pages/HomePage.ets` `packChipLabel` 重写成只看 `p.name` → `p.id` 兜底，去掉 `p.labelZh` / `categoryCatalog.getById` 两条中文分支；`regionLabel` 同步收紧成直接返回 `region.displayName`（`regionFromPack(pack)` 从 `pack.name` 取，所以是英文）；`regionStory` / `categoryCatalog` overlay 体系保留给将来的 storyZh 长文用。**测试**：`harmonyos/entry/src/ohosTest/ets/test/PackManagerFlow.ui.test.ets` 新增 4 个用例（前两个为 V0.6.7.4 跨页传播验证，后两个 + 一个为 V0.6.7.5 英文标签 / `try/finally` 防染池）：① `togglingNonSelectedPackOffRemovesItsHomeRegionChip` 切 `home-cottage` off/on 验证 chip 增减、`AdventureCardTitle` 不变；② `togglingSelectedPackOffSwapsAdventureCardThenRestores` 切 `fruit-forest` off/on 验证 `chipPacks[0]` fallback 路径、`TODAY_REGION_ID_KEY` 自动复位；③ `homeRegionChipsRenderEnglishPackNames` 烟测 5 个 builtin chip 的 `getText()` 严格等于 `Fruit Forest / School Castle / Home Cottage / Animal Safari / Ocean Realm`，回归到中文会被立刻挂掉；两个 toggle 用例用 `try/finally` 在 assertion 失败时把 pack 翻回去，避免污染设备 prefs。`harmonyos/AppScope/app.json5` 推到 `0.6.7.5` (versionCode 1006013)。**未做**：`PackLibrary.applySceneFallback` 没有扩成 labelZh 级 fallback —— 我们最终选了「英文优先」，labelZh 路径在 chip / title 上都不再被消费，没必要替换那条 merge 逻辑。先前更新：2026-05-10（新增 V0.6.7.3 PackManagerPage 固定按钮可读性修复：把 `pages/PackManagerPage.ets` `packRow` 中的 📌 按钮换成「📌 固定 / 已固定」文字按钮 —— 旧版 `Button(this.isPinned(p.id) ? '📌' : '·')` 在 36×36 vp 方框里只渲染单字符 emoji，未 pin 状态显示 `·` 中点字符像无意义的占位符，已 pin 状态的 📌 在 fontSize 16 下也几乎看不清；新版按钮宽度由 padding 自适应，`已固定` 用 `#FEF3C7` 黄底 + `#B45309` 深橙色字、`📌 固定` 用 `#F3F4F6` 灰底 + `#6B7280` 中性字，pinned/unpinned 一眼可辨。同时把固定按钮的渲染条件收紧到「该词包已激活」，未激活的词包行用 `Blank().height(36)` 占位 —— 旧版无论是否激活都画 `·`，但 `togglePackPin` 在未激活时会直接 `showPackToast('只有已激活的词包才能 📌 固定')` 早退，按钮其实是死区。`togglePackPin` 的早退分支保留作为 defensive guard（注释更新到 V0.6.7.3）。`statusRow` 提示文案 `点击右侧开关切换激活；📌 防止满分自动轮换` 同步改成 `开关：切换激活 · 固定：防止满分自动轮换`，避免 emoji 提示与按钮文案不一致。`harmonyos/AppScope/app.json5` 推到 `0.6.7.3` (versionCode 1006011)。**测试**：未触动 ohosTest 内容 —— 现有 `PackPin_<id>` button id 与 `togglePackPin` 行为契约不变；`cd harmonyos && hvigorw assembleHap` + `hvigorw test` 双绿，模拟器实跑确认 `Active+unpinned/Active+pinned/Inactive` 三态视觉差异清晰。先前更新：2026-05-10（新增 V0.6.7.2 解除设备绑定按钮迁移到孩子档案页：把 `pages/ConfigPage.ets` 的「解除设备绑定」行 (`ConfigUnbindBindingButton` / `ConfigUnbindToast` + `@State unbindBusy / unbindToastText` + `private unbindToastTimer` + `onUnbindTap` / `runUnbind` / `showUnbindToast` + `DeviceUnbindClient` / `RealParentFetchAdapter` / `effectiveServerBaseUrl` 三个 import) 全部下线。`pages/BoundDeviceInfoPage.ets` 原本只做 `CloudCredentials.clearBinding()` 的「(本地)」按钮升级成完整服务端 unbind：先 `ParentPinDialog` 校验 6 位家长 PIN，再 `DeviceUnbindClient.unbind()` 调 `POST /api/v1/family/{family_id}/unbind`，成功后 `recorder.attachCloudSync(undefined)` + 弹 `BoundDeviceInfoUnbindToast`「已解除设备绑定」+ 600 ms 后 `router.back()` 回 ConfigPage（其 `onPageShow` 重新跑 `refreshBindingState` 翻回「绑定家长账号」未绑定状态）。按钮文案 `解除绑定 (本地)` → `解除设备绑定` / `正在解除…`，去掉旧的「V0.6.2 仅在本机清除凭据 → 家长网页删除」提示行；新加 `BoundDeviceInfoUnbindButton` / `BoundDeviceInfoUnbindToast` 两个 id。**测试**：`harmonyos/entry/src/ohosTest/ets/test/ParentBindingFlowV06.ui.test.ets` 重写 `unbindWithPinFlipsBackToUnbound → unbindFromBoundDeviceInfoPageFlipsBackToUnbound` 用例，新路径是 `ConfigBoundDeviceInfoButton → BoundDeviceInfoUnbindButton → ParentPinDialog → 6 位 PIN → BoundDeviceInfoUnbindToast → 自动 back → ConfigBindParentButton`；`waitForUnbindToast` helper 也从 `ConfigUnbindToast` 改读 `BoundDeviceInfoUnbindToast`。`shortCodeRedeemFlipsConfigToBound` 移除对 `ConfigUnbindBindingButton` 的 assert（现在 ConfigPage bound 状态只剩 `ConfigBoundDeviceInfoButton` + `ConfigCloudSyncButton`）。`unboundConfigPageRendersBindEntryOnly` 同步移除 `ConfigUnbindBindingButton === null` 校验。`harmonyos/AppScope/app.json5` 版本推到 `0.6.7.2` (versionCode 1006010)。`docs/WordMagicGame_overall_spec.md` §1 / §3 / §6 路由树 / §15.2 ohosTest 索引同步刷新。先前更新：2026-05-10（新增 V0.6.7.1 移除 ConfigPage `词库同步` 行：把 `pages/ConfigPage.ets` 的 `syncRow` builder、`@State syncStatusText / syncToastText / syncBusy`、`private syncToastTimer / wordPackCache`、`refreshSyncStatus` / `formatLocal` / `onSyncTap` / `showSyncToast` / `toastForOutcome` 全部下线，并删除 `WordPackCache` / `RemoteWordPackService` / `WordPackSyncService` / `SyncOutcome` / `SyncStatus` 的页面级 import；`build()` 同步移除 `this.syncRow()` 调用。所有手动同步入口现在统一在 `pages/PackManagerPage` 顶部的 `🔄 同步词包` 按钮（`PackManagerSyncButton`）—— 该按钮并行调 `GlobalPackService.ensureLatest()` + `FamilyPackService.ensureLatest(deviceToken)`（仅在已绑定家长账号时），相比旧 `WordPackSyncService.syncOnce()` 的单一全局 pack cache 更贴近 V0.6.5+ 三层 pack 模型的 mental model。删除 `harmonyos/entry/src/ohosTest/ets/test/ConfigSyncFlow.ui.test.ets` 整文件（4 cases：syncRowIsRendered / tapSyncCachesPackAndShowsSuccessToast / cachedStatusPersistsAcrossPageReentry / secondConsecutiveSyncIsIdempotent），并从 `List.test.ets` 同步去掉 `configSyncFlowUiTest` import + 注册。**保留**：`WordPackBootstrapper.bootstrap()` 在 app 启动期通过 `WordPackSyncService` 后台刷新 `WordPackCache` 给 HomePage 类别 overlay / `CategoryCatalog` 用，`pages/PackManagerPage` 完全独立于这条旧链路；启动期 bootstrap 行为 / `SyncOutcome` 单元测试都没改。`harmonyos/AppScope/app.json5` 版本同步推到 `0.6.7.1` (versionCode 1006009)。`docs/WordMagicGame_overall_spec.md` 同步更新 §5.1 / §10.4 概览段、§13.5 同步流程、§15.2 UI 测试索引。先前更新：2026-05-10（新增 V0.6.7 词包管理独立成页：把 V0.6.5.1 的 `📦 我的词包` 卡从 ConfigPage 抽出来独立成 `pages/PackManagerPage`，进入入口为 ConfigPage 的 `ConfigPackManagerEntry` 行；新页 UI 用 `Toggle` switch 替换 ✓ 按钮，源标签贴成彩色 pill 在英文 pack 名左侧，pinned/未 pinned 的 📌 状态保留；ConfigPage 删除 inline picker 全部状态 / 方法 / @Builder。先前更新：2026-05-10（新增 V0.6.6 GameConfig 旧词库字段清理：从 `models/GameConfig.ets` 删除 `enabledCategories` / `customWordsRaw` 字段与 `KNOWN_CATEGORIES` / `CUSTOM_CATEGORY_KEY` / `parseCustomWords` / `computeFinalPool` 配套常量与函数；`MIN_POOL_SIZE` 仅作为 review-mode 兜底常量保留。`services/GameConfigPersistence.ets` 不再 (de)serialize 这两个字段，旧设备升级时静默忽略。`pages/ConfigPage.ets` 删除「词库类别」chip 行（含 ConfigCategoryFruit/Place/Home/Custom）、「自定义」入口与 ConfigValidationHint，pack picker 成为唯一词源选择入口；保存路径不再做 finalPool 校验。`pages/BattlePage.ets` normal / review 模式 universe 直接用 `repo.all()`，不再做 category filter；today 模式逻辑不变。删除 `pages/CustomWordsPage.ets` 与 `main_pages.json` 中对应注册项。`harmonyos/entry/src/test/LocalUnit.test.ets` 删除 `parseCustomWords` / `computeFinalPool` 两组 describe 块与 `defaultsMatchEngineDefaults` / `cloneIsDeepEnoughToIsolateDraft` 中的旧字段断言；`harmonyos/entry/src/ohosTest/ets/test/RoutingFlow.ui.test.ets` 删除 `openConfigAndApply` / `openCustomWordsAndSave` / `setChipSelected` / `categoryChipId` / `stepStepperTo` / `TestGameConfig` / `applyConfigShared` / `resetToDefaultConfigShared` 等已死帮助函数与 `FRUIT_PROMPTS` / `CUSTOM_TEST_*` 常量，保留所有跨文件 export（`launchAppShared` / `clickByIdShared` / `tapConfigCancelShared` / `tapCorrectAnswerShared` / `tapWrongAnswerShared`）。`docs/WordMagicGame_overall_spec.md` 同步重写到 V0.6.6 reality。先前更新：2026-05-10（新增 V0.6.5.1 三层词包模型 UI 集成补丁：`WordPackBootstrapper.bootstrapPackLibrary(ctx)` offline cold-start composer 把 builtins / cached globals / cached families 合并成 `PackLibrary`；`TodayAdventureBuilder.buildFromPack(pack, ...)` 用 `pack.scene` 合成 `AdventureRegion`、用 `pack.words` 作候选池；HomePage chip row 改为由 `PackSelectionService.getActiveIds()` 经 `PackLibrary` 解析驱动，chip id 仍为 `RegionChip_<id>` 保持现有 UI 测试可用；BattlePage today-mode 读 `TODAY_ACTIVE_PACK_KEY` 用 `pack.words` 直接构 `bundleRepo`，并在「Won + 3 stars」时调 `PackSelectionService.recordPerfectAdventure` 触发 perfect-rotation；ConfigPage 新增 `📦 我的词包` 板块，含「同步词包」按钮（同时拉 global + family）、最多 5 个 active 词包的勾选 + pin 按钮、状态文案；CustomWordsPage 顶部加软性弃用 banner（兼容 `configCustomWordsOnly_oneShotVictory` 测试，仍保留编辑 / 保存）；新增 `PackNetworkFetchAdapter` 共享 `FamilyPackFetchAdapter` + `GlobalPackFetchAdapter`（HEAD + lastResponseHeader）。GameConfig 字段 cleanup（删除 `enabledCategories` / `customWordsRaw`）延后到 V0.6.6 —— review/normal mode 仍依赖这两个字段，且多个 UI 测试还在驱动 categoryRow + CustomWordsPage 的传统路径。先前更新：2026-05-09（新增 V0.6.5 三层词包模型：服务端复用 `FamilyPackDefinition` + `GLOBAL_PACK_FAMILY_ID` 哨兵实现 built-in / global / family 三层；新增 `/api/v1/admin/global-packs/**` 管理路由、`/api/v1/public/global-packs/latest.json` 匿名只读路由、`scripts/migrate_global_packs_v0_6_5.py` 默认 seed；项目级 `.cursor/rules/api-route-pattern.mdc` 沉淀 `/api/v1/{admin,public,family/{family_id}}/**` 路由约束；`legacy_route_aliases.py` 提供向前兼容期路径别名层；客户端引入 `Pack` 模型、`BuiltinPackLoader`、`GlobalPackService`、`PackLibrary`、`PackSelectionService` 五个 foundation 服务，UI 集成（HomePage chip row、ConfigPage pack picker、BattlePage repo-from-pack）延后到 V0.6.5.1。先前更新：2026-05-06（新增 V0.7.0 monorepo native clients 重排方案：仓库根目录升级为 monorepo，HarmonyOS DevEco 工程整体迁入 `harmonyos/`，iOS / Android 作为根目录原生客户端保留位，`server/` 继续作为根目录 FastAPI 后端，`shared/` 仅沉淀 contracts / schemas / golden fixtures，不走跨平台 runtime。详细方案见 [`2026-05-06-v0.7.0-monorepo-native-clients-restructure-design.md`](superpowers/specs/2026-05-06-v0.7.0-monorepo-native-clients-restructure-design.md)。先前更新：V0.5.8 家长管理后台改版完成 —— "管理员控制台"重命名为"家长管理后台"，进入即锁定竖屏；admin 路由统一移除 JWT 闸（V0.6 以家长账户隔离）；课本图上传走 `LessonImagePicker` + `MultipartBuilder` + `ParentApiClient.importLesson` POST `/admin/lessons/import` → OpenAI vision → 草稿；`LessonDraftReviewPage` 复核保留 / 编辑 / 弃用 → PATCH + /approve 或 /reject；UI 测试 `ParentAdminFlow.ui.test.ets` 增补"📷拍照 / 🖼️从相册 / 待复核草稿 / 无登录 UI"断言，新增 `LessonDraftReviewFlow.ui.test.ets` 烟雾测试。）
...
| V0.9   | AI 剧情与语境学习版      | 句子填词、主题剧情、LLM 生成剧情草稿、个性化冒险                                | 必需       |
| V0.10  | 战斗音频混音与 BGM 版 | 新增 `BattleAudioMixer`，让战斗 BGM、combo/攻击/受伤音效、单词朗读按优先级共存；用 duck + 单次恢复策略解决 TTS 抢焦点问题 | 无 |
| V0.11  | Cocos2D 战斗美术化重构版 | 用 Cocos Creator 重写战斗表现层，支持更完整的角色、怪物、动画、特效和多美术主题           | 可选       |
...
V0.9 的目标是让学习从单词识别进入语境理解，并让战斗更有故事感。
...
7. **先完成 V0.10 战斗音频策略，再启动 Cocos2D 重构**：BGM、SFX 与单词朗读的优先级边界应先在 ArkUI 战斗原型里跑稳，再进入 V0.11 表现层迁移。

推荐近期第一批实施计划围绕 V0.3，不要同时启动服务端后台。后台等客户端题型、词状态和奖励模型跑通后再做，字段会更稳定。
...
这条路线能让游戏更完整，也为未来 FastAPI 后台、LLM 题库、家长账号、二维码绑定和 AI 剧情留下稳定演进空间。
