...
- **三端原生客户端（V0.7.1 bootstrap parity，已交付）**：
  - **HarmonyOS**（`harmonyos/`，`app.json5` 0.9.1 / 1009001）：17 页闭环；三层 `Pack` + `PackSelectionService`；ohosTest UI 套件覆盖主路径；V0.8.3 polish、V0.8.4 scheduler core、V0.8.6 怪物等级积分金币、V0.8.7 复习入口语义与 V0.8.8 daily check-in 已落地；V0.9.1 句子填词已实现，full-suite UI gate 79/79 通过。
...
| V0.9 系列（6 子版本）/ V0.10 / V0.11 | V0.9.2 Done | V0.9.1 句子填词与 V0.9.2 Boss 个性 / 登场对话均已三端对齐并合入 main。V0.9.2 交付内容：100 Boss 双语台词、通用 `MessageBubble` + Lab、Boss 入场气泡、怪物等级标签、15-word built-in packs、10/5/10 初装战斗默认值、严格题型阶段调度、选中词包再战不串包；退场气泡保持关闭，留到未来 exit-effect 设计。下一步：V0.9.3 区域剧情卡 + 章节完成庆祝，之后 V0.9.4–V0.9.6、V0.10、V0.11（顺序见 §23） |
...
| V0.9.1 | 三端 full UI green / screenshots partial | 句子填词题型（AI 语境第一步）  | HarmonyOS / iOS / Android 均已新增 `QuestionKind.SentenceCloze`：例句留空目标词，3 选 1 填回；复用现有 `example.{en,zh}`；默认开启并纳入题型开关与 Challenge pool；内置 5 词包补齐 example；缺例句的词自动回退；默认不自动朗读答案词，只有主动点击喇叭才发音。2026-05-25 三端 full UI 自动化通过：HarmonyOS 79/79、iOS 28/28、Android 34/34；screenshots 仍为 partial。设计：[`docs/features/2026-05-24-sentence-cloze-v0-9-1/00-design.md`](features/2026-05-24-sentence-cloze-v0-9-1/00-design.md) | 复用既有例句字段 |
| V0.9.2 | Done | Boss 个性与登场对话 | 三端已交付：100 个 Boss 双语台词；所有等级统一使用非阻塞 `BattleBossIntro*` `MessageBubble`；SuperBoss 不再使用独立大 banner；同局同 catalog 怪物重复出现时抑制重复 intro；Boss 退场气泡关闭并保留文案给未来 exit-effect；怪物卡显示 `L1` / `L2` / `L3` / `L4`；5 个内置词包扩到 15 词；初装默认 `10 monsters / monster HP 5 / player HP 10`；严格题型阶段调度与选中词包再战修复。设计：[`docs/features/2026-05-25-boss-dialogue-v0-9-2/00-design.md`](features/2026-05-25-boss-dialogue-v0-9-2/00-design.md) | 无 |
...
| **V0.9.2** | Boss 个性与登场对话 | 故事性最低成本切入 | Done：100 Boss 双语台词、统一 `MessageBubble` 入场气泡、怪物等级标签、15-word built-ins、10/5/10 初装默认、严格题型阶段调度；Boss 退场效果延后单独设计 |
...
> **2026-05-26 刷新。** V0.3–**V0.8.2+**（含三端 bootstrap、家长词库、管理员后台、admin global pack delete、admin / parent family pack delete、draft pack split）已在主干交付；V0.9.1 句子填词已完成三端 full UI 自动化收尾；V0.9.2 Boss 个性与登场对话已三端对齐并合入 main。V0.8.6 怪物等级积分金币与 V0.8.8 每日打卡已完成三端对齐与模拟器验收，V0.8.7 复习入口近期错词策略已三端代码对齐，iOS / Android targeted UI 验证已通过。短期重点：进入 V0.9.3 区域剧情卡，同时继续 V0.8.3 / V0.8.4 verification closeout 与 V0.9.1 release gate cleanup。
...
3. **V0.9.3 区域剧情卡 + 章节完成庆祝**：V0.9.2 已完成，下一项故事感增量从 Pack/region 层切入；先做 Harmony design + plan，再走三端 SOP。
4. **V0.9.1 release gate cleanup**：功能已三端 green；剩余截图 / release checklist 收口。
5. **V0.10 战斗音频**：三端战斗层落地 `BattleAudioMixer`（Harmony 先稳，`SpellQuestionFlow` 连跑 5 次全绿后再合入 BGM）。在 V0.9.3 完成后启动以稳为先。
6. **V0.9.4–V0.9.6**：听音 + 魔法书 → 个性化今日 + 错词幽灵 → LLM 辅助内容生产。
...
下一步最适合推进的主题（2026-05-25）：

```text
V0.9.1 screenshot gate + release gate cleanup
  ↓
...
V0.3–V0.8.8 已构成当前产品的 **三端可玩、可绑、可运营词库、可后台排查、可日常留存** 底座；V0.9.1 已完成三端 full UI 自动化收尾，但发版仍要尊重 feature folder、screenshot gate 和后续 parity checklist。V0.8.3–V0.8.4 的剩余事项主要是验证与 release gate hygiene，可与 V0.9.1 screenshot/release closeout 并行推进。
