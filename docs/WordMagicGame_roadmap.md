...
- **三端原生客户端（V0.7.1 bootstrap parity，已交付）**：
  - **HarmonyOS**（`harmonyos/`，`app.json5` 0.9.1 / 1009001）：17 页闭环；三层 `Pack` + `PackSelectionService`；ohosTest UI 套件覆盖主路径；V0.8.3 polish、V0.8.4 scheduler core、V0.8.6 怪物等级积分金币、V0.8.7 复习入口语义与 V0.8.8 daily check-in 已落地；V0.9.1 句子填词已实现，full-suite UI gate 仍待复跑。
...
| V0.9 系列（6 子版本）/ V0.10 / V0.11 | V0.9.1 三端代码对齐完成 / screenshots pending | V0.9.1 Harmony 已实现并部分过 gate；`replication_approved: true` 后 iOS / Android 对齐已完成 focused unit/build/UI 验证。剩余：截图刷新、Harmony full UI suite 复跑与 release gate cleanup。V0.9.2–V0.9.6（Boss 个性→区域剧情→听音+魔法书→个性化今日→LLM 辅助内容）、V0.10 战斗 BGM 混音、V0.11 Cocos（顺序见 §23） |
...
| V0.9.1 | 三端代码对齐完成 / screenshots pending | 句子填词题型（AI 语境第一步）  | HarmonyOS / iOS / Android 均已新增 `QuestionKind.SentenceCloze`：例句留空目标词，3 选 1 填回；复用现有 `example.{en,zh}`；默认开启并纳入题型开关与 Challenge pool；内置 5 词包补齐 example；缺例句的词自动回退；默认不自动朗读答案词，只有主动点击喇叭才发音。Harmony targeted BattleFlow ohosTest、iOS focused XCTest/XCUITest、Android unit/build/focused Compose UI 均通过；screenshots / Harmony full UI suite 仍待补。设计：[`docs/features/2026-05-24-sentence-cloze-v0-9-1/00-design.md`](features/2026-05-24-sentence-cloze-v0-9-1/00-design.md) | 复用既有例句字段 |
...
> **2026-05-24 刷新。** V0.3–**V0.8.2+**（含三端 bootstrap、家长词库、管理员后台、admin global pack delete、admin / parent family pack delete、draft pack split）已在主干交付；iOS / Android app metadata 仍为 **0.8.8 / 1008008**，HarmonyOS 已进入 **0.9.1 / 1009001** 句子填词实现分支。V0.8.6 怪物等级积分金币与 V0.8.8 每日打卡已完成三端对齐与模拟器验收，V0.8.7 复习入口近期错词策略已三端代码对齐，iOS / Android targeted UI 验证已通过。短期重点：V0.9.1 Harmony full-suite UI gate 与截图补齐，然后人工签署 replication trigger；并继续 V0.8.3 / V0.8.4 verification closeout。
...
3. **V0.9.1 句子填词 Harmony gate**：Harmony implementation 已完成，unit / HAP / CodeLinter / targeted BattleFlow 通过；full `scripts/run_ui_tests.sh` 本轮未取得 OHOS 报告，screenshots 与人工签字仍待补齐，iOS / Android 复制不得启动。
4. **V0.9.2 / V0.9.3 故事感增量**：Boss 个性 + 区域剧情卡（低代码量、高情绪 ROI）。
...
```text
V0.9.1 Harmony full gate（full ohosTest / screenshots / 人工签字）
  ↓ 签署 20-replication-trigger.md 后复制
V0.9 系列（6 子版本，docs/features/ 三端 SOP）
  → V0.9.1 句子填词 iOS / Android replication
  → V0.9.2 Boss 个性
  → V0.9.3 区域剧情卡 + 章节完成庆祝
...
V0.3–V0.8.8 已构成当前产品的 **三端可玩、可绑、可运营词库、可后台排查、可日常留存** 底座；V0.9.1 已在 HarmonyOS 完成第一轮实现，但复制与发版仍要尊重 feature folder、full soft gate、签字 gate 和后续 parity checklist。V0.8.3–V0.8.4 的剩余事项主要是验证与 release gate hygiene，可与 V0.9.1 gate closeout 并行推进。
