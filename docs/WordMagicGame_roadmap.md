...
> 最近更新：2026-04-29（V0.3.9 魔法愿望单兑换流程重构：6 位家长 PIN + GiftBox 庆祝动画 + 持久化兑换记录页；V0.3.8 怪物图鉴扩展到 10 张 + 7 boss 接入战斗 + V0.2 颜色变种 Slime 退役）
...
| V0.3.9 | 魔法愿望单兑换流程重构      | 用 6 位家长 PIN 弹窗替代 3 秒长按门；兑换成功播放 GiftBox 庆祝动画（~1.68s 播 + 1.5s 停），期间遮罩 + HitTestMode.Block 屏蔽所有交互；新增持久化兑换记录页（cap 50，最新在前），从愿望单头部 📜 入口进入；卡片版式改为左侧名+价、右侧奖品 emoji + 申请兑换按钮并排靠右；Pending 中间态退役（reader 兼容旧持久化）；ConfigPage 加家长密码入口行 | 无 |
