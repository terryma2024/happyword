...
V0.8.1 聚焦家长自己的词库工作台，路径仍然放在现有 `/parent/` 家长 Web 后台里。每个家长/家庭都作为一个租户，服务端以 `family_id` 做真实隔离边界，客户端可传家长/家庭编号作为显式上下文，但授权必须从 `device_token -> DeviceBinding -> family_id` 反查。

建议能力：

- `/parent/packs` 词库列表：展示 active / archived、草稿词数、当前发布版本、是否有未发布修改。
- `/parent/packs/{pack_id}` 词库详情：展示元信息、草稿表格、发布状态和回滚入口。
...
- 孩子设备拉取：新增 `/api/v1/child/packs/latest.json`，返回 global 词库 + 当前 family 词库的合并结果。
...
V0.8.2 新增独立 `/admin/` Web 后台，面向系统管理员/运营者，不混入 `/parent/` 家长后台。
