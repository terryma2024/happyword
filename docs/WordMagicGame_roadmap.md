...
> 最近更新：**2026-05-29** — 主干已推进到 **0.9.5 / 1009005**。V0.8.3 **战斗与词包体验小优化**已按产品判断关闭：10 词包激活 / 自动轮换、怪物等级与 bonus / HP-2 重击、伤害浮字三端语义均已落地。V0.9.5 **魔法书图鉴**已完成：`HomeSpellbookButton`、`HomePackSpellbookCover`、`SpellbookPage`、locked/seen/mastered 词卡、词详情、按 pack 进度与一次性 +50 魔法币奖励均已三端落地；三端版本号已同步为 `0.9.5 / 1009005`；服务端 spellbook cover 生成、Blob 上传、admin / parent 触发入口与 OpenAPI 契约也已合入。原计划的推荐 / 章节感与 LLM 内容扩量方向已从路线图移除，不再作为后续版本推进。设计基线见 [`docs/features/2026-05-29-spellbook-v0-9-5/00-design.md`](features/2026-05-29-spellbook-v0-9-5/00-design.md)、[`docs/features/2026-05-26-learning-plan-review-v0-9-3/00-design.md`](features/2026-05-26-learning-plan-review-v0-9-3/00-design.md)、[`docs/features/2026-05-24-sentence-cloze-v0-9-1/00-design.md`](features/2026-05-24-sentence-cloze-v0-9-1/00-design.md)、[`docs/features/2026-05-25-boss-dialogue-v0-9-2/00-design.md`](features/2026-05-25-boss-dialogue-v0-9-2/00-design.md)、[`docs/features/2026-05-18-battle-polish-v0-8-3/00-design.md`](features/2026-05-18-battle-polish-v0-8-3/00-design.md)、[`docs/features/2026-05-18-battle-balance-v0-8-4/00-design.md`](features/2026-05-18-battle-balance-v0-8-4/00-design.md)、[`docs/features/2026-05-23-coin-reward-by-monster-level-v0-8-6/00-design.md`](features/2026-05-23-coin-reward-by-monster-level-v0-8-6/00-design.md) 与 [`docs/features/2026-05-23-daily-checkin-v0-8-8/00-design.md`](features/2026-05-23-daily-checkin-v0-8-8/00-design.md)。
...
- **仍未实现的边界。** 学习报告仍是 **device-local**，未做云端按 pack 聚合；战斗 BGM / `BattleAudioMixer` 与 Cocos2D 战斗表现层仍未实现。推荐 / 章节感与 LLM 内容扩量方向已移出路线图。

因此后续路线不应回到「从零搭原型」或「单端 replica / 后台追平」，而应围绕 **V0.10 战斗音频、V0.11 战斗表现层** 继续演进。
...
| V0.8.3 | 已完成 | 战斗与词包体验小优化 | 三端已完成：① 词包激活上限 5→10，且满 10 时自动关闭"队首未 pin"词包并 toast；② 怪物分 4 级（初/中/高/Super = 10/60/20/10），30% 高级/Super 怪带 bonus 标记（V0.8.3 原始结算为 ✨×1.3，已被 V0.8.6 怪物等级积分金币替代），50% 高级/Super 攻击造成 HP-2；③ `DamageFloaterLabel` 在玩家与怪物受击时从头顶升起。详见 [`docs/features/2026-05-18-battle-polish-v0-8-3/50-parity-checklist.md`](features/2026-05-18-battle-polish-v0-8-3/50-parity-checklist.md) | 无 |
| V0.8.4 | 进行中 | 战斗平衡与题型节奏 | 三端 core 已实现：魔法师默认 HP 10；`Spell` 错点字母每次 -1 HP；今日战斗：每词最多 1 次中文选词 + 1 次单字母填空，之后仅双字母填空 / 多字母选择各 50%。剩余 gate 是 targeted tests / release checklist；详见 [`docs/features/2026-05-18-battle-balance-v0-8-4/50-parity-checklist.md`](features/2026-05-18-battle-balance-v0-8-4/50-parity-checklist.md) | 无 |
...
> **状态：已完成。** 详细 cross-platform 设计：[`docs/features/2026-05-18-battle-polish-v0-8-3/00-design.md`](features/2026-05-18-battle-polish-v0-8-3/00-design.md)。产品 owner 已确认 V0.8.3 不再作为 verification 阻塞项。
...
V0.8.4 验收（三端 core 已有；release gate 仍需处理未重跑的完整套件）：
...
V0.9 的目标是让学习从单词识别进入语境理解，并把“今天该学什么 / 什么时候复习”讲清楚，同时逐步增强词包本身的故事感。V0.9 已按实际交付收束为 **5 个可独立发布的子版本**：题型 → 角色反馈 → 学习计划 → 词包故事 → 魔法书收集。AI 始终是"后台辅助内容生产 + 人工审核"，**儿童端不直接调 LLM**。

每个子版本都走三端 SOP：`docs/features/<feature-id>/` Harmony 先实现 → `20-replication-trigger.md` 签字 → iOS / Android 并行复制。原推荐 / 章节感与 LLM 内容扩量方向已从路线图移除，不再作为 planned work。
...
| **V0.9.5** | 魔法书图鉴 | 把掌握词沉淀成可收集、可回看的成长记录 | 已完成：新 `SpellbookPage`；按 pack 分组展示掌握词卡、未解锁词卡和收集进度；HomePage 增 `HomeSpellbookButton` 与 `HomePackSpellbookCover`；一次性 +50 魔法币奖励；服务端 spellbook cover 生成链路已合入 |

设计原则：

- AI 内容 **只在服务端生成**，进 `LlmDraft` 草稿 → 家长 / 系统管理员审核 → 发布后才进客户端。
- 优先**复用已有字段**：V0.9.1 用现成 `example`，V0.9.4 复用 `Pack.scene.storyEn/storyZh`，V0.9.5 复用现有 `WordStat.memoryState`、pack library 与学习报告聚合，不新增题型或服务端契约。
...
V0.9.5 已完成。本版没有新增听音题型，而是把现有掌握度、pack library 与词包故事基础转成孩子可见的收集进度：打开魔法书、看每个 pack 的封面与词卡状态、点开已见 / 已掌握词卡回看，并在整包掌握后领取一次性魔法币奖励。
...
产品 owner 已确认 V0.9.5 关闭；截图刷新与额外复跑不再作为路线图阻塞项。
...
V0.10 的 PcmAudioLab 已验证“逻辑混音 + CoreSpeechKit 直接播放 TTS”在当前环境不可用，且仍可能让 BGM 停止，因此 HarmonyOS 首选路线调整为：BGM/SFX 继续保留独立 lane，TTS 先通过 CoreSpeechKit 合成 PCM，再由 app 自有 `AudioRenderer` 播放。PcmAudioLab 不再暴露系统直播放音对照/切换，避免把不可用路径误认为可选方案。
...
> **2026-05-29 刷新。** V0.3–**V0.8.2+**（含三端 bootstrap、家长词库、管理员后台、admin global pack delete、admin / parent family pack delete、draft pack split）已在主干交付；V0.8.3 战斗与词包体验小优化已按产品判断关闭；V0.9.1 句子填词、V0.9.2 Boss 个性 / 登场对话、V0.9.3 学习计划与复习逻辑重构、V0.9.4 词包故事与提审版本硬化、V0.9.5 魔法书图鉴均已三端对齐并合入 main。推荐 / 章节感与 LLM 内容扩量方向已移出路线图。短期重点：继续 V0.8.4 release gate hygiene，随后进入 V0.10 战斗音频。
...
2. **V0.10 战斗音频**：三端战斗层落地 `BattleAudioMixer`（Harmony 先稳，`SpellQuestionFlow` 连跑 5 次全绿后再合入 BGM）。在复习入口与现有题型继续稳定后启动，以稳为先。
3. **V0.9.1–V0.9.5 release notes hygiene**：已完成，不再作为短期阻塞项；后续仅随发版清单同步引用。
4. **V0.11 Cocos 战斗表现层**：玩法与音频稳定后再迁移表现层。
...
V0.3–V0.9.5 已构成当前产品的 **三端可玩、可绑、可运营词库、可后台排查、可日常留存、可语境练习、可故事化 Boss 入场、可解释今日复习、可展示词包故事、可收藏掌握词** 底座。V0.8.4 的剩余事项主要是 release gate hygiene；推荐 / 章节感与 LLM 内容扩量方向不再推进。
