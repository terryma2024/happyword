...
- **三端原生客户端（V0.7.1 bootstrap parity，已交付）**：
  - **HarmonyOS**（`harmonyos/`，`app.json5` 0.9.2 / 1009002）：17 页闭环；三层 `Pack` + `PackSelectionService`；ohosTest UI 套件覆盖主路径；V0.8.3 polish、V0.8.4 scheduler core、V0.8.6 怪物等级积分金币、V0.8.7 复习入口语义、V0.8.8 daily check-in、V0.9.1 句子填词与 V0.9.2 Boss 个性 / 登场对话已落地。
  - **iOS**（`ios/WordMagicGame/`，Swift / SwiftUI，0.9.2 / 1009002）：与 Harmony 同矩阵——核心环（Home / Battle / Result）、成长环（PackManager、愿望单、图鉴、今日计划、按词包学习报告）、绑定与家长 PIN/后台、Debug 路由；V0.8.4 scheduler core、V0.8.3 polish parity、V0.8.6 怪物等级积分金币、V0.8.7 复习入口语义、V0.8.8 daily check-in、V0.9.1 sentence-cloze 与 V0.9.2 boss-dialogue 已落到代码层，release-pre 仍保留生产 backend 给 Apple review 的 P0 确认项。
  - **Android**（`android/`，Kotlin / Compose，0.9.2 / 1009002）：17 页均已实现；V0.8.4 scheduler core、V0.8.3 polish parity、V0.8.6 怪物等级积分金币、V0.8.7 复习入口语义、V0.8.8 daily check-in、V0.9.1 sentence-cloze 与 V0.9.2 boss-dialogue 已落到代码层；Phase 5 发布硬化门禁见 [`docs/android-replica/07-release-readiness-checklist.md`](android-replica/07-release-readiness-checklist.md)。
  - **对齐边界**：三端共享 `shared/fixtures` 解码契约；V0.9.1 句子填词与 V0.9.2 Boss 个性 / 登场对话已完成三端代码对齐；V0.10 战斗 BGM、V0.11 Cocos 未实现；学习报告仍为 **device-local**（未上云按 pack 聚合）。
...
| V0.9.1 | Done | 句子填词题型（AI 语境第一步）  | HarmonyOS / iOS / Android 均已新增 `QuestionKind.SentenceCloze`：例句留空目标词，3 选 1 填回；复用现有 `example.{en,zh}`；默认开启并纳入题型开关与 Challenge pool；内置 5 词包补齐 example；缺例句的词自动回退；默认不自动朗读答案词，只有主动点击喇叭才发音。2026-05-25 三端 full UI 自动化通过：HarmonyOS 79/79、iOS 28/28、Android 34/34；截图 / 视觉验证证据已接受为 V0.9.1 closeout。设计：[`docs/features/2026-05-24-sentence-cloze-v0-9-1/00-design.md`](features/2026-05-24-sentence-cloze-v0-9-1/00-design.md) | 复用既有例句字段 |
...
> **2026-05-26 刷新。** V0.3–**V0.8.2+**（含三端 bootstrap、家长词库、管理员后台、admin global pack delete、admin / parent family pack delete、draft pack split）已在主干交付；V0.9.1 句子填词与 V0.9.2 Boss 个性 / 登场对话均已三端对齐并合入 main。V0.8.6 怪物等级积分金币与 V0.8.8 每日打卡已完成三端对齐与模拟器验收，V0.8.7 复习入口近期错词策略已三端代码对齐，iOS / Android targeted UI 验证已通过。短期重点：进入 V0.9.3 区域剧情卡，同时继续 V0.8.3 / V0.8.4 verification closeout。
...
4. **V0.9.4–V0.9.6 内容与学习增强**：听音 + 魔法书 → 个性化今日 + 错词幽灵 → LLM 辅助内容生产。
5. **V0.10 战斗音频**：三端战斗层落地 `BattleAudioMixer`（Harmony 先稳，`SpellQuestionFlow` 连跑 5 次全绿后再合入 BGM）。在 V0.9.3 完成后启动以稳为先。
6. **V0.9.1 / V0.9.2 release notes hygiene**：已完成，不再作为短期阻塞项；后续仅随发版清单同步引用。
...
V0.3–V0.9.2 已构成当前产品的 **三端可玩、可绑、可运营词库、可后台排查、可日常留存、可语境练习、可故事化 Boss 入场** 底座。V0.8.3–V0.8.4 的剩余事项主要是验证与 release gate hygiene，可与 V0.9.3 设计推进并行处理。
