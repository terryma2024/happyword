...
| V0.9.1 | Harmony implemented / soft gate partial | 句子填词题型（AI 语境第一步）  | HarmonyOS 已新增 `QuestionKind.SentenceCloze`：例句留空目标词，3 选 1 填回；复用现有 `example.{en,zh}`；默认开启并纳入题型开关与 Challenge pool；内置 5 词包补齐 example；缺例句的词自动回退。targeted BattleFlow ohosTest 已通过；full suite / screenshots / 人工签字仍待完成。设计：[`docs/features/2026-05-24-sentence-cloze-v0-9-1/00-design.md`](features/2026-05-24-sentence-cloze-v0-9-1/00-design.md) | 复用既有例句字段 |
...
| 子版本 | 主题 | 趣味 Δ | 关键改动 |
| --- | --- | --- | --- |
| **V0.9.1** | 句子填词题型 | 把"识别"升级为"语境理解" | 新 `QuestionKind.SentenceCloze`；复用 `example.{en,zh}`；默认开启并进入 Challenge 池；5 个内置词包补 example；设计见 [`2026-05-24-sentence-cloze-v0-9-1/00-design.md`](features/2026-05-24-sentence-cloze-v0-9-1/00-design.md) |
