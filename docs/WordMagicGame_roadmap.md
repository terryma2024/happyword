...
> 最近更新：**2026-05-24** — V0.8.7 复习按钮 bugfix 已三端对齐：Home toolbar 复习入口读取近期回答错误词，若存在则以这些错词作为临时 focused word plan 开战（1 个错词也可启动，选择题干扰项仍可来自完整词库）；若近期无错词则显示 `先答错几题再来复习吧` toast。HarmonyOS 同步修正了旧的 3 错词解锁与小词池退回全量词库行为；iOS 顶部 review 图标从误接「今日计划」改为真正复习入口；Android 从固定 toast 改为可启动复习战斗，并补齐本局 3 怪配置传入 BattleScreen 的 UI 状态。UI 自动化新增覆盖「空错词 toast → 答错 1 个词 → 复习战斗首题为该错词」。验证：iOS targeted XCUITest 通过，Android targeted connected UI test 通过，Android instrumentation Kotlin 编译通过；HarmonyOS ohosTest 断言已补强但本机 runner 本轮无报告码输出后中断，需后续在稳定设备通道复跑。此前 Roadmap gate cleanup 已确认 HarmonyOS / Android 为 **0.8.4 / 1008004**，iOS 为 **0.8.4 / 1008006**；V0.8.3 polish parity、admin global pack hard delete 与 phrase parity 仍纳入当前评估。设计基线见 [`2026-05-08-v0.8-backoffice-and-vocabulary-management-design.md`](superpowers/specs/2026-05-08-v0.8-backoffice-and-vocabulary-management-design.md)、[`docs/features/2026-05-18-battle-polish-v0-8-3/00-design.md`](features/2026-05-18-battle-polish-v0-8-3/00-design.md) 与 [`docs/features/2026-05-18-battle-balance-v0-8-4/00-design.md`](features/2026-05-18-battle-balance-v0-8-4/00-design.md)。
...
> **基线日期：2026-05-23。** 下文「已交付」指本仓库主干可构建、可测、可部署的实现；「规划中」指路线图目标尚未合入或仅设计文档。
...
- **三端原生客户端（V0.7.1 bootstrap parity，已交付）**：
  - **HarmonyOS**（`harmonyos/`，`app.json5` 0.8.4 / 1008004）：17 页闭环；三层 `Pack` + `PackSelectionService`；ohosTest UI 套件覆盖主路径；V0.8.3 polish 与 V0.8.4 scheduler core 已落地；V0.8.7 复习入口改为「有近期错词即临时词包开战，无错词 toast」。
  - **iOS**（`ios/WordMagicGame/`，Swift / SwiftUI，0.8.4 / 1008006）：与 Harmony 同矩阵——核心环（Home / Battle / Result）、成长环（PackManager、愿望单、图鉴、今日计划、按词包学习报告）、绑定与家长 PIN/后台、Debug 路由；V0.8.4 scheduler core、V0.8.3 polish parity 与 V0.8.7 复习入口语义已落到代码层，release-pre 仍保留生产 backend 给 Apple review 的 P0 确认项。
  - **Android**（`android/`，Kotlin / Compose，0.8.4 / 1008004）：17 页均已实现；V0.8.4 scheduler core、V0.8.3 polish parity 与 V0.8.7 复习入口语义已落到代码层；Phase 5 发布硬化门禁见 [`docs/android-replica/07-release-readiness-checklist.md`](android-replica/07-release-readiness-checklist.md)。
...
- **30% bonus 怪物**（仅高级 / Super）：生成时按 30% 概率打 `bonus = true`，怪物头顶贴 ✨ 金星（id `MonsterBonusStar`）。通关结算时本场击杀 bonus 怪物数为 K → ✨ 魔法币奖励 × 1.3 向上取整（⭐ 保留 0–3 cap）；ResultPage 加 `BattleResultBonusCoinRow`：`Bonus 怪物 ×K → +M ✨`。
...
> **2026-05-23 刷新。** V0.3–**V0.8.2+**（含三端 bootstrap、家长词库、管理员后台、admin global pack delete）已在主干交付；HarmonyOS / Android app metadata 为 **0.8.4 / 1008004**，iOS 为 **0.8.4 / 1008006**。V0.8.3 replica gaps 已补到 iOS / Android 代码层；短期重点改为开发机 verification + V0.8.4 release gate cleanup。V0.9 已拆为 6 个子版本。
