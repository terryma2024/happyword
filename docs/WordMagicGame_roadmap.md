...
> 最近更新：**2026-05-29** — 主干已推进到 **0.9.4 / 1009004**。V0.9.3 学习计划与复习逻辑重构已完成三端对齐：本地 `YYYYMMDD` daily state、稳定 daily review snapshot、A/B 完成语义、Home 复习计数与 `TodayPlanPage` 进度均已落地，验收为 HarmonyOS 81/81、iOS UI 30/30、Android 37/37。V0.9.4 实际落地为 **词包故事与提审版本硬化**：三端 `SceneMetadata.storyEn/storyZh` 解析与首页故事行、内置 5 词包故事文案、global/family pack story 保留合并、家长 / 管理员后台 story 编辑与 LLM 生成、OpenAPI 同步、三端版本号 0.9.4，以及 iOS `0.9.4 (1009004)` App Store Connect 上传并于 2026-05-29 提审。原计划的听音题型与魔法书图鉴顺延到 V0.9.5。设计基线见 [`docs/features/2026-05-26-learning-plan-review-v0-9-3/00-design.md`](features/2026-05-26-learning-plan-review-v0-9-3/00-design.md)、[`docs/features/2026-05-24-sentence-cloze-v0-9-1/00-design.md`](features/2026-05-24-sentence-cloze-v0-9-1/00-design.md)、[`docs/features/2026-05-25-boss-dialogue-v0-9-2/00-design.md`](features/2026-05-25-boss-dialogue-v0-9-2/00-design.md)、[`docs/features/2026-05-18-battle-polish-v0-8-3/00-design.md`](features/2026-05-18-battle-polish-v0-8-3/00-design.md)、[`docs/features/2026-05-18-battle-balance-v0-8-4/00-design.md`](features/2026-05-18-battle-balance-v0-8-4/00-design.md)、[`docs/features/2026-05-23-coin-reward-by-monster-level-v0-8-6/00-design.md`](features/2026-05-23-coin-reward-by-monster-level-v0-8-6/00-design.md) 与 [`docs/features/2026-05-23-daily-checkin-v0-8-8/00-design.md`](features/2026-05-23-daily-checkin-v0-8-8/00-design.md)。
...
- **仍未实现的边界。** 学习报告仍是 **device-local**，未做云端按 pack 聚合；听音题型、魔法书图鉴、个性化推荐、战斗 BGM / `BattleAudioMixer` 与 Cocos2D 战斗表现层仍未实现。

因此后续路线不应回到「从零搭原型」或「单端 replica / 后台追平」，而应围绕 **V0.9.5 听音与收集、V0.9.6 个性化推荐、V0.10 战斗音频、V0.11 战斗表现层** 继续演进。
...
| V0.9.5 | 计划中 | 听音题型 + 魔法书图鉴 | 新 `QuestionKind.ListenAndPick`（点喇叭→3 选 1 释义）；新 `SpellbookPage`（按 pack 分组的已掌握词收集进度） | 复用 `audioUrl` |
...
V0.9 的目标是让学习从单词识别进入语境理解，并把“今天该学什么 / 什么时候复习 / 为什么推荐这一局”讲清楚，同时逐步增强词包本身的故事感。V0.9 已按实际交付滚动拆分为 **7 个可独立发布的子版本**：题型 → 角色反馈 → 学习计划 → 词包故事 → 听音与收集 → 推荐与章节感 → 内容工厂。AI 始终是"后台辅助内容生产 + 人工审核"，**儿童端不直接调 LLM**。
...
| **V0.9.5** | 听音题型 + 魔法书图鉴 | 把 `audioUrl` 资产用进玩法 + 收集成就感 | 新 `QuestionKind.ListenAndPick`；新 `SpellbookPage`；HomePage 增 `HomeSpellbookButton` |
...
- 优先**复用已有字段**：V0.9.1 用现成 `example`，V0.9.4 复用 `Pack.scene.storyEn/storyZh`，V0.9.5 用现成 `audioUrl`，V0.9.6 优先复用 V0.9.3 计划模型、V0.9.4 pack story 与现有 `WordStat`。
...
V0.9.4 的实际交付目标从原先“听音题型 + 魔法书图鉴”调整为更低风险的内容与发布硬化：先让每个词包拥有一条可审核、可发布、三端可展示的短故事，并完成 `0.9.4 / 1009004` 版本号同步与 iOS 提审准备。听音题型与魔法书图鉴顺延到 V0.9.5。
...
3. **V0.9.5 听音题型 + 魔法书图鉴**：原 V0.9.4 目标顺延到此；先复用 `audioUrl` 做 `ListenAndPick`，再把掌握词沉淀到 `SpellbookPage`。
4. **V0.9.6–V0.9.7 内容与学习增强**：个性化今日推荐 + 区域章节感 → LLM 辅助内容生产。
5. **V0.10 战斗音频**：三端战斗层落地 `BattleAudioMixer`（Harmony 先稳，`SpellQuestionFlow` 连跑 5 次全绿后再合入 BGM）。在听音题型与复习入口继续稳定后启动，以稳为先。
...
V0.3–V0.9.4 已构成当前产品的 **三端可玩、可绑、可运营词库、可后台排查、可日常留存、可语境练习、可故事化 Boss 入场、可解释今日复习、可展示词包故事** 底座。V0.8.3–V0.8.4 的剩余事项主要是验证与 release gate hygiene，可与 V0.9.5 听音题型设计推进并行处理。
