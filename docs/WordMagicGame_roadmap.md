...
> 最近更新：**2026-05-24** — **V0.9.1 句子填词题型** 已完成三端 focused implementation / verification pass：新增 `QuestionKind.SentenceCloze`、例句 cloze generator、默认开启的题型开关、Challenge pool 调度、Battle UI 稳定 ID、5 个内置词包全量 `example.en/zh` 回填；sentence-cloze 默认不自动朗读答案词，只保留主动点击喇叭发音；三端 metadata/version 提升到 **0.9.1 / 1009001**（Android `1_009_001`）。当前 gate：HarmonyOS unit / HAP / CodeLinter / built-in example validation / targeted `BattleFlow` ohosTest 通过，full `scripts/run_ui_tests.sh` 本轮 runner 卡住未取得 OHOS 报告；`20-replication-trigger.md` 已签 `replication_approved: true`，iOS / Android focused unit/build/UI 验证通过；Stage 5 剩余截图刷新与 Harmony full UI suite 复跑。同步刷新：**V0.8.3 战斗 polish** 与 **V0.8.4 战斗平衡** parity checklist 已全绿或显式 N/A，路线图状态收口为已完成。上一基线 **V0.8.8 每日打卡与连续奖励** 已完成 HarmonyOS + server 基线、iOS / Android 功能对齐与三端模拟器验收。设计基线见 [`docs/features/2026-05-24-sentence-cloze-v0-9-1/00-design.md`](features/2026-05-24-sentence-cloze-v0-9-1/00-design.md)、[`2026-05-24-admin-family-pack-delete-design.md`](superpowers/specs/2026-05-24-admin-family-pack-delete-design.md)、[`2026-05-23-draft-pack-split-design.md`](superpowers/specs/2026-05-23-draft-pack-split-design.md)、[`2026-05-08-v0.8-backoffice-and-vocabulary-management-design.md`](superpowers/specs/2026-05-08-v0.8-backoffice-and-vocabulary-management-design.md)、[`docs/features/2026-05-18-battle-polish-v0-8-3/00-design.md`](features/2026-05-18-battle-polish-v0-8-3/00-design.md)、[`docs/features/2026-05-18-battle-balance-v0-8-4/00-design.md`](features/2026-05-18-battle-balance-v0-8-4/00-design.md)、[`docs/features/2026-05-23-coin-reward-by-monster-level-v0-8-6/00-design.md`](features/2026-05-23-coin-reward-by-monster-level-v0-8-6/00-design.md) 与 [`docs/features/2026-05-23-daily-checkin-v0-8-8/00-design.md`](features/2026-05-23-daily-checkin-v0-8-8/00-design.md)。
...
> **基线日期：2026-05-24。** 下文「已交付」指本仓库主干可构建、可测、可部署的实现；「规划中」指路线图目标尚未合入或仅设计文档。

当前工程是 **monorepo 产品工程**：**HarmonyOS、iOS、Android 三端在 bootstrap 范围内功能已对齐**（同一 17 页路由矩阵与 V0.6.7.8 词包/报告语义）；HarmonyOS 仍为设计与 ohosTest 的权威参考实现；服务端为内容与账户权威源。

- **Monorepo（V0.7.0，已交付）**：根目录并列 `harmonyos/`、`ios/`、`android/`、`server/`、`shared/`；DevEco 打开 `harmonyos/`；`shared/` 仅 contracts / schemas / fixtures。
- **三端原生客户端（V0.7.1 bootstrap parity，已交付）**：
  - **HarmonyOS**（`harmonyos/`，`app.json5` 0.9.1 / 1009001）：17 页闭环；三层 `Pack` + `PackSelectionService`；ohosTest UI 套件覆盖主路径；V0.8.3 polish、V0.8.4 scheduler core、V0.8.6 怪物等级积分金币、V0.8.7 复习入口语义与 V0.8.8 daily check-in 已落地；V0.9.1 句子填词已实现，targeted `BattleFlow` gate 通过，full-suite UI gate 与截图仍待复跑/刷新。
  - **iOS**（`ios/WordMagicGame/`，Swift / SwiftUI，0.9.1 / 1009001）：与 Harmony 同矩阵——核心环（Home / Battle / Result）、成长环（PackManager、愿望单、图鉴、今日计划、按词包学习报告）、绑定与家长 PIN/后台、Debug 路由；V0.8.4 scheduler core、V0.8.3 polish parity、V0.8.6 怪物等级积分金币、V0.8.7 复习入口语义、V0.8.8 daily check-in 与 V0.9.1 sentence-cloze 已落到代码层，release-pre 仍保留生产 backend 给 Apple review 的 P0 确认项。
  - **Android**（`android/`，Kotlin / Compose，0.9.1 / 1009001）：17 页均已实现；V0.8.4 scheduler core、V0.8.3 polish parity、V0.8.6 怪物等级积分金币、V0.8.7 复习入口语义、V0.8.8 daily check-in 与 V0.9.1 sentence-cloze 已落到代码层；Phase 5 发布硬化门禁见 [`docs/android-replica/07-release-readiness-checklist.md`](android-replica/07-release-readiness-checklist.md)。
  - **对齐边界**：三端共享 `shared/fixtures` 解码契约；V0.9.1 句子填词已进入三端代码对齐；V0.10 战斗 BGM、V0.11 Cocos 未实现；学习报告仍为 **device-local**（未上云按 pack 聚合）。
- **服务端（V0.5.1–V0.8.2+，已交付）**：FastAPI + MongoDB + Vercel。内容管线（词条 CRUD、版本化词包、LLM 草稿、课本图 import、插画/发音 Blob）；账户与绑定（家长 OTP + OAuth、pair、child word-stats / wishlist）；**V0.8.1** 家长词库 HTML 工作台 + family pack JSON API + 子端 **global+family 合并** `packs/latest.json`（`parent_packs_pages.py`、`child_family_pack.py`）；**V0.8.2** 系统管理员 HTML 控制台 `/admin/`（`admin_pages.py`、`admin_audit_service.py`，会话 cookie + 高风险操作审计）；**2026-05-22 follow-up** 已补 `DELETE /api/v1/admin/global-packs/{pack_id}` 与 HTML 删除入口，硬删除官方词包并记录审计；**2026-05-23 follow-up** 已补 family/global 草稿词包拆分，只能从 source draft 选词创建新词包，支持 `copy` / `move`，并覆盖 JSON API、HTML 表单、契约与 e2e 场景；**2026-05-24 follow-up** 已补 `/admin/family-packs/{pack_id}/delete` 与 `/family/{family_id}/packs/{pack_id}/delete`，可硬删除 family pack definition、draft、published versions 与 pointer，管理员路径记录 `family_pack.definition_delete` 审计。遗留 JSON admin API（V0.5.x）与 HTML 控制台并存，新运营操作走 V0.8.2 壳。

因此后续路线不应回到「从零搭原型」或「单端 replica / 后台追平」，而应围绕 **三端 feature SOP 新能力、V0.6 体验增强（可选）、战斗音频（V0.10）、AI 语境（V0.9）** 演进。

### 2.1 路线图与现状差距（2026-05-24）

| 路线图项 | 状态 | 说明 / 下一步 |
| --- | --- | --- |
| V0.3–V0.7.0 | 已完成 | 学习内核、内容后台、V0.6 账户绑定、monorepo 重排 |
...
| **Admin global pack delete** | **已完成** | `DELETE /api/v1/admin/global-packs/{pack_id}` + HTML 删除入口；硬删除官方词包并写审计，覆盖 `test_admin_global_pack_router.py` / `test_admin_pages.py` |
| **Admin / parent family pack delete** | **已完成** | `/admin/family-packs/{pack_id}/delete` 与 `/family/{family_id}/packs/{pack_id}/delete`；硬删除 family pack definition / draft / published versions / pointer，管理员路径写 `family_pack.definition_delete` 审计；覆盖 `test_family_pack_service.py` / `test_admin_pages.py` / `test_parent_packs_pages.py` |
| **Draft pack split** | **已完成** | 家长 family 词包与管理员 global 词包均可从草稿勾选部分单词，以 `copy` / `move` 创建新词包；source 必须是 draft，target 只允许新建；覆盖 service / JSON API / HTML / e2e / OpenAPI contracts |
| **Phrase rendering parity** | **已完成** | FillLetter / Spell 对短语（如 `magic wand`）保留空格、跳过冠词 `a/an/the` 遮挡；Harmony / iOS / Android 已同步 |
| **V0.8.3 战斗与词包体验小优化** | **Done** | [`docs/features/2026-05-18-battle-polish-v0-8-3/`](features/2026-05-18-battle-polish-v0-8-3/) — 10 槽、自动轮换、MonsterLevel、bonus 标记、HP-2、伤害浮字与稳定 ID 已在 Harmony / iOS / Android parity checklist 中全绿或显式 N/A |
| **V0.8.4 战斗平衡与题型节奏** | **Done** | [`docs/features/2026-05-18-battle-balance-v0-8-4/`](features/2026-05-18-battle-balance-v0-8-4/) — 默认 HP 10、Spell 错点扣血、题型 intro→Challenge 调度、V0.8.3 前置 polish parity 已在三端 checklist 中全绿或显式 N/A |
...
| **V0.8.6 怪物等级积分金币** | **Done** | [`docs/features/2026-05-23-coin-reward-by-monster-level-v0-8-6/`](features/2026-05-23-coin-reward-by-monster-level-v0-8-6/) — 战斗结束金币改为 `Σ 数量 × 怪物等级值`（初/中/高/Super = 1/2/3/4），完全替代星星金币与 Bonus `×1.3`；模拟器验收完成，截图 artifact 明确跳过 |
| **V0.8.7 复习入口近期错词策略** | **已完成 / Harmony UI rerun pending** | 三端 HomeReviewButton 语义对齐：`recentWrongIds(12)` 非空时以错词 id 作为临时 focused word plan 开战；空时 toast。iOS targeted XCUITest 与 Android targeted connected UI test 已覆盖空错词 toast + 1 个错词复习首题；HarmonyOS ohosTest 覆盖已补强，等待稳定设备 runner 复跑 |
| V0.6 体验增强（非阻塞） | 可选 | 云端学习报告按 pack、兑换推送通知（未纳入 V0.8 范围） |
| V0.9 系列（6 子版本）/ V0.10 / V0.11 | V0.9.1 Stage 5 screenshots / full UI pending | V0.9.1 三端代码对齐与 focused unit/build/UI 验证已完成，剩余截图刷新与 Harmony full UI suite 复跑。V0.9.2–V0.9.6（Boss 个性→区域剧情→听音+魔法书→个性化今日→LLM 辅助内容）、V0.10 战斗 BGM 混音、V0.11 Cocos（顺序见 §23） |
...
| V0.8.3 | 已完成 | 战斗与词包体验小优化 | 三端已覆盖三个子任务：① 词包激活上限 5→10，且满 10 时自动关闭"队首未 pin"词包并 toast；② 怪物分 4 级（初/中/高/Super = 10/60/20/10），30% 高级/Super 怪带 bonus 标记（V0.8.3 原始结算为 ✨×1.3，已被 V0.8.6 怪物等级积分金币替代），50% 高级/Super 攻击造成 HP-2；③ `DamageFloaterLabel` 在玩家与怪物受击时从头顶升起。三端 checklist 全绿或显式 N/A；详见 [`docs/features/2026-05-18-battle-polish-v0-8-3/50-parity-checklist.md`](features/2026-05-18-battle-polish-v0-8-3/50-parity-checklist.md) | 无 |
| V0.8.4 | 已完成 | 战斗平衡与题型节奏 | 三端已实现：魔法师默认 HP 10；`Spell` 错点字母每次 -1 HP；今日战斗：每词最多 1 次中文选词 + 1 次单字母填空，之后从 Challenge pool 抽题；V0.8.3 前置 parity 已全绿或显式 N/A；详见 [`docs/features/2026-05-18-battle-balance-v0-8-4/50-parity-checklist.md`](features/2026-05-18-battle-balance-v0-8-4/50-parity-checklist.md) | 无 |
...
| V0.9.1 | Stage 5 screenshots / full UI pending | 句子填词题型（AI 语境第一步）  | HarmonyOS / iOS / Android 均已新增 `QuestionKind.SentenceCloze`：例句留空目标词，3 选 1 填回；复用现有 `example.{en,zh}`；默认开启并纳入题型开关与 Challenge pool；内置 5 词包补齐 example；缺例句的词自动回退；默认不自动朗读答案词，只有主动点击喇叭才发音。Harmony targeted BattleFlow ohosTest、iOS focused XCTest/XCUITest、Android unit/build/focused Compose UI 均通过；screenshots / Harmony full UI suite 仍待补。设计：[`docs/features/2026-05-24-sentence-cloze-v0-9-1/00-design.md`](features/2026-05-24-sentence-cloze-v0-9-1/00-design.md) | 复用既有例句字段 |
...
| V0.9.4 | 计划中 | 听音题型 + 魔法书图鉴 | 新 `QuestionKind.ListenAndPick`（点喇叭→3 选 1 释义）；新 `SpellbookPage`（按 pack 分组的已掌握词收集进度） | 复用 `audioUrl` |
| V0.9.5 | 计划中 | 个性化今日推荐 + 错词幽灵 | HomePage 增 `HomeRecommendedAdventureCard`（本地规则）；每周一次"复仇关卡"重问近 7 天错词 | 可选服务端推荐 |
| V0.9.6 | 计划中 | LLM 辅助内容生产闭环 | `/admin/llm/*` 扩生成例句/Boss 台词/章节文案草稿；走现有 `LlmDraft` + V0.8.2 审核；客户端不动 | 必需 |
...
> **状态：已完成。** 详细 cross-platform 设计：[`docs/features/2026-05-18-battle-polish-v0-8-3/00-design.md`](features/2026-05-18-battle-polish-v0-8-3/00-design.md)。当前 gate / gap 以 [`20-replication-trigger.md`](features/2026-05-18-battle-polish-v0-8-3/20-replication-trigger.md) 与 [`50-parity-checklist.md`](features/2026-05-18-battle-polish-v0-8-3/50-parity-checklist.md) 为准；截至 2026-05-24，parity checklist 全绿或显式 N/A。
...
V0.8.3 验收（三端均需；截至 2026-05-24，`50-parity-checklist.md` 已全绿或显式 N/A）：

- 单战可激活 10 个词包；第 11 个 toggle 触发自动轮换；全 pin 时拒绝。
- 怪物分级在 monster codex 详情可见（小角标），但 HomePage / 战斗 chip 不暴露 level 给孩子。
- 高级 / Super 攻击在统计上接近 50% 出现 HP-2；bonus 怪物在 30% 概率出现。V0.8.6 起 bonus 不再加币，金币完全由怪物等级积分决定。
...
> **状态：已完成。** 设计：[`docs/features/2026-05-18-battle-balance-v0-8-4/00-design.md`](features/2026-05-18-battle-balance-v0-8-4/00-design.md)。当前 checklist：[`50-parity-checklist.md`](features/2026-05-18-battle-balance-v0-8-4/50-parity-checklist.md)；截至 2026-05-24，三端 core 与 V0.8.3 前置 polish parity 均全绿或显式 N/A。
...
| **仅 Challenge 类**（`fill-letter-medium` / `spell`，V0.9.1 起含 `sentence-cloze`） | `challenge_only` | **跳过 Intro**，从第 1 题起就是 Challenge |
| **Intro + Challenge 混合** | `two_phase` | Intro pass → Challenge（从已开启 Challenge pool 均匀抽取；V0.8.4 为 medium / Spell 50/50，V0.9.1 起加入 sentence-cloze） |
...
V0.8.4 验收（三端 core 已有；截至 2026-05-24，`50-parity-checklist.md` 已全绿或显式 N/A）：
...
| **V0.9.2** | Boss 个性与登场对话 | 故事性最低成本切入 | `SceneMetadata.bossCandidates` 扩 `{introLine, defeatLine}`；`BattleBossDialogueOverlay` |
| **V0.9.3** | 区域剧情卡 + 章节完成庆祝 | Pack 升级为"章节" | `Pack.scene.{introZh, outroZh}`；HomePage 章节标题过场；全 Boss 3⭐ 通关弹窗 + ✨ |
| **V0.9.4** | 听音题型 + 魔法书图鉴 | 把 `audioUrl` 资产用进玩法 + 收集成就感 | 新 `QuestionKind.ListenAndPick`；新 `SpellbookPage`；HomePage 增 `HomeSpellbookButton` |
| **V0.9.5** | 个性化今日 + 错词幽灵 | 提升次日 / 多日留存 | HomePage 增 `HomeRecommendedAdventureCard`（本地规则）；每周"复仇关卡"重问近 7 天错词 |
| **V0.9.6** | LLM 辅助内容生产闭环 | 让前 5 步内容能扩量 | `/admin/llm/*` 扩例句/Boss 台词/章节文案；走 `LlmDraft` + V0.8.2 审核；客户端不动 |

设计原则：

- AI 内容 **只在服务端生成**，进 `LlmDraft` 草稿 → 家长 / 系统管理员审核 → 发布后才进客户端。
- 优先**复用已有字段**：V0.9.1 用现成 `example`、V0.9.4 用现成 `audioUrl`、V0.9.5 用现成 `WordStat`，新增字段走 schema_version 单调递增 + 老客户端 ignore。
- 不与 V0.10 / V0.11 抢边界：V0.9.x 新题型只用现有 SFX + 视觉/文本；BGM / 混音留 V0.10，战斗特效升级留 V0.11。
...
| 学习系统  | 错词复习、轻量遗忘曲线、按词包本地报告     | 个性化学习计划、长期掌握度模型、云端报告                 |
| 内容系统  | 后台 CMS、家长/管理员词库工作台、三层 Pack  | LLM 题库增强、审核流优化            |
...
> **2026-05-24 刷新。** V0.3–**V0.8.2+**（含三端 bootstrap、家长词库、管理员后台、admin global pack delete、admin / parent family pack delete、draft pack split）已在主干交付；V0.8.3 战斗 polish、V0.8.4 战斗平衡、V0.8.6 怪物等级积分金币与 V0.8.8 每日打卡已完成三端对齐；V0.8.7 复习入口近期错词策略已三端代码对齐，iOS / Android targeted UI 验证已通过。三端 app metadata 已推进到 **0.9.1 / 1009001**（Android `1_009_001`），V0.9.1 句子填词已完成三端 focused unit/build/UI 验证；短期重点转为截图刷新、Harmony full-suite UI gate 复跑与 V0.9.2 设计/实现启动。

短期优先级建议如下：

1. **V0.9.1 Stage 5 cleanup**：刷新 settings / battle 截图，复跑 Harmony full `scripts/run_ui_tests.sh`（当前 runner 上次未产出 OHOS 报告），并在 [`50-parity-checklist.md`](features/2026-05-24-sentence-cloze-v0-9-1/50-parity-checklist.md) 收口最后一行。
2. **V0.9.2 / V0.9.3 故事感增量**：Boss 个性 + 区域剧情卡（低代码量、高情绪 ROI）。
3. **V0.10 战斗音频**：三端战斗层落地 `BattleAudioMixer`（Harmony 先稳，`SpellQuestionFlow` 连跑 5 次全绿后再合入 BGM）。在 V0.9.3 完成后启动以稳为先。
4. **V0.9.4–V0.9.6**：听音 + 魔法书 → 个性化今日 + 错词幽灵 → LLM 辅助内容生产。
5. **V0.11 Cocos 战斗表现层**：玩法与音频稳定后再迁移表现层。
6. **新功能走三端 SOP**：`docs/features/<feature-id>/` — Harmony 设计实现 → `20-replication-trigger.md` 签字 → iOS / Android 复制。
7. **体验增强（可选）**：云端学习报告按 pack、兑换推送通知、运维 runbook 补全——不阻塞版本号推进，可按 feature 单独立项。
...
下一步最适合推进的主题（2026-05-24）：

```text
V0.9.1 Stage 5 cleanup（screenshots / Harmony full ohosTest）
  ↓
V0.9 系列（6 子版本，docs/features/ 三端 SOP）
  → V0.9.1 句子填词收口
  → V0.9.2 Boss 个性
  → V0.9.3 区域剧情卡 + 章节完成庆祝
  → V0.10 战斗音频混音（BattleAudioMixer + BGM）  ← 在 V0.9.3 之后插入
  → V0.9.4 听音题型 + 魔法书图鉴
  → V0.9.5 个性化今日 + 错词幽灵
  → V0.9.6 LLM 辅助内容生产闭环
  → V0.11 Cocos2D 战斗表现层
...
V0.3–V0.8.8 已构成当前产品的 **三端可玩、可绑、可运营词库、可后台排查、可日常留存** 底座；V0.8.3–V0.8.4 已完成 parity closeout；V0.9.1 已完成三端 focused verification，剩余事项集中在截图刷新与 Harmony full UI suite 复跑。后续继续按 feature folder、签字 gate 和 parity checklist 推进 V0.9.x。
