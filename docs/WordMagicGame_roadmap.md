...
> 最近更新：**2026-05-29** — 主干已推进到 **0.9.5 / 1009005**。V0.9.5 **魔法书图鉴**客户端主功能已完成 HarmonyOS-first 交付并复制到 iOS / Android：`HomeSpellbookButton`、`HomePackSpellbookCover`、`SpellbookPage`、locked/seen/mastered 词卡、词详情、按 pack 进度与一次性 +50 魔法币奖励均已落地；三端版本号已同步为 `0.9.5 / 1009005`。iOS 记录 `SpellbookTests` 4/4 与 full unit 179/179 通过，Android 记录 Spellbook / local progress targeted tests 与 full `testDebugUnitTest` 通过；HarmonyOS trigger 记录 unit、HAP build、CodeLinter 通过，并由 human signature 放行复制。服务端 spellbook cover 生成、Blob 上传、admin / parent 触发入口与 OpenAPI 契约也已合入。V0.9.5 当前仍处 **Stage 5 cleanup**：截图刷新、ohosTest 复跑、parity checklist / sign-off 和后台验收记录需要补齐。设计基线见 [`docs/features/2026-05-29-spellbook-v0-9-5/00-design.md`](features/2026-05-29-spellbook-v0-9-5/00-design.md)、[`docs/features/2026-05-26-learning-plan-review-v0-9-3/00-design.md`](features/2026-05-26-learning-plan-review-v0-9-3/00-design.md)、[`docs/features/2026-05-24-sentence-cloze-v0-9-1/00-design.md`](features/2026-05-24-sentence-cloze-v0-9-1/00-design.md)、[`docs/features/2026-05-25-boss-dialogue-v0-9-2/00-design.md`](features/2026-05-25-boss-dialogue-v0-9-2/00-design.md)、[`docs/features/2026-05-18-battle-polish-v0-8-3/00-design.md`](features/2026-05-18-battle-polish-v0-8-3/00-design.md)、[`docs/features/2026-05-18-battle-balance-v0-8-4/00-design.md`](features/2026-05-18-battle-balance-v0-8-4/00-design.md)、[`docs/features/2026-05-23-coin-reward-by-monster-level-v0-8-6/00-design.md`](features/2026-05-23-coin-reward-by-monster-level-v0-8-6/00-design.md) 与 [`docs/features/2026-05-23-daily-checkin-v0-8-8/00-design.md`](features/2026-05-23-daily-checkin-v0-8-8/00-design.md)。
...
路线图的核心原则是：先把学习游戏内核和端云契约做扎实；**V0.7.1 起**，HarmonyOS、iOS、Android 三个原生客户端在 bootstrap 范围内（17 页矩阵、三层词包、绑定、愿望单、本地学习报告等）保持同一产品语义；**V0.7.1 之后的新能力**仍按三端 feature SOP 由 Harmony 先设计/实现，再复制到 iOS / Android。服务端作为内容生产、词库发布、家长管理和 AI 能力的支撑层持续进入。

## 2. 当前实现状态摘要

> **基线日期：2026-05-29。** 下文「已交付」指本仓库主干可构建、可测、可部署的实现；「计划中」指路线图目标尚未合入或仅有方向性文档。
...
- **三端客户端当前版本：0.9.5 / 1009005。** HarmonyOS、iOS、Android 已完成 bootstrap 17 页矩阵、三层词包、家长绑定 / PIN / 后台入口、愿望单、怪物图鉴、今日计划、本地学习报告、V0.9.1 句子填词、V0.9.2 Boss 个性 / 登场对话、V0.9.3 daily learning / review state、V0.9.4 词包故事展示，以及 V0.9.5 魔法书图鉴客户端主功能。iOS `0.9.4 (1009004)` 已于 2026-05-29 上传并提交 App Review；仓库代码已继续推进到 `0.9.5 / 1009005`。
...
- **仍未实现 / 未 closeout 的边界。** 学习报告仍是 **device-local**，未做云端按 pack 聚合；V0.9.5 仍需截图刷新、ohosTest 复跑与 parity sign-off；个性化推荐、战斗 BGM / `BattleAudioMixer` 与 Cocos2D 战斗表现层仍未实现。

因此后续路线不应回到「从零搭原型」或「单端 replica / 后台追平」，而应围绕 **V0.9.5 closeout、V0.9.6 个性化推荐、V0.10 战斗音频、V0.11 战斗表现层** 继续演进。

## 3. 路线图总览

状态图例：**已完成** = 主干已交付；**进行中** = 部分交付或硬化/parity 阶段；**计划中** = 尚未开始产品实现。

| 版本     | 状态 | 主题               | 核心目标                                                      | 服务端依赖    |
| ------ | --- | ---------------- | --------------------------------------------------------- | -------- |
| V0.3   | 已完成 | 趣味学习内核版          | 今日冒险、主题关卡包装、怪物题型差异、补字母题、轻量遗忘曲线、本地魔法愿望单                    | 无        |
| V0.3.5 | 已完成 | 区域与拼写难度补完版       | 新增学校城堡与家庭小屋两个主题区域、补字母题中级难度（双空两步选择）、今日冒险区域切换               | 无        |
| V0.3.6 | 已完成 | 首页入口合并版          | 合并“开始游戏”与“今日冒险”为单一主入口；首页不再显示右侧开始游戏区域；保留复习入口并迁移到顶部工具栏/卡片入口 | 无        |
| V0.3.7 | 已完成 | 怪物图鉴页            | 新增怪物图鉴页，左右箭头翻看 3 张难度形象（Slime / Zombie / Dragon）与童话风简介；首页工具栏提供入口        | 无        |
| V0.3.8 | 已完成 | 怪物图鉴扩展（7 boss 上线 + V0.2 颜色变种 Slime 退役） | 怪物图鉴扩到 10 张（3 archetype + 7 童话风 boss）；3 个 region 各持有 2-3 只 boss 候选，每天按 hash(regionId+date) 确定性轮换战斗中的 boss SVG；同时退役 V0.2 的 10 条 colored slime 变种（`Lava Imp` / `Frost Wisp` 等），catalog 收紧到 10 条，boss 索引由 14-20 重排为 4-10 | 无 |
| V0.3.9 | 已完成 | 魔法愿望单兑换流程重构      | 用 6 位家长 PIN 弹窗替代 3 秒长按门；兑换成功播放 GiftBox 庆祝动画（~1.68s 播 + 1.5s 停），期间遮罩 + HitTestMode.Block 屏蔽所有交互；新增持久化兑换记录页（cap 50，最新在前），从愿望单头部 📜 入口进入；卡片版式改为最左 emoji + 中间名+价 + 右侧申请兑换按钮的三段式；Pending 中间态退役（reader 兼容旧持久化）；ConfigPage 加家长密码入口行 | 无 |
| V0.3.10 | 已完成 | App 图标与按钮图标重构 | 用 Recraft V4 vector 生成 8 个统一「童话魔法羊皮纸」风格 SVG：app 启动图标（前景 magician 1024 + 背景紫粉金渐变魔法夜空 1024 + startIcon 尖帽金星 216）+ HomePage 顶部工具栏 4 个圆形按钮（📚📖🪄⚙ → review/codex/wand/gear）+ WishlistPage 头部 📜 按钮（→ scroll）；新增 `tools/recraft/{generate-icons.sh, svg-to-png.mjs, icons-to-launcher.sh}` 工具链；UI 测试改为 by-id 断言；不动战斗页 / 不做密度限定多 PNG / 不做深色主题变体 | 无 |
| V0.4.1 | 已完成 | 完整拼写题（Boss 限定）      | 新增 `QuestionKind.Spell` + `SpellGenerator`（4-9 字母门）+ `SpellingArea` 点选式字母板组件；首字母预亮，剩余字母按生成器 RNG 打乱成池；错点不扣血、不消耗、仅红色抖动；PlanQuestionSource 拆分 Boss/Elite，Boss 回退链 Spell→Medium→Beginner→Choice；BattleEngine.submitAnswer 接受 Spell 全词为合法选项；不动 LearningRecorder 写入 / 不接系统键盘 / 不做听写模式 | 无 |
| V0.4.2 | 已完成 | 多空补字母升级（已并入 V0.4.1） | 原计划「FillLetterMedium 升级到 3+ 空 + 撤销 + 错误反馈」与 V0.4.1 的完整拼写题在难度曲线上重复；点选式 Spell 已经覆盖「更长字符串的主动回忆」场景，故不再单独立项 | 无 |
| V0.4.3 | 已完成 | 精细记忆状态（已完成）       | `WordStat` 新增连续正/错次数 + `mastery`；`MemoryScheduler` 用连胜阈值促升 Familiar(2)/Mastered(5)，旧 `correctCount` 阈值（4/10）降级为 v2-snapshot 兼容回退；快照升级到 v3，老 blob 用 correct/seen 回填 mastery | 无 |
| V0.4.4 | 已完成 | 每日学习计划（已完成）       | HomePage 工具栏 📋 入口跳 `TodayPlanPage`；`TodayPlanService` 用 (regionId+dayKey) 哈希种子复用 `TodayAdventureBuilder`，按 Review/Learning/New 拆 3 段展示当日单词；行内显示 `MemoryState` 徽章 + ✓ 已完成（基于 V0.4.3 streak 数据派生）；read-only，不接入战斗或编辑 | 无 |
| V0.4.5 | 已完成 | 本地学习报告（已完成）       | `LearningReportBuilder` 计算总正确率 / 4 态计数 / 今日复习完成率 / 薄弱分类（按正确率升序，skip seen=0）；`pages/LearningReportPage` 4 卡片渲染 + 全分类详情；TodayPlanPage 顶栏 📊 入口；不接入趋势图 / 不做云端同步 | 无 |
| V0.4.6 | 已完成 | 更多主题区域（已完成）       | 词库 +20 词覆盖 `animal` / `ocean` 两个新分类；新增 `animal-safari` (Phoenix + Unicorn 共享) / `ocean-realm` (Kraken 独占) 区域；`home-cottage` 的 Kraken 迁出兑现 V0.3.8 承诺；regionPicker 横向 Scroll 容纳 5 chips；boss 共享允许多 region 共用旋转池 | 无 |
| V0.4.7 | 已完成 | 自定义愿望单条目（已完成）   | `MagicWish.isCustom` + 快照 v2；`WishlistStore.addCustomWish/removeCustomWish` 带名称/币数/emoji 校验；`WishlistPage` 头部 `+ 添加` 按钮 + 自定义卡 ✕ 删除按钮，均过家长 PIN 闸；新增 `AddCustomWishDialog` 三段表单；不接真实支付 | 无 |
| V0.4.8 | 已完成 | 视觉与音效抛光（已完成）     | 6 项小修补：HomePage 工具栏 review/codex/wand→gift/gear 与 WishlistPage scroll 五个 SVG 加 `.syncLoad(true)` 修复返回后图标占位符闪烁；HomeWishlistButton 由 wand → 礼盒（Recraft V4 重生）；launcher PNG 改自 `rawfile/icons/start_icon.svg`（`tools/recraft/start-icon-to-launcher.sh`）；战斗中魔法师施法瞬间叠加 `magican_fight.svg` 替身（CharacterCard 走 `altAssetPath` + opacity 切换）；魔法弹圆形 → 椭圆 + 中心单词；答错时叠加 `player_hurt.ogg` 受伤音效（小女生音色，Mixkit License + ffmpeg pitch shift）。**战斗 BGM 因 TTS 强制焦点抢占无法稳定共存（详见 §V0.4.8.7 deferred 段），整套 BGM 改动已回退，明确留到 V0.10 战斗音频混音与 BGM 版** | 无 |
| V0.5.1 | 已完成 | 内容后台 Walking Skeleton（已完成） | 端到端最薄链路：`server/` Python 3.12 + FastAPI + Beanie + JWT + admin bootstrap；部署 Vercel + MongoDB Atlas Marketplace；客户端 `RemoteWordPackConfig` / `WordPackCache` / `RemoteWordPackService` / `WordPackBootstrapper` 三段冷启动；prod URL `https://happyword.cool` 全链路 smoke 通过 | 必需 |
| V0.5.2 | 已完成 | 词条 CRUD + rawfile 迁移 | `POST/PUT/DELETE /admin/words/{id}`；`scripts/seed_from_rawfile.py` 把 V0.4.6 50 词一次性灌入 MongoDB；客户端零改动 | 必需 |
| V0.5.3 | 已完成 | 版本化词包发布 + 回滚 | `WordPack` 快照 + `PackPointer` 指针；`POST /admin/packs/publish` + `rollback`；`/packs/latest.json` ETag / If-None-Match 304；客户端 cache 加 `cachedEtag`/`cachedVersion` | 必需 |
| V0.5.4 | 已完成 | LLM 单词级草稿（干扰项 + 例句） | `POST /admin/llm/drafts/words` + 审核 approve/reject；OpenAI gpt-4o-mini；schema_v1→v2 加 `distractors?` / `example.{en,zh}?`；客户端类型层兼容、战斗端不使用 | 必需 |
| V0.5.5 | 已完成 | 教程照片 → 一键分类导入 | `POST /admin/lessons/import`（multipart → Vercel Blob → gpt-4o vision）抽词 + 自动起 title/storyZh/category；新 `categories` 集合 + 5 条 manual seed；schema_v2→v4 加 `categories[]`；HomePage region 卡片下方渲染 `storyZh` | 必需 |
| V0.5.6 | 已完成 | 词条插画 + 发音 MP3 | `POST /admin/assets/words/{id}/{illustration,audio}` 上传 Blob；schema_v4→v5 加 `illustrationUrl?` / `audioUrl?`；客户端新 `RemoteAssetCache`，`PronunciationService` 命中 mp3 优先；BattlePage / TodayPlanPage 命中 illustrationUrl 渲染 `Image(url)` 替代 emoji | 必需 |
| V0.5.7 | 已完成 | 收尾（监控 + 文档 + rawfile 退化） | `/admin/stats`、OpenAI usage 日志、`backup_pack.py`；客户端 cache 命中即跳过 rawfile（rawfile 仅作首次冷装兜底）；admin 操作手册 + 运维 runbook | 必需 |
| V0.5.8 | 已完成 | 家长管理后台改版（已完成）     | "管理员控制台"重命名为"家长管理后台"；进入即锁定竖屏 + 离开恢复横屏；删除 JWT 登录闸（`Depends(current_admin_user)` 移出全部 admin 路由，V0.6 改为家长账户隔离）；以"📷 拍照 / 🖼️ 从相册"上传课本图替代手填发布流：`LessonImagePicker`（gallery + camera）+ `MultipartBuilder`（RFC-7578 单图 + 头注入防护）+ `ParentApiClient.importLesson` POST `/api/v1/family/{family_id}/lessons/import` → vision → 草稿；新 `LessonDraftReviewPage`：原图缩略 + 主题标签 + 候选词列表（保留 / 编辑 / 弃用）+ 全部确认 / 全部拒绝；保留独立"发布新版本词包"按钮，让家长能批量积累草稿后再一次性发布 | 必需 |
| V0.6   | 已完成 | 家长账户与设备绑定版       | 家长账号、孩子档案、二维码绑定设备、云端学习同步、云端愿望单（含 V0.5.8 留下的 admin 路由家长账户隔离）；V0.6.x ScanBindingPage 增「📷 从图库选择二维码」入口（@kit.ScanKit `detectBarcode.decode` 静态解码 + 共享 `PhotoPickerService`）+ 系统扫码 UI 同步打开 `enableAlbum:true`，让家长把网页 QR 截图发到孩子设备完成绑定的两条入口都可达 | 必需       |
| V0.6.5 | 已完成 | 三层词包模型（服务端 + 客户端 foundation） | 词包重构：built-in（rawfile，5 包不可删）/ global（管理员后台维护，所有 family 可见，`/api/v1/public/global-packs/latest.json` 匿名访问）/ family（家长上传，`family_id`-scoped）三层模型；server-side 复用 `FamilyPackDefinition` + `GLOBAL_PACK_FAMILY_ID` 哨兵，新增 `app/services/global_pack_service.py` 薄包装、`/api/v1/admin/global-packs/**` 管理路由、`/api/v1/public/global-packs/latest.json` 匿名只读路由（含 ETag/HEAD）、`scripts/migrate_global_packs_v0_6_5.py` seed 5 个默认 global pack；publish 阶段过滤 `category=="test"` 词；新增 `.cursor/rules/api-route-pattern.mdc` 沉淀 `/api/v1/{admin,public,family/{family_id}}/**` 路由约束；`legacy_route_aliases.py` 把不合规的旧路径以 `include_in_schema=False` 别名重定向到新路径，向前兼容期保留旧路由；客户端引入 `Pack` + `SceneMetadata` 模型、`BuiltinPackLoader`（5 个 per-pack rawfile + scene 元数据）、`GlobalPackService`（匿名 ETag 客户端 + prefs slot `wordmagic_global_packs`）、`PackLibrary`（builtin/global/family 三层 union + scene fallback）、`PackSelectionService`（设备级最多 5 包 + pin + perfect-rotation，`wordmagic_pack_selection`）。**UI 集成（HomePage chip row、ConfigPage pack picker、BattlePage repo-from-pack）延后到 V0.6.5.1**；当前版本 runtime 行为不变，新服务以 foundation 形式落地，后续 minor patch 再接 UI | 必需 |
| V0.6.5.1 | 已完成 | 三层词包 UI 集成（已完成） | 把 V0.6.5 落地的 PackLibrary / PackSelectionService 接入产品 UI：`WordPackBootstrapper.bootstrapPackLibrary(ctx)` 离线 cold-start composer 合并 builtins + cached globals + cached families 成 `PackLibrary`（family 通过复用 `GlobalPackService.parsePacks` 完成 blob → Pack[] 的字段提升，再改写 `source='family'`）；`TodayAdventureBuilder.buildFromPack()` 以 `pack.scene` 合成 `AdventureRegion`、`pack.words` 作候选池，legacy `build(region, repo, ...)` 仍服务于 review mode；新增 `regionFromPack` + `PackHomeIntegration` 模块（`HomeIntegrationBundle` cold-start loader + `LibraryCandidateProvider` 用于 perfect-rotation 候选）；新增 `PackNetworkFetchAdapter` 在 `@kit.NetworkKit` 上同时实现 `FamilyPackFetchAdapter` + `GlobalPackFetchAdapter` 两个接口（HEAD + 响应头捕获 + Vercel preview bypass header）；HomePage 用 `chipPacks: Pack[]` 替代 `ADVENTURE_REGIONS`，chip id 维持 `RegionChip_<id>` 保持 UI 测试可用，`enterTodayAdventure` 在拿到真实 Pack 时调 `buildFromPack` 并把 Pack 通过 `TODAY_ACTIVE_PACK_KEY` AppStorage 槽传给 BattlePage；BattlePage today-mode 读取 `TODAY_ACTIVE_PACK_KEY` 直接用 `pack.words` 构 `bundleRepo`（让 PlanQuestionSource 即使 rawfile 缺这条 category 也能解析），战斗结束若 `Won && stars>=3` 异步回调 `PackSelectionService.recordPerfectAdventure(packId, LibraryCandidateProvider)` 触发非 pin 词包的自动轮换；ConfigPage 顶部新增 `📦 我的词包` 板块（`ConfigPackPickerStatus` / `ConfigPackSyncButton` / `ConfigPackToggle_<id>` / `ConfigPackPin_<id>` / `ConfigPackSource_<id>` 一组 ID）：每行 [✓] checkbox + 包名 + 来源徽章（内置/官方/家庭）+ 📌 pin 按钮，激活上限 5 个由 `MAX_ACTIVE_PACKS` 闸住，pin 仅对已激活包生效；同步按钮同时拉 `GlobalPackService.ensureLatest()` 与（设备已绑定时）`FamilyPackService.ensureLatest(deviceToken)`，完成后重 load library 并刷新 picker；CustomWordsPage 顶部加软性 deprecation banner（`CustomWordsDeprecationBanner` ID）指引家长改用「家长后台 → 上传课本」，但保留编辑/保存以兼容 `configCustomWordsOnly_oneShotVictory` UI 测试。**GameConfig 字段 cleanup（移除 `enabledCategories` / `customWordsRaw`）延后到 V0.6.6** —— review / normal mode 仍依赖这两个字段，且 `RoutingFlow.ui.test.ets` 多条用例还在驱动 `ConfigCategoryFruit/Place/Home/Custom` + CustomWordsPage 编辑流，本次只做加法不做减法 | 必需 |
| V0.7.0 | 已完成 | Monorepo 原生多客户端重排 | 根目录升级为 monorepo；HarmonyOS DevEco 工程整体迁入 `harmonyos/`；根目录保留 `ios/`、`android/` 原生客户端位、`server/` 后端、`shared/` contracts / fixtures；更新命令、文档、CI 和 DevEco 打开路径 | 无 |
| V0.7.1 | 已完成 | 三端原生客户端 bootstrap 功能对齐 | **HarmonyOS / iOS / Android** 按同一 17 页路由矩阵复刻 V0.6.7.8 产品语义：核心战斗环、三层词包（builtin / global / family）、PackManager 激活与 pin、愿望单与兑换记录、扫码/短码绑定、按词包本地学习报告、家长 PIN/后台、Debug 预览路由；契约对齐 `shared/contracts` + `shared/fixtures`。iOS：Swift / SwiftUI（[`ios/release-pre.md`](../ios/release-pre.md)）。Android：Kotlin / Compose（[`docs/android-replica/00-index.md`](android-replica/00-index.md)）。**不含** V0.9 剧情、V0.10 BGM、V0.11 Cocos | 无 |
...
| V0.8.2+ | 已完成 | 后台词包高风险操作补齐 | Family 与 global 草稿详情页支持勾选部分 draft words，选择 copy / move 后创建新词包；官方 global pack 支持管理员硬删除并记录审计；family pack 支持管理员与 owning parent 硬删除 definition / draft / versions / pointer，管理员路径记录 `family_pack.definition_delete`；相关 API / HTML / OpenAPI / e2e 已覆盖 | 必需 |
| V0.8.3 | 进行中 | 战斗与词包体验小优化 | 三端代码已覆盖三个子任务：① 词包激活上限 5→10，且满 10 时自动关闭"队首未 pin"词包并 toast；② 怪物分 4 级（初/中/高/Super = 10/60/20/10），30% 高级/Super 怪带 bonus 标记（V0.8.3 原始结算为 ✨×1.3，已被 V0.8.6 怪物等级积分金币替代），50% 高级/Super 攻击造成 HP-2；③ `DamageFloaterLabel` 在玩家与怪物受击时从头顶升起。剩余工作是开发机验证；详见 [`docs/features/2026-05-18-battle-polish-v0-8-3/50-parity-checklist.md`](features/2026-05-18-battle-polish-v0-8-3/50-parity-checklist.md) | 无 |
...
| V0.8.6 | 已完成 | 怪物等级积分金币 | 三端已实现：金币 = 本局击败怪物等级积分总和，初/中/高/Super 分别计 1/2/3/4；完全替代星星金币与 Bonus 加成，星级和 Bonus 战斗表现保留。HarmonyOS 用户验收已通过；iOS / Android focused tests、build smoke 与模拟器验收已通过；详见 [`docs/features/2026-05-23-coin-reward-by-monster-level-v0-8-6/50-parity-checklist.md`](features/2026-05-23-coin-reward-by-monster-level-v0-8-6/50-parity-checklist.md) | 无 |
| V0.8.7 | 已完成 | 复习入口近期错词 bugfix | 三端复习按钮策略对齐：近期有回答错误的词时，临时作为 focused word plan 开启 3 怪 / 120s 复习战斗；近期无错词时显示 toast。支持 1-2 个错词的小词池，干扰项仍从完整词库补足，避免退回全量出题。UI 自动化覆盖已补：iOS / Android targeted 场景通过，HarmonyOS ohosTest 场景已补强但本轮 runner 挂起需复跑 | 无 |
| V0.9.1 | 已完成 | 句子填词题型（AI 语境第一步）  | HarmonyOS / iOS / Android 均已新增 `QuestionKind.SentenceCloze`：例句留空目标词，3 选 1 填回；复用现有 `example.{en,zh}`；默认开启并纳入题型开关与 Challenge pool；内置 5 词包补齐 example；缺例句的词自动回退；默认不自动朗读答案词，只有主动点击喇叭才发音。2026-05-25 三端 full UI 自动化通过：HarmonyOS 79/79、iOS 28/28、Android 34/34；截图 / 视觉验证证据已接受为 V0.9.1 closeout。设计：[`docs/features/2026-05-24-sentence-cloze-v0-9-1/00-design.md`](features/2026-05-24-sentence-cloze-v0-9-1/00-design.md) | 复用既有例句字段 |
| V0.9.2 | 已完成 | Boss 个性与登场对话 | 三端已交付：100 个 Boss 双语台词；所有等级统一使用非阻塞 `BattleBossIntro*` `MessageBubble`；SuperBoss 不再使用独立大 banner；同局同 catalog 怪物重复出现时抑制重复 intro；Boss 退场气泡关闭并保留文案给未来 exit-effect；怪物卡显示 `L1` / `L2` / `L3` / `L4`；5 个内置词包扩到 15 词；初装默认 `10 monsters / monster HP 5 / player HP 10`；严格题型阶段调度与选中词包再战修复。设计：[`docs/features/2026-05-25-boss-dialogue-v0-9-2/00-design.md`](features/2026-05-25-boss-dialogue-v0-9-2/00-design.md) | 无 |
| V0.9.3 | 已完成 | 学习计划与复习逻辑重构 | 三端已把 `TodayPlanPage`、Home 复习入口、错词 / 到期词 / 薄弱词调度和 review battle 统一到本地 daily learning state：`A=packBattleWon`、`B=reviewAllDone`、稳定 review snapshot、同日错题不注入当天必复习、review battle 600s + 动态怪物数。验收：HarmonyOS 81/81、iOS UI 30/30、Android 37/37。设计：[`docs/features/2026-05-26-learning-plan-review-v0-9-3/00-design.md`](features/2026-05-26-learning-plan-review-v0-9-3/00-design.md) | 无 |
...
| V0.9.5 | 进行中 | 魔法书图鉴 | 三端客户端主功能已合入：`HomeSpellbookButton`、`HomePackSpellbookCover`、`SpellbookPage`、locked/seen/mastered 词卡、词详情、按 pack 进度、一次性 +50 魔法币奖励与 `scene.spellbookCoverUrl` 解析；服务端 cover 生成 / Blob / admin-parent 入口 / OpenAPI 已合入。剩余：截图刷新、Harmony ohosTest 复跑、parity checklist / sign-off 与后台验收记录清理 | 必需 |
| V0.9.6 | 计划中 | 个性化今日推荐 + 区域章节感 | 在 V0.9.3 统一学习任务模型与 V0.9.4 pack story 基础上，HomePage 增 `HomeRecommendedAdventureCard`；区域剧情卡、章节标题过场、章节完成庆祝按 SOP 单独设计 | 可选服务端推荐 / 后台编辑入口 |
| V0.9.7 | 计划中 | LLM 辅助内容生产闭环 | `/admin/llm/*` 扩生成例句/Boss 台词/章节文案草稿；走现有 `LlmDraft` + V0.8.2 审核；客户端不动 | 必需 |
| V0.10  | 计划中 | 战斗音频混音与 BGM 版 | 新增 `BattleAudioMixer`，让战斗 BGM、combo/攻击/受伤音效、单词朗读按优先级共存；用 duck + 单次恢复策略解决 TTS 抢焦点问题 | 无 |
| V0.11  | 计划中 | Cocos2D 战斗美术化重构版 | 用 Cocos Creator 重写战斗表现层，支持更完整的角色、怪物、动画、特效和多美术主题           | 可选       |
...
- **保留复习入口（迁移）**：
  ...
  - 当前复习入口策略（V0.8.7 修正后）：近期有回答错误的词时，以这些错词作为临时 focused word plan 开启复习战斗；近期无错词时显示 toast，不进入战斗。

### 6.2 验收标准建议

- 首页只存在一个显著主 CTA（今日冒险），点击即可进入战斗。
- 首页不再出现右侧“开始游戏/复习按钮”竖列区域。
- 复习入口可达（在顶部工具栏或冒险卡片内）；有近期错词时进入复习战斗，无近期错词时显示 `先答错几题再来复习吧`。
...
- **新增 7 张怪物美术资源**：
  - `harmonyos/entry/src/main/resources/rawfile/character/{witch,phoenix,unicorn,kraken,pumpkin-king,snow-queen,imp-king}.svg`，全部由 `tools/recraft/generate-bosses.sh` 经 Recraft V4 vector 生成；7 张原始响应 JSON 落盘到 `generated/recraft/` 留作可复现证据。
- **数据层扩展**：
...
- **launcher PNG 光栅化**：用 librsvg `rsvg-convert` 把前 3 个 SVG 转成 PNG 输出到 `harmonyos/entry/src/main/resources/base/media/{foreground,background,startIcon}.png`（现有 layered_image.json 不动）。
- **rawfile SVG 同步**：5 个 in-app SVG 复制到 `harmonyos/entry/src/main/resources/rawfile/icons/`，由 ArkUI 内置 SVG renderer 在 `Image($rawfile('icons/<name>.svg'))` 中直接渲染。
- **生成 / 转换工具链**（新增 3 个脚本）：
...
- 不碰战斗页（BattlePage）任何 SVG / 图像：战斗美术化已顺延到 V0.11 Cocos2D 重构范围。
...
- 编辑 / 重新洗牌今日计划：本版本是只读视图。V0.4.7 后续的「自定义条目」能力先落在愿望单，今日计划页继续只读，等到 V0.5 远程词包接入再考虑「跳过 / 替换今日单词」。
...
- `harmonyos/entry/src/main/resources/rawfile/data/words_v1.json` 新增 2 个分类共 20 个词：
...
### 11.6 V0.4.7 自定义愿望单条目

V0.4.7 让家长可以在本地往魔法愿望单里加入自家的奖励项（例如 "看动画 30 分钟"），不再被三条内置奖励（看 iPad / 手表充值 / 小礼物）锁死。本版本仍坚持 V0.3.9 的家长 PIN 闸，**不接入真实支付**：兑换路径仍是「PIN → CoinAccount.redeem → GiftBox 庆祝」。

**范围**

- 模型：`MagicWish` 新增 `isCustom: boolean`（默认 false）；`WishlistSnapshot.version` 从 1 升 2，老 v1 blob 解析时所有 wish 默认 `isCustom=false`，再保存时升级到 v2。
- 持久化：`WishlistStore` 新增两个方法：
  - `addCustomWish(name, costCoins, iconEmoji, nowMs)`：返回新 id，校验失败返回空串。校验区间：name 1-12 字、cost 5-200（整数）、emoji ≤4 char（空 → ⭐ 兜底）。id 形如 `custom-${nowMs}-${rand6hex}`，与既有 catalog id（`wish-…`）天然不冲突；同一毫秒内重复加也会通过 `findWish` 探测后递增后缀。
  - `removeCustomWish(wishId)`：仅 `isCustom=true` 的 wish 可删；返回 `false` 时 wish 不存在或来自默认 catalog。
- UI：`WishlistPage` 头部增 `WishlistAddCustomButton`（"+ 添加" 文字按钮）；自定义 wish 卡片尾部增 `WishRemove_${id}` 圆形 ✕ 按钮（默认 catalog wish 不显示）。两条路径都用既有 `ParentPinDialog` 闸——通过 `pendingPinIntent: 'redeem' | 'add-custom' | 'remove-custom'` 字段在 onConfirm 回调里分发到 `redeemNow` / `openAddCustomDialog` / `confirmRemoveCustom`，避免开三个独立 PIN 弹窗副本。
- 表单：新增 `components/AddCustomWishDialog.ets` `@CustomDialog`：三段输入（名称 / 数量 / 表情）+ 红色「添加」按钮，自带行内错误提示（"名称最多 12 字" / "数量需在 5 ~ 200 之间" 等）。导出纯函数 `validateCustomWishForm` 与 `buildCustomWishFormResult`，方便单测覆盖校验逻辑。
- 删除确认：用 `promptAction.showDialog`（系统级 AlertDialog，按文本拿按钮 index）二次确认，避免误删。
- 设置未配 PIN：`onTapAddCustom` / `onTapRemoveCustom` 与 `onApply` 共享 `showNoPinAlert`，文案改为「请先在设置页面配置家长密码（6 位数字），再使用此功能。」。

**验收**

- 单元测试 `WishlistStore.test`（V0.4.7 段，11 条新增）：
  - 新增 wish `isCustom=true`、追加在 list 末、id 以 `custom-` 开头、空 emoji 兜底 ⭐、空名称 / 超长名称 / 超低或超高 cost / 非整数 cost（12.7 → 12）路径全覆盖；
  - `removeCustomWish` 删除自定义返回 true，对默认 wish 返回 false 且不修改 list，对未知 id 返回 false；
  - 持久化：新增 wish → flushNow → 新建 store + rehydrate 后 wish 仍在；
  - 兼容：v1 JSON blob 解析后所有 wish `isCustom=false`，state 'pending' 仍归一为 Idle（V0.3.9 老规则不变）。
- UI 测试 `CustomWishlistFlow.ui.test`（2 条）：
  1. `addCustomButtonRendersOnWishlistAndDefaultsHaveNoRemove`：进入 WishlistPage，断言 `WishlistAddCustomButton` 与 "+ 添加" 文案存在；默认 wish `wish-ipad-20min` 没有 `WishRemove_…` 节点。
  2. `addCustomDialogOpensAfterCorrectPin`：先在 ConfigPage 设 PIN 123456，回 HomePage；进 WishlistPage 点 ➕ → ParentPinDialog → 输入 PIN → AddCustomWishDialog 出现（含 6 个新 id：dialog 容器 / 3 个 input / submit / cancel）；点 cancel 关闭；back 回 HomePage。
- 全套 38 / 38 on-device UI 测试通过；no-device 单测全过；CodeLinter 无新增缺陷（仅与 ParentPinDialog 共享的 `@performance/avoid-overusing-custom-component-check` 警告，非新引入）。

**明确不做（留给 V0.5+）**

- TextInput 全程 UI 测试：模拟器软键盘会盖住 Submit + 与 `@CustomDialog autoCancel: true` 的 pressBack 路径冲突；表单校验和提交路径已被纯函数 + 持久化单测覆盖，UI 层只验证「PIN → 弹窗能正确开/关」即停。
- 编辑既有自定义 wish：体验上「删除 + 重加」覆盖一致需求，UI 表面更窄。
- 自定义 wish 排序 / 拖拽：默认按插入顺序排在 catalog 三条之后；后续 V0.5 家长视角可选作。
- 真实支付 / 家长支付宝代付 / 通知：所有 V0.4 支付保留 V0.3.9 的「coin → 家长承诺线下兑现」契约。
- 上传图片自定义图标：仅支持 1-2 字 emoji，避免引入文件存储。

### 11.7 V0.4.8 视觉与音效抛光

V0.4 作为离线学习版的最后一棒，不做新功能、只清理 V0.4 全程积累的视觉与体验细节。范围由六项独立修补组成，互相不耦合：

**1. 工具栏 SVG `.syncLoad(true)`（review-icon bugfix）**

- HomePage 工具栏的五个 V0.3.10 SVG 图标（review / codex / wand→gift / gear）和 WishlistPage 的 scroll 图标全部加 `.syncLoad(true)`。原来的 `Image($rawfile('icons/review.svg'))` 是异步解码，从 BattlePage / WishlistPage / ConfigPage 返回 HomePage 时，58 KB review.svg 第二次挂载会出现 1-2 帧的占位符空白，被孩子家长描述成「图标变回 emoji」。CharacterCard 早就走 syncLoad；这次把同样的姿势复用到工具栏。
- HomePlanButton 的 📋 emoji 故意保留：单独一个生成 SVG 不属于 V0.4.8 的范围，留给 V0.5 内容包统一回流。

**2. HomeWishlistButton 礼盒图标（Recraft V4 重生）**

- 用 `tools/recraft/generate-v4-vector.mjs --prompt-file tools/recraft/prompts/wishlist-gift.txt` 生成 67 KB 的 `rawfile/icons/wishlist.svg`（红丝带 + 米色礼盒 + 星尘点缀），替换原 `wand.svg`（仅入口图标变更，不动愿望单内容、PIN、storage）。
- `wand.svg` 留在仓库以便随时回滚；后续若决定全面收紧再单独剔除。
- 入口 ID `HomeWishlistButton` 不变，UI 测试无需调整。

**3. Launcher PNG ← `rawfile/icons/start_icon.svg`**

- 新增 `tools/recraft/start-icon-to-launcher.sh`：直接把仓库内的 `start_icon.svg` 用 `rsvg-convert` 渲染成 1024×1024 透明 foreground / 1024×1024 #1D3557 实心 background（ImageMagick）/ 216×216 startIcon。
- 不修改 `layered_image.json` / `module.json5` / `app.json5`：清单仍然引用 `$media:foreground / $media:background / $media:startIcon`，只是 PNG 字节被重写。
- 编辑 `start_icon.svg` 后再跑一次脚本即刷新。

**4. 战斗中魔法师施法替身（`magican_fight.svg`）**

- 用户原始诉求是 `attack 时 assetPath 切到 magican_fight.svg`。最初实现走 `currentPlayerAsset()` 在 BattlePage 内根据 `playerFighting` 返回不同的 `assetPath`，CharacterCard `Image($rawfile(this.assetPath)).syncLoad(true)` 跟着重新解码。
- 实测上：5 道题往 Boss 推进的过程会触发 5 轮 `playerFighting=true → false` 反复同步解码 115 KB 的 `magican_fight.svg`，模拟器 UI 线程被堵到 `SpellingArea_Pool_x` 点击丢失（150 ms 内 `Slot_x` 仍是 `_`，`fillsSpellSlotsByTappingCorrectPoolLetters` 三次连测稳定失败）。
- 最终采用 **opacity 双图叠层** 方案：CharacterCard 新增 `altAssetPath: string` + `altActive: boolean` 两个 @Prop，构造期同时挂载两张 Image（各解码一次），`altActive` 以 opacity 0/1 切换，避免 `assetPath` 动态切换走 syncLoad。
- 切换时机由 `triggerForwardCast()` 集中管理：每次正答 / 连击触发 +1 `projectileForwardPulse` 时同时把 `playerFighting` 置为 true 并起 560 ms 定时器（覆盖 SPAWN+TRAVEL+IMPACT 580 ms ≈ 实际飞行窗口），定时器到点把 `playerFighting` 切回 false，撞回 idle 姿势。`aboutToDisappear` 清掉定时器以防回退到 HomePage 后状态污染。

**5. 魔法弹椭圆 + 中心单词**

- `MagicProjectile` 新增 `@Prop label: string`，将原来 `Circle` 内核换成 `Stack { Ellipse(60×34 / 84×48 combo) + Text(label) }`：椭圆走原 `coreColor` / `ringColor`，Text 用 `fontWeight Bold + textShadow {radius:2 color:#000}` 在白色文字外加一圈轮廓，确保白底前景与浅粉色后景都能读出来。
- 外圈光晕保持 `Circle` 不动（无方向性的圆形更适合做 halo）。
- BattlePage 的两个 MagicProjectile 实例都把 `lastAnswerWord` 喂给 `label`：正答时是孩子刚拼对的单词，错答时是孩子被攻击的同一个单词，强化学习信号。
- 所有节点继续 `hitTestBehavior(HitTestMode.None)`，不影响 `BattleOptionA/B/C` / `SpellingArea_Pool_x` 的点击；`MagicProjectileForward / MagicProjectileBackward` ID 仍挂在外层 Stack，UI 测试无回归。

**6. 答错音效 `player_hurt.ogg`（小女生受伤声 — Mixkit Female exclamation of pain，pitch +18%）**

- 用户在 V0.4.8 二次反馈：「player_hurt SFX 的声音要用小女生的声音」。原选 `Mixkit Man-in-pain` 给小女孩魔法师听上去违和，换成 Mixkit Female exclamation of pain（`https://assets.mixkit.co/active_storage/sfx/2206/2206.wav`，Mixkit License — 商用免费、无需署名）。
- ffmpeg 链：`asetrate=44100*1.18,aresample=44100` 把音高拉高约 +2.8 半音、时长压到 0.85 s 左右；再 `-ac 1 -ar 22050 -b:a 64k -c:a libvorbis` 落到 22050 Hz 单声道 OGG（~12 KB）。成年女声 → 童声/萌娘音的常用 trick，避免再去 freesound 找一段已经被 anime tag 污染的样本。
...
**7. 战斗背景音乐（暂时下线，回退到 SFX-only；推迟到 V0.10 战斗音频混音与 BGM 版）**
...
- 留下的开放问题（推迟到 **V0.10 战斗音频混音与 BGM 版** 专门处理，设计基线见 [`2026-04-30-battle-audio-mixer-design.md`](superpowers/specs/2026-04-30-battle-audio-mixer-design.md)）：
...
- 仓库层副作用：删除了 `harmonyos/entry/src/main/resources/rawfile/sound/bgm_fight.ogg`；用户提供的 `bgm_battle.mp3` 源 MP3 之前已转完即删；`hit_crit.ogg`（用户提供 `hit_crit.mp3` 重新转码 ~8 KB）这条二轮反馈的 SFX 资源更新**保留**，因为它独立于 BGM 工作链。
- 明确不做：本次不改 PronunciationService、不动 audio focus / interrupt 配置、不在 BattlePage 加任何 BGM 相关字段，让 V0.4.8 收口干净。

**验收**

- 每个修补点单独 commit，便于事后回滚；最后追加一次「opacity 切换替代 assetPath swap」的修复 commit。
- 全量 UI 测试 38 / 38 通过；单测随键集变化升级；CodeLinter 无新增缺陷（仅遗留两条 V0.3.9 / V0.4.7 的 `@CustomDialog avoid-overusing-custom-component-check`）。
- launcher 三 PNG 经 hvigorw 打包验证，HAP 安装到模拟器后启动正常。

**明确不做（留给 V0.5+）**

- HomePlanButton 📋 emoji 替换：超出本次「修复 V0.4 已知缺陷」范围。
- 区分题型对应的施法姿势（Choice / Spell / FillLetter 都共用 `magican_fight.svg`）：单一 fight 姿势已能传达「释放魔法」语义，多姿势成本不对等。
- 魔法弹文本本地化（中文翻译同时显示）：英文单词中心展示已能在击中前清晰传达学习对象，避免双语挤压可读性。
- 受伤音效本地化变体（按区域 / 角色不同）：当前一个统一短促 grunt 足够；后续 V0.9 AI 剧情版可考虑按怪物配本。
- `wand.svg` 文件清理：保留 SVG 资源以便快速回滚，仅替换引用。
...
| V0.5.2 | 词条 CRUD + rawfile 迁移 | 已完成 |
| V0.5.3 | 版本化词包发布 + 回滚 | 已完成 |
| V0.5.4 | LLM 单词级草稿（干扰项 + 例句） | 已完成 |
| V0.5.5 | 教程照片 → 一键分类导入 | 已完成 |
| V0.5.6 | 词条插画 + 发音 MP3 | 已完成 |
| V0.5.7 | 收尾（监控 + 文档 + rawfile 退化） | 已完成（`admin/stats`、备份脚本已落地；运维 runbook 是否齐全单独核对） |
...
- **服务端骨架（`server/`）**：uv 项目，FastAPI app；endpoints `/api/v1/public/health`、`POST /api/v1/admin/auth/login`、`GET /api/v1/admin/auth/me`、`GET /api/v1/public/packs/latest.json`；Beanie 在 lifespan 里 `init_beanie` 注册 `User` + `Word` 两个 document model；admin 用户在 startup hook 里幂等 bootstrap（`ADMIN_BOOTSTRAP_USER` / `ADMIN_BOOTSTRAP_PASS` 仅首次有效，后续无视——避免重启把 admin 密码改回环境变量）。
...
- 数据迁移：`server/scripts/seed_from_rawfile.py` 一次性把 `harmonyos/entry/src/main/resources/rawfile/data/words_v1.json` 50 词导入 MongoDB（idempotent upsert by id；已存在的词不覆盖，避免线上 admin 已修改的词被脚本重置）。
...
> **状态（2026-05-17）：已完成。** 下列能力在 `server/` 与三端客户端已落地。V0.8.2 已提供独立 `/admin/` 运营控制台（会话 + 审计）。**未纳入 V0.6 主范围的后续增强**：家长 Web 端按 pack 聚合的云端学习报告、兑换推送通知。
...
- 设备二维码绑定，家长端展示二维码，客户端用 `@kit.ScanKit` 扫码绑定；客户端同时支持「打开扫码器」（V0.6.x 起 `enableAlbum:true`，系统扫码 UI 内置相册按钮可选图）、「📷 从图库选择二维码」（V0.6.x 增量，dedicated button 走 `detectBarcode.decode` 静态解码）和「无法扫码？手动输入短码」三条路径，全部走同一条 redeem 状态机。
...
## 14. V0.7.0 Monorepo 原生多客户端重排版

> **状态（2026-05-17）：已完成（结构）。** 目录结构、`harmonyos/AppScope/app.json5` 版本号 0.7.0、文档与脚本路径已按 [`2026-05-06-v0.7.0-monorepo-native-clients-restructure-design.md`](superpowers/specs/2026-05-06-v0.7.0-monorepo-native-clients-restructure-design.md) 就位。本版本 **仅** 做仓库重排；三端产品功能对齐在 **V0.7.1** 交付（见 §2、§3 表）。

V0.7.0 的目标是把当前“根目录即 HarmonyOS 工程”的仓库升级为“根目录 monorepo + 根目录三端原生客户端 + 根目录 server”的结构，为 iOS / Android 原生端铺路，同时让 DevEco Studio 可以平滑迁移到新的 `harmonyos/` 工程根。

设计基线：

- [`2026-05-06-v0.7.0-monorepo-native-clients-restructure-design.md`](superpowers/specs/2026-05-06-v0.7.0-monorepo-native-clients-restructure-design.md)

目标结构：

```text
happyword/
  harmonyos/   # HarmonyOS NEXT / ArkTS / ArkUI / DevEco Studio project root
...
- **shared 不放运行时代码**：`shared/` 只沉淀 OpenAPI、JSON Schema、错误码、同步协议、golden fixtures，三端各自实现 native 业务代码并用 fixtures 对齐行为。
- **server 保持根目录一级模块**：`server/` 已经具备 FastAPI、uv、pytest、Vercel 配置，不需要移动。
...
- 不实现 iOS / Android 产品功能（由 **V0.7.1** 承接，见下节）。
- 不改变服务端 API 行为。
- 不重构 HarmonyOS 页面、服务或游戏逻辑。
- 不把 `shared/` 做成跨平台 SDK。
- 不删除任何设计源文件、图片、音频或字体资源；未使用资源继续按 asset retention policy 沉淀到 `assets/`。

## 14.1 V0.7.1 三端原生客户端 bootstrap 功能对齐

> **状态（2026-05-17）：已完成。** HarmonyOS、iOS、Android 在 bootstrap 范围内已对齐同一产品语义；后续增量功能不再用 V0.7.x 小版本堆叠，改走 `docs/features/` 三端 SOP。

### 范围（三端一致）

对照 `harmonyos/entry/src/main/resources/base/profile/main_pages.json` 的 **17 个页面**：

| 能力域 | 页面 / 能力 | 三端状态 |
| --- | --- | --- |
| 核心学习环 | HomePage、BattlePage、ResultPage | 已对齐 |
| 设置与家长门 | ConfigPage、ParentPinSetupPage | 已对齐 |
| 成长与词包 | PackManagerPage、WishlistPage、RedemptionHistoryPage、MonsterCodexPage、TodayPlanPage、LearningReportPage（按词包） | 已对齐 |
| 家长侧 | ParentAdminPage、LessonDraftReviewPage | 已对齐 |
| 云绑定 | ScanBindingPage、BoundDeviceInfoPage | 已对齐 |
| Debug | DevMenuPage、BypassSecretPage（仅 Debug/Release 策略 gating） | 已对齐 |

### 参考文档

- HarmonyOS 基线：`docs/WordMagicGame_overall_spec.md` §2（V0.6.7.8）
- iOS：`docs/ios-replica/00-index.md`，发布门禁 [`ios/release-pre.md`](../ios/release-pre.md)
- Android：`docs/android-replica/00-index.md`，Phase 5 清单 [`docs/android-replica/07-release-readiness-checklist.md`](android-replica/07-release-readiness-checklist.md)

### V0.7.1 明确不做

- 不保证三端像素级 UI 一致（允许各平台原生控件与布局差异）。
- 不包含 V0.9 句子填词 / AI 剧情、V0.10 战斗 BGM 混音、V0.11 Cocos 战斗层。
- 不把学习报告同步到云端或家长 Web（仍为可选体验增强，非 V0.8 范围）。

## 15. V0.8 后台系统化与词库管理版

> **状态（2026-05-23）：已完成（V0.8.1 + V0.8.2 + follow-ups）。** 家长词库工作台与子端合并拉取、系统管理员 HTML 控制台与审计日志均已落地；后续已补 admin global pack hard delete 与 family/global 草稿词包拆分新建包。实现基线见设计文档 §Implementation baseline；服务端入口：`parent_packs_pages.py`、`child_family_pack.py`（`packs/latest.json`）、`admin_pages.py`、`admin_audit_service.py`。

V0.8 的目标是把 V0.6 已经建立的家长账户、设备绑定和 family pack 能力整理成可用的后台体系。它分成两个连续子版本：先让家长能管理自己的 family 词库，再让系统管理员拥有独立的运营排查后台。

设计基线：

- [`2026-05-08-v0.8-backoffice-and-vocabulary-management-design.md`](superpowers/specs/2026-05-08-v0.8-backoffice-and-vocabulary-management-design.md)
- [`2026-05-08-v0.8.1-parent-vocabulary-management.md`](superpowers/plans/2026-05-08-v0.8.1-parent-vocabulary-management.md)

### 15.1 V0.8.1 家长词库管理闭环

> **状态：已完成。**

V0.8.1 聚焦家长自己的词库工作台，路径仍然放在现有 `/family/{family_id}/` 家长 Web 后台里。每个家长/家庭都作为一个租户，服务端以 `family_id` 做真实隔离边界，客户端可传家长/家庭编号作为显式上下文，但授权必须从 `device_token -> DeviceBinding -> family_id` 反查。

建议能力：

- `/family/{family_id}/packs` 词库列表：展示 active / archived、草稿词数、当前发布版本、是否有未发布修改。
- `/family/{family_id}/packs/{pack_id}` 词库详情：展示元信息、草稿表格、发布状态和回滚入口。
- 图片导入到草稿：上传课本页/练习册图片，AI 提取词表后只写入 `FamilyPackDraft`，不允许跳过人工复核直接发布。
- 表格编辑：支持新增、删除、批量粘贴、修改英文/中文释义/分类/难度/可选例句与资源 URL。
- 发布与回滚：草稿校验通过后发布为 `FamilyWordPack` 快照，通过 `FamilyPackPointer` 指向当前版本。
- 孩子设备拉取：新增 `/api/v1/family/{family_id}/packs/latest.json`，返回 global 词库 + 当前 family 词库的合并结果。
- 2026-05-23 follow-up：词包详情页支持从 source draft 勾选部分单词，选择 `copy` / `move` 后创建一个新的 family pack；`move` 会从原草稿移除被选词，`copy` 保持原草稿不变，不支持拆到已有词包。

关键数据规则：

- 词库分为 **global 词库** 与 **family 词库**。
- global 词库由系统维护，对所有家庭共享。
- family 词库由家长维护，只属于一个 `family_id`。
- 合并时 global 是基础层，family 是覆盖层；family 自定义词必须使用 `fam-<family_id短码>-...` 前缀。
- 如果 family entry 标记某个 global word 为 hidden，该家庭的孩子设备拉取时应隐藏该词。
- `ETag` 必须同时包含 global pack 版本和 family pack 版本，且响应使用 private cache。
...
V0.8.2 新增独立 `/admin/` Web 后台，面向系统管理员/运营者，不混入 `/family/{family_id}/` 家长后台。

建议能力：

- `/admin/` 系统总览：注册家长、家庭、绑定设备、global 词数、family 词库、发布活动、导入失败和高风险操作摘要。
- `/admin/parents` 与 `/admin/parents/{parent_id}`：按 email、家长编号、username、`family_id` 搜索并查看家庭、孩子档案、绑定设备、family 词库和发布历史。
- `/admin/devices`：查看设备 ID、binding ID、`family_id`、绑定历史、最后同步和 revoked 状态；异常时允许解绑/撤销设备。
- `/admin/global-packs`：查看 global 词库、发布和回滚官方词包。
- `/admin/family-packs`：只读排查 family 词库，允许恢复误归档或回滚上一版，但不允许系统管理员静默编辑家长草稿词条。
- `/admin/audit-logs`：记录禁用/恢复家长、解绑设备、发布/回滚 global pack、恢复/回滚 family pack 等高风险操作。
- 2026-05-23 follow-up：global pack 详情页支持从官方草稿勾选部分单词，以 `copy` / `move` 创建新的 global pack，并记录 `global_pack.draft_split` 审计；拆分仍只作用于草稿，不直接发布。

V0.8.2 的核心原则是“看全局、有限干预、全审计”。所有高风险操作必须二次确认并填写原因。

V0.8.2 明确不做：

- 不做管理员 impersonation 家长。
- 不做完整运营 CMS 活动系统。
- 不允许绕过租户边界直接修改 family draft。

### 15.3 V0.8.3 战斗与词包体验小优化

> **状态：三端代码补齐；iOS verification pending。** 详细 cross-platform 设计：[`docs/features/2026-05-18-battle-polish-v0-8-3/00-design.md`](features/2026-05-18-battle-polish-v0-8-3/00-design.md)。当前 gate / gap 以 [`20-replication-trigger.md`](features/2026-05-18-battle-polish-v0-8-3/20-replication-trigger.md) 与 [`50-parity-checklist.md`](features/2026-05-18-battle-polish-v0-8-3/50-parity-checklist.md) 为准。

V0.8.3 是插在 V0.8 后台线和 V0.9 AI/语境线之间的**纯 polish 版本**，主题完全独立、不依赖 LLM。目标：把孩子已经能感知的两个粗糙点（词包激活上限太死板、怪物缺辨识度与重量感）打磨掉，作为 V0.9 系列动手前的基础体验升级。

V0.8.3 包含三个**可独立 PR 提交但合并到同一 0.8.3 发版**的子任务：

**1. 词包激活配置增强**（合并自原 V0.8.3.1 + V0.8.3.2）

- `PackSelectionService.MAX_ACTIVE_PACKS` 由 `5` 改为 `10`。ConfigPage / PackManagerPage 文案 `X/5` → `X/10`。
- 用户在 PackManagerPage 点 Toggle 打开第 11 个词包时：**先开**，然后自动找出激活池里"未 📌 固定 + 激活顺序最早"的那个并关闭它，弹 toast `已关闭 '<oldName>' 以激活 '<newName>'`。
- 如果 10 个全部 📌 固定：拒绝并提示"请先取消固定一个词包"。**不**自动关闭已 pin 的。
- HomePage chip row 已是 Scroll 容器，10 chip 横排兼容。
- 三端 parity：iOS / Android 同步把激活上限的硬编码 5 改成 10，复刻自动关闭逻辑。

**2. 怪物分级 + 题型路由 + bonus + HP-2 重击**

- 新维度 `MonsterLevel`：`beginner | intermediate | advanced | super`，目标比例 **10 / 60 / 20 / 10**。现有 10 个怪物按 **1 / 6 / 2 / 1** 先打标签（具体映射见 [`00-design.md`](features/2026-05-18-battle-polish-v0-8-3/00-design.md) §6.2），后续在 V0.9 期间分批扩到 100。
- **题型 ↔ 等级绑定**：初级 = Choice / 中级 = FillLetter beginner (1 空) / 高级 = FillLetter medium (2–3 空) / SuperBoss = Spell。`QuestionGenerator` 按战场每位置预选定的等级抽题型。V0.9.1 SentenceCloze 上线后会在 V0.9.1 plan 里追加进高级 / Super 池。
- **30% bonus 怪物**（仅高级 / Super）：生成时按 30% 概率打 `bonus = true`，怪物头顶贴 ✨ 金星（id `MonsterBonusStar`）。V0.8.3 最初让本场击杀 bonus 怪物数 K 触发 ✨ 魔法币奖励 × 1.3；**V0.8.6 起金币规则已完全替换为怪物等级积分，bonus 只保留战斗表现/计数，不再加金币，`BattleResultBonusCoinRow` 退役**。
- **50% HP-2 重击**（仅高级 / Super 攻击玩家）：`BattleEngine.applyMonsterAttack(monster)` 当怪物等级是高级 / Super 时掷 50%，命中则玩家 HP 扣 2。

**3. HP -1 / -2 浮字反馈**

- 新组件 `components/DamageFloaterLabel.ets`：从角色头顶 -10vp 升起，450ms ease-out（opacity 0→1→0、translateY -28vp）。
- `-1`：`#F87171`（亮红）+ 1px 白描边，字号 18vp。
- `-2`：`#7F1D1D`（深暗红）+ 2px 阴影，字号 20vp，**更"沉"**。
- 触发：
  - 玩家答对 / 答错的命中点都触发受击方浮字（怪物 -1，暴击 -2；玩家 -1，高级/Super 50% 命中 -2）。
  - 与 V0.4.x 现有的"暴击双倍伤害"逻辑兼容（暴击时怪物 HP -2 浮字）。

V0.8.3 验收（三端均需；截至 2026-05-21，Harmony 已满足，iOS / Android 仍需补齐或由产品 owner 显式延期）：

- 单战可激活 10 个词包；第 11 个 toggle 触发自动轮换；全 pin 时拒绝。
- 怪物分级在 monster codex 详情可见（小角标），但 HomePage / 战斗 chip 不暴露 level 给孩子。
- 高级 / Super 攻击在统计上接近 50% 出现 HP-2；bonus 怪物在 30% 概率出现且击杀后结算页加币。
- 浮字动画无明显丢帧，连答 N 次不留旧 label。
- HarmonyOS ohosTest 必须存在新增烟测：`PackManagerFlow.ui.test`（10+1 自动轮换、10-pin 拒绝）+ `BattleFlow.ui.test`（damage floater 出现+消失）。

V0.8.3 明确不做：

- 不动 V0.10 战斗 BGM / 混音边界（保持现有 `AudioService` / `PronunciationService`）。
- 不引入 LLM 或服务端新数据（怪物 level 与 bonus 是设备端常量元数据）。
- 不改 ⭐ cap（保持 0–3）。
...
> **状态：三端 core 已实现；release gate cleanup open（依赖 V0.8.3 parity 决议）。** 设计：[`docs/features/2026-05-18-battle-balance-v0-8-4/00-design.md`](features/2026-05-18-battle-balance-v0-8-4/00-design.md)。当前 checklist：[`50-parity-checklist.md`](features/2026-05-18-battle-balance-v0-8-4/50-parity-checklist.md)。
...
V0.8.4 验收（三端 core 已有；release gate 仍需处理 V0.8.3 前置 parity 和未重跑的完整套件）：
...
## 16. V0.9 语境学习、学习系统与故事感增强版（滚动拆分）

V0.9 的目标是让学习从单词识别进入语境理解，并把“今天该学什么 / 什么时候复习 / 为什么推荐这一局”讲清楚，同时逐步增强词包本身的故事感。V0.9 已按实际交付滚动拆分为 **7 个可独立发布的子版本**：题型 → 角色反馈 → 学习计划 → 词包故事 → 魔法书收集 → 推荐与章节感 → 内容工厂。AI 始终是"后台辅助内容生产 + 人工审核"，**儿童端不直接调 LLM**。

每个子版本都走三端 SOP：`docs/features/<feature-id>/` Harmony 先实现 → `20-replication-trigger.md` 签字 → iOS / Android 并行复制。

| 子版本 | 主题 | 趣味 Δ | 关键改动 |
| --- | --- | --- | --- |
| **V0.9.1** | 句子填词题型 | 把"识别"升级为"语境理解" | 新 `QuestionKind.SentenceCloze`；复用 `example.{en,zh}`；默认开启并进入 Challenge 池；默认不自动朗读答案词，仅喇叭手动发音；5 个内置词包补 example；设计见 [`2026-05-24-sentence-cloze-v0-9-1/00-design.md`](features/2026-05-24-sentence-cloze-v0-9-1/00-design.md) |
| **V0.9.2** | Boss 个性与登场对话 | 故事性最低成本切入 | 已完成：100 Boss 双语台词、统一 `MessageBubble` 入场气泡、怪物等级标签、15-word built-ins、10/5/10 初装默认、严格题型阶段调度；Boss 退场效果延后单独设计 |
| **V0.9.3** | 学习计划与复习逻辑重构 | 把学习系统从“散落入口”升级为“可执行计划” | 已完成：本地 daily learning state、稳定 review snapshot、A/B 完成语义、Home 复习计数、TodayPlan A/B aware 进度；设计见 [`2026-05-26-learning-plan-review-v0-9-3/00-design.md`](features/2026-05-26-learning-plan-review-v0-9-3/00-design.md) |
| **V0.9.4** | 词包故事与提审版本硬化 | 让 pack 不只是词表，而是有一句冒险引子 | 已完成：三端 `storyEn/storyZh` 解析、首页故事行、内置 5 词包故事、family/global pack story 保存 / 生成 / OpenAPI 同步；三端版本号 `0.9.4 / 1009004`；iOS `0.9.4 (1009004)` 已提交 App Review |
| **V0.9.5** | 魔法书图鉴 | 把掌握词沉淀成可收集、可回看的成长记录 | 三端客户端主功能已合入：新 `SpellbookPage`；按 pack 分组展示掌握词卡、未解锁词卡和收集进度；HomePage 增 `HomeSpellbookButton` 与 `HomePackSpellbookCover`；一次性 +50 魔法币奖励；服务端 spellbook cover 生成链路已合入；当前处 Stage 5 cleanup |
| **V0.9.6** | 个性化今日推荐 + 区域章节感 | 提升次日 / 多日留存 | HomePage 增 `HomeRecommendedAdventureCard`（基于 V0.9.3 计划模型与 V0.9.4 pack story）；区域剧情卡、章节标题过场、章节完成庆祝顺延到此阶段 |
| **V0.9.7** | LLM 辅助内容生产闭环 | 让前 6 步内容能扩量 | `/admin/llm/*` 扩例句/Boss 台词/章节文案；走 `LlmDraft` + V0.8.2 审核；客户端不动 |

设计原则：

- AI 内容 **只在服务端生成**，进 `LlmDraft` 草稿 → 家长 / 系统管理员审核 → 发布后才进客户端。
- 优先**复用已有字段**：V0.9.1 用现成 `example`，V0.9.4 复用 `Pack.scene.storyEn/storyZh`，V0.9.5 复用现有 `WordStat.memoryState`、pack library 与学习报告聚合，不新增题型或服务端契约，V0.9.6 优先复用 V0.9.3 计划模型、V0.9.4 pack story 与现有 `WordStat`。
- 学习系统优先**本地可解释**：V0.9.3 不依赖新服务端；先让三端在 device-local `WordStat`、pack-local 学习报告和现有战斗记录上给出一致计划。绑定设备的云端同步保持现状，不在本版本强制引入新的云端计划权威源。
- 不与 V0.10 / V0.11 抢边界：V0.9.5 不新增战斗题型或音频交互；BGM / 混音留 V0.10，战斗特效升级留 V0.11。

### 16.1 V0.9.3 学习计划与复习逻辑重构

V0.9.3 已把分散的学习线索收拢成一个孩子能执行、家长能理解、三端能一致实现的本地 daily learning 模型。实现没有引入云端权威计划源，而是用稳定的本地 day key、review snapshot 与 A/B 完成语义把 Home、TodayPlanPage、review battle 和 daily check-in 连起来。

已交付范围：

- **Daily state**：三端使用本地 `YYYYMMDD` day key；`packBattleWon` 作为 A，`reviewAllDone` 作为 B；`A || B` 触发 daily check-in 完成，`A && B` 才是完整今日冒险完成。
- **稳定复习列表**：每天生成一次 `DailyReviewSnapshot`，来源为到期复习、近期错词和薄弱词，最多 50 个；同一本地日内产生的错题不会注入当天必复习列表。
- **Home 入口对齐**：Home 只保留选 pack/scene 开战与复习按钮两条主战斗入口；A 完成但 B 未完成时显示剩余复习数。
- **TodayPlanPage 解释**：页面显示 A/B aware 进度与当天 required review section，已复习词行内标记。
- **Review battle**：复习战斗按 review word count 与当前配置动态计算怪物数，时间统一为 600 秒；单词出现并提交一次答案即计入当天已复习。
- **三端验证**：HarmonyOS `scripts/run_ui_tests.sh --rebuild` 81/81；iOS UI 30/30；Android connected UI 37/37。详见 [`docs/features/2026-05-26-learning-plan-review-v0-9-3/50-parity-checklist.md`](features/2026-05-26-learning-plan-review-v0-9-3/50-parity-checklist.md)。

V0.9.3 明确不做：

- 不引入云端学习计划权威源；仍以本地学习记录为主，后续云端报告另立项。
- 不做完整 SM-2 / FSRS 算法迁移；本版先把现有 `MemoryState`、连续正错、`nextReviewMs`、近期错词变成可解释队列。
- 不新增付费、家长推送或每日强制任务；计划是推荐与引导，不是惩罚。
- 不改变 V0.9.1 / V0.9.2 的题型调度与 Boss 对话表现，除非计划同源需要调整入口参数。

### 16.2 V0.9.4 词包故事与提审版本硬化

V0.9.4 的实际交付目标从原先“听音题型 + 魔法书图鉴”调整为更低风险的内容与发布硬化：先让每个词包拥有一条可审核、可发布、三端可展示的短故事，并完成 `0.9.4 / 1009004` 版本号同步与 iOS 提审准备。后续评估认为现有题型已经足够覆盖当前学习目标，因此听音题型取消，魔法书图鉴独立作为 V0.9.5 推进。

已交付范围：

- **三端 pack story 展示**：`SceneMetadata` / `Pack.scene` 支持 `storyEn` 与 `storyZh`；Home adventure card 预留两行 story 文案；HarmonyOS 优先中文，iOS / Android 当前优先英文展示。
- **内置词包回填**：5 个 built-in packs 均补齐故事文案，缓存与 fallback merge 保留 story 字段。
- **服务端 story 管理**：parent pack detail、admin global pack detail、admin family pack list 支持 story 手填保存；新增 pack story LLM 生成服务与审计动作。
- **契约同步**：family/global pack 响应、lesson draft review DTO、OpenAPI 产物与三端 cloud parser 均兼容 story 字段。
- **发布硬化**：HarmonyOS、iOS、Android 版本号同步为 `0.9.4 / 1009004`；iOS release archive 上传成功并于 2026-05-29 提交 App Review，状态记录在 [`ios/release-pre.md`](../ios/release-pre.md)。

### 16.3 V0.9.5 魔法书图鉴

V0.9.5 已从“计划中”推进到 **Stage 5 cleanup**。本版没有新增听音题型，而是把现有掌握度、pack library 与词包故事基础转成孩子可见的收集进度：打开魔法书、看每个 pack 的封面与词卡状态、点开已见 / 已掌握词卡回看，并在整包掌握后领取一次性魔法币奖励。

已交付范围：

- **三端客户端**：HarmonyOS、iOS、Android 均已实现 `HomeSpellbookButton`、`HomePackSpellbookCover`、`SpellbookPage`、locked / seen / mastered 词卡、词详情层、pack progress 与 claimed badge。
- **奖励语义**：整包 mastered 后可领取一次性 +50 魔法币；claim state 以 `packId` 本地持久化，V0.9.5 不做云端同步。
- **封面元数据**：三端均兼容 `scene.spellbookCoverUrl`，并保留本地封面 / fallback 规则。
- **服务端封面生成**：已合入 spellbook cover prompt、image provider 调用、Blob 上传、global / family pack 写回 `scene.spellbookCoverUrl`、admin / parent 页面触发入口、审计与 OpenAPI 契约。
- **版本号**：HarmonyOS、iOS、Android 均已推进到 `0.9.5 / 1009005`。
- **验证记录**：HarmonyOS replication trigger 记录 unit、HAP build 与 CodeLinter 通过；iOS 记录 `SpellbookTests` 4/4 与 full unit 179/179 通过；Android 记录 Spellbook / local progress targeted tests 与 full `testDebugUnitTest` 通过。

剩余 closeout：

- 刷新三端截图：Home with spellbook cover/button、Spellbook page、word detail。
- 复跑 HarmonyOS ohosTest，并把结果补入 [`20-replication-trigger.md`](features/2026-05-29-spellbook-v0-9-5/20-replication-trigger.md) / [`50-parity-checklist.md`](features/2026-05-29-spellbook-v0-9-5/50-parity-checklist.md)。
- 补齐 server / admin acceptance 勾选与最终 `done_by` / `done_at` sign-off。

## 17. V0.10 战斗音频混音与 BGM 版

V0.10 的目标是把 V0.4.8 暂时回退的战斗 BGM 重新纳入产品，但不再用“页面里直接调用 AVPlayer 播一条循环音乐”的方式实现。该版本要先建立战斗音频总控 `BattleAudioMixer`，让 BGM、短音效和单词朗读按优先级协作，保证学习发音清晰，同时保留轻松、有节奏的战斗氛围。

设计基线：

- [`2026-04-30-battle-audio-mixer-design.md`](superpowers/specs/2026-04-30-battle-audio-mixer-design.md)
...
BattlePage
  -> BattleAudioMixer
      -> MusicLane: 战斗 BGM，循环播放，低音量，可 duck
      -> SfxLane: 普通攻击、combo、受伤、胜利失败等短音效
      -> VoiceLane: 单词朗读 / TTS，最高优先级
```

关键原则：

- **学习清晰度优先**：单词朗读永远高于 BGM；TTS 播放时 BGM 降低音量。
- **BGM 只做背景**：默认低音量循环，不抢单词朗读，不盖住攻击反馈。
- **短音效可以叠加**：普通攻击、combo、受伤、怪物击败和结算音效不应互相吞掉。
- **不做轮询式抢救**：TTS 导致 BGM 被系统暂停时，每次朗读结束最多恢复一次；失败就静默降级。
- **页面只发语义事件**：BattlePage 调用 `playComboAttack()` / `speakWord(word)`，不直接操作播放器。
- **测试稳定性是合入门槛**：`SpellQuestionFlow.ui.test.ets` 需要连跑 5 次 100% 通过，不能靠放宽 polling 掩盖 UI thread 抖动。

V0.10 建议能力：

- 新增 `BattleAudioMixer`，收敛 `AudioService` 与 `PronunciationService` 在 BattlePage 的直接调用。
- 新增 `MusicLane`，播放 `bgm_battle_loop.ogg` 一类 OGG Vorbis 循环资源。
- 新增或接入 `combo_attack.ogg`，让 combo 攻击有独立音效层。
- TTS 播放前 duck BGM，完成或超时后恢复音量。
- 对 TTS 强制暂停 BGM 的场景只做一次 `safeResumeAfterVoice()`。
...
V0.11 的目标是在玩法、学习系统和战斗音频策略都打磨稳定后，把战斗场景从 ArkUI 元素拼装升级为更完整的 2D 游戏表现层。ArkUI 继续承担应用外壳和业务页面，Cocos Creator / Cocos2D 负责战斗场景、角色、怪物、动画、特效和美术主题。
...
- **数据接口先行**：在 V0.3-V0.10 中逐步形成可序列化的 `BattleSession`、`Question`、`AnswerOutcome`、`SessionResult` 等边界对象。
...
V0.11 不建议重写全部 App。它是战斗表现层重构，不是产品业务层重构。等 V0.3-V0.10 的题型、学习状态、奖励、后台模型和音频策略稳定后再做，可以减少跨引擎迁移返工。
...
| 学习系统  | 错词复习、轻量遗忘曲线、按词包本地报告；V0.9.3 已统一本地 daily learning / review state；V0.9.5 已把 mastered 词沉淀到魔法书图鉴 | 个性化学习计划、长期掌握度模型、云端报告                 |
| 内容系统  | 后台 CMS、家长/管理员词库工作台、三层 Pack、V0.9.4 词包故事、V0.9.5 spellbook cover 生成链路  | LLM 题库增强、审核流优化            |
...
> **2026-05-29 刷新。** V0.3–**V0.8.2+**（含三端 bootstrap、家长词库、管理员后台、admin global pack delete、admin / parent family pack delete、draft pack split）已在主干交付；V0.9.1 句子填词、V0.9.2 Boss 个性 / 登场对话、V0.9.3 学习计划与复习逻辑重构、V0.9.4 词包故事与提审版本硬化均已三端对齐并合入 main。V0.9.5 魔法书图鉴客户端主功能、服务端封面生成链路与三端 `0.9.5 / 1009005` 版本号已合入，当前处 Stage 5 cleanup。V0.8.6 怪物等级积分金币与 V0.8.8 每日打卡已完成三端对齐与模拟器验收，V0.8.7 复习入口近期错词策略已被 V0.9.3 daily review state 吸收。短期重点：补齐 V0.9.5 closeout，同时继续 V0.8.3 / V0.8.4 verification hygiene。

短期优先级建议如下：

1. **V0.8.3 verification closeout**：Android targeted tests 已通过；剩余是在具备 Xcode 的开发机上重跑 iOS targeted tests，确认 10 槽 / auto-rotate / `MonsterLevel` / bonus / HP-2 parity。详见 [`docs/features/2026-05-18-battle-polish-v0-8-3/50-parity-checklist.md`](features/2026-05-18-battle-polish-v0-8-3/50-parity-checklist.md)。
2. **V0.8.4 release gate closeout**：V0.8.4 core 已在三端出现；前置 V0.8.3 代码 gap 已关，下一步补跑 iOS / Android full suite / assemble gate。详见 [`docs/features/2026-05-18-battle-balance-v0-8-4/50-parity-checklist.md`](features/2026-05-18-battle-balance-v0-8-4/50-parity-checklist.md)。
3. **V0.9.5 closeout**：客户端三端复制与服务端封面生成链路已合入；剩余截图刷新、Harmony ohosTest 复跑、server/admin acceptance 勾选与最终 sign-off。详见 [`docs/features/2026-05-29-spellbook-v0-9-5/50-parity-checklist.md`](features/2026-05-29-spellbook-v0-9-5/50-parity-checklist.md)。
4. **V0.9.6–V0.9.7 内容与学习增强**：个性化今日推荐 + 区域章节感 → LLM 辅助内容生产。
5. **V0.10 战斗音频**：三端战斗层落地 `BattleAudioMixer`（Harmony 先稳，`SpellQuestionFlow` 连跑 5 次全绿后再合入 BGM）。在复习入口与现有题型继续稳定后启动，以稳为先。
6. **V0.9.1–V0.9.5 release notes hygiene**：V0.9.1–V0.9.4 已完成；V0.9.5 等 closeout 后同步发布说明。
7. **V0.11 Cocos 战斗表现层**：玩法与音频稳定后再迁移表现层。
8. **新功能走三端 SOP**：`docs/features/<feature-id>/` — Harmony 设计实现 → `20-replication-trigger.md` 签字 → iOS / Android 复制。
9. **体验增强（可选）**：云端学习报告按 pack、兑换推送通知、运维 runbook 补全——不阻塞版本号推进，可按 feature 单独立项。
...
| 音频策略过早复杂化 | BGM/TTS 互抢焦点导致 UI flake | V0.10 专门做 BattleAudioMixer，先逻辑混音，SpellQuestionFlow 连跑通过后合入 |
| Cocos 迁移过早 | 玩法未定导致表现层返工 | V0.11 再做战斗美术化重构，V0.3 起保持逻辑与表现分离 |
...
下一步最适合推进的主题（2026-05-29）：

```text
V0.9.5 closeout
  ↓
V0.9 系列后续（docs/features/ 三端 SOP）
  → V0.9.6 个性化今日推荐 + 区域章节感
  → V0.9.7 LLM 辅助内容生产闭环
  → V0.10 战斗音频混音（BattleAudioMixer + BGM）
  → V0.11 Cocos2D 战斗表现层
```

V0.3–V0.9.5 已构成当前产品的 **三端可玩、可绑、可运营词库、可后台排查、可日常留存、可语境练习、可故事化 Boss 入场、可解释今日复习、可展示词包故事、可收藏掌握词** 底座。V0.8.3–V0.8.4 与 V0.9.5 的剩余事项主要是验证、截图与 release gate hygiene，可与 V0.9.6 设计推进并行处理。
