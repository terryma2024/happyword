...
> 最近更新：2026-04-30（V0.4.3 精细记忆状态：WordStat 新增 `consecutiveCorrect` / `consecutiveWrong` / `mastery`，LearningRecorder 在每次答题后维护连胜计数 + ±0.1/-0.2 mastery delta（[0,1] 钳制），MemoryScheduler 改用连胜阈值促升 Familiar/Mastered（streak 优先，传统 correctCount 降级为旧版兼容），LearningSnapshot 升级到 v3 + 老 v2 blob 用 correct/seen 比例回填 mastery；V0.4.1 完整拼写题：Boss-only SpellingArea + SpellGenerator）
