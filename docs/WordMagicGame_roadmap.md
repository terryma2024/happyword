...
> 最近更新：2026-04-30（V0.4.4 每日学习计划页：HomePage 工具栏新增 📋 `HomePlanButton` 入口；新增 `pages/TodayPlanPage` 路由 + `TodayPlanService`，按 dayKey 哈希种子复用 `TodayAdventureBuilder` 生成稳定计划，按 `WordSource` 拆三段（复习/学习中/新词）渲染并标注 `MemoryState` 徽章；done-today 信号由 V0.4.3 streak 数据派生（`lastAnsweredMs >= localMidnight && consecutiveCorrect > 0`），不引入新持久化层；同步收紧 SpellQuestionFlow UI 测试：在 `driveUntilSpell` 起步前留 500ms 安顿、读插槽前留 150ms，turns 上限从 30 拉到 50，缓解长期 suite 串行后的状态拥塞）
