...
| V0.9 系列（6 子版本）/ V0.10 / V0.11 | V0.9.1 三端 full UI green / screenshots partial | V0.9.1 Harmony 已实现并过 unit / HAP / CodeLinter / full ohosTest gate；`replication_approved: true` 后 iOS / Android 对齐已完成 unit/build/full UI 验证。剩余：截图刷新与 release gate cleanup。V0.9.2–V0.9.6（Boss 个性→区域剧情→听音+魔法书→个性化今日→LLM 辅助内容）、V0.10 战斗 BGM 混音、V0.11 Cocos（顺序见 §23） |
...
> **2026-05-25 刷新。** V0.3–**V0.8.2+**（含三端 bootstrap、家长词库、管理员后台、admin global pack delete、admin / parent family pack delete、draft pack split）已在主干交付；V0.9.1 句子填词已完成三端 full UI 自动化收尾。V0.8.6 怪物等级积分金币与 V0.8.8 每日打卡已完成三端对齐与模拟器验收，V0.8.7 复习入口近期错词策略已三端代码对齐，iOS / Android targeted UI 验证已通过。短期重点：补齐 V0.9.1 screenshot gate 与 release gate cleanup，并继续 V0.8.3 / V0.8.4 verification closeout。
...
3. **V0.9.1 句子填词 gate**：Harmony implementation 与 iOS / Android replication 已完成；Harmony unit / HAP / CodeLinter / full `scripts/run_ui_tests.sh --rebuild` 通过，iOS unit/build/full UI 与 Android unit/build/full UI 通过。剩余：screenshots gate 与 release gate cleanup。
