...
> **基线日期：2026-05-17。** 下文「已交付」指本仓库主干可构建、可测、可部署的实现；「规划中」指路线图目标尚未合入或仅设计文档。
...
- **三端原生客户端（V0.7.1 bootstrap parity，已交付）**：
  - **HarmonyOS**（`harmonyos/`，产品功能线 V0.6.7.8，`app.json5` 0.7.0）：17 页闭环；三层 `Pack` + `PackSelectionService`；ohosTest UI 套件覆盖主路径。
  - **iOS**（`ios/WordMagicGame/`，Swift / SwiftUI）：与 Harmony 同矩阵——核心环（Home / Battle / Result）、成长环（PackManager、愿望单、图鉴、今日计划、按词包学习报告）、绑定与家长 PIN/后台、Debug 路由；Release/TestFlight 门禁见 [`ios/release-pre.md`](../ios/release-pre.md)（v0.7.0 build 1007004 已验证）。
  - **Android**（`android/`，Kotlin / Compose）：17 页均已实现；Phase 5 发布硬化门禁已验证（见 [`docs/android-replica/07-release-readiness-checklist.md`](android-replica/07-release-readiness-checklist.md)）。
  - **对齐边界**：三端共享 `shared/fixtures` 解码契约；bootstrap 不含 V0.9 句子填词、V0.10 战斗 BGM、V0.11 Cocos；学习报告仍为 **device-local**（未上云按 pack 聚合）。
- **服务端（V0.5.1–V0.8.2，已交付）**：FastAPI + MongoDB + Vercel。内容管线（词条 CRUD、版本化词包、LLM 草稿、课本图 import、插画/发音 Blob）；账户与绑定（家长 OTP + OAuth、pair、child word-stats / wishlist）；**V0.8.1** 家长词库 HTML 工作台 + family pack JSON API + 子端 **global+family 合并** `packs/latest.json`（`parent_packs_pages.py`、`child_family_pack.py`）；**V0.8.2** 系统管理员 HTML 控制台 `/admin/`（`admin_pages.py`、`admin_audit_service.py`，会话 cookie + 高风险操作审计）。遗留 JSON admin API（V0.5.x）与 HTML 控制台并存，新运营操作走 V0.8.2 壳。
...
> **状态：设计已锁，待 Harmony 实施。** 详细 cross-platform 设计：[`docs/features/2026-05-18-battle-polish-v0-8-3/00-design.md`](features/2026-05-18-battle-polish-v0-8-3/00-design.md)。Harmony 任务计划：[`docs/features/2026-05-18-battle-polish-v0-8-3/10-harmony-plan.md`](features/2026-05-18-battle-polish-v0-8-3/10-harmony-plan.md)。
...
> **2026-05-18 刷新。** V0.3–**V0.8.2**（含三端 bootstrap、家长词库、管理员后台）已在主干交付；**V0.8.3 设计已锁，待 Harmony 实施**；V0.9 已拆为 6 个子版本。下列为 **当前** 短期优先级。

短期优先级建议如下：

1. **V0.8.3 战斗与词包体验小优化**：当前最近期可发版本。先 Harmony 实施 → 签字 → iOS / Android 复制。详见 [`docs/features/2026-05-18-battle-polish-v0-8-3/`](features/2026-05-18-battle-polish-v0-8-3/)。
2. **V0.9.1 句子填词**：V0.8.3 三端落地后启动；最大的题型趣味 Δ，且不依赖 V0.10 音频。
3. **V0.9.2 / V0.9.3 故事感增量**：Boss 个性 + 区域剧情卡（低代码量、高情绪 ROI）。
4. **V0.10 战斗音频**：三端战斗层落地 `BattleAudioMixer`（Harmony 先稳，`SpellQuestionFlow` 连跑 5 次全绿后再合入 BGM）。在 V0.9.3 完成后启动以稳为先。
5. **V0.9.4–V0.9.6**：听音 + 魔法书 → 个性化今日 + 错词幽灵 → LLM 辅助内容生产。
6. **V0.11 Cocos 战斗表现层**：玩法与音频稳定后再迁移表现层。
7. **新功能走三端 SOP**：`docs/features/<feature-id>/` — Harmony 设计实现 → `20-replication-trigger.md` 签字 → iOS / Android 复制。
8. **体验增强（可选）**：云端学习报告按 pack、兑换推送通知、运维 runbook 补全——不阻塞版本号推进，可按 feature 单独立项。
