...
> **基线日期：2026-05-26。** 下文「已交付」指本仓库主干可构建、可测、可部署的实现；「计划中」指路线图目标尚未合入或仅有方向性文档。
...
- **三端客户端当前版本：0.9.2 / 1009002。** HarmonyOS、iOS、Android 已完成 bootstrap 17 页矩阵、三层词包、家长绑定 / PIN / 后台入口、愿望单、怪物图鉴、今日计划、本地学习报告、V0.9.1 句子填词和 V0.9.2 Boss 个性 / 登场对话的代码对齐。V0.9.1 full UI 自动化已通过：HarmonyOS 79/79、iOS 28/28、Android 34/34。
- **学习与战斗现状。** 当前已具备轻量遗忘曲线、近期错词复习入口、按词包本地报告、V0.8.8 每日打卡、V0.8.6 怪物等级积分金币、V0.8.4 题型阶段调度、15-word built-in packs 和 10/5/10 初装战斗默认值。仍需承认的产品债是：`TodayPlanPage` 偏只读，复习入口偏“近期错词快捷入口”，today / review battle 的计划语义尚未统一；这正是 V0.9.3 的目标。
- **服务端当前能力。** `server/` 是 FastAPI + MongoDB + Vercel 后端，已覆盖内容管线、词条 CRUD、版本化词包、LLM 草稿、课本图 import、插画 / 发音 Blob、家长账户 / 设备绑定、child word-stats / wishlist、V0.8.1 家长词库工作台、V0.8.2 系统管理员 HTML 控制台与审计。后台高风险操作已补齐：global pack 硬删除、family pack 管理员 / 家长硬删除、family/global 草稿词包 split copy/move。
- **仍未实现的边界。** 学习报告仍是 **device-local**，未做云端按 pack 聚合；V0.9.3 `LearningPlan` 仍处于计划中；V0.10 战斗 BGM / `BattleAudioMixer` 未实现；V0.11 Cocos2D 战斗表现层未实现。

因此后续路线不应回到「从零搭原型」或「单端 replica / 后台追平」，而应围绕 **三端 feature SOP 新能力、V0.9.3 学习计划重构、V0.10 战斗音频、V0.11 战斗表现层** 继续演进。
...
| V0.9.3 | 计划中 | 学习计划与复习逻辑重构 | 把 `TodayPlanPage`、Home 复习入口、错词 / 到期词调度和战斗计划生成统一到同一套 `LearningPlan` 语义；今日计划从只读清单升级为可执行任务面板；复习入口从“仅近期错词”升级为“到期复习 + 近期错词 + 薄弱词”优先队列 | 无 |
| V0.9.4 | 计划中 | 听音题型 + 魔法书图鉴 | 新 `QuestionKind.ListenAndPick`（点喇叭→3 选 1 释义）；新 `SpellbookPage`（按 pack 分组的已掌握词收集进度） | 复用 `audioUrl` |
| V0.9.5 | 计划中 | 个性化今日推荐 + 区域章节感 | 在 V0.9.3 统一学习任务模型之上，HomePage 增 `HomeRecommendedAdventureCard`；区域剧情卡、章节标题过场、章节完成庆祝顺延到此阶段按 SOP 单独设计 | 可选服务端推荐 / 后台编辑入口 |
...
## 16. V0.9 语境学习与学习系统重构版（拆为 6 个子版本）

V0.9 的目标是让学习从单词识别进入语境理解，并把“今天该学什么 / 什么时候复习 / 为什么推荐这一局”讲清楚。V0.9 被拆成 **6 个可独立发布的子版本**，按"题型 → 角色反馈 → 学习计划 → 听音与收集 → 推荐与章节感 → 内容工厂"顺序推进。AI 始终是"后台辅助内容生产 + 人工审核"，**儿童端不直接调 LLM**。
...
| **V0.9.3** | 学习计划与复习逻辑重构 | 把学习系统从“散落入口”升级为“可执行计划” | 统一 `LearningPlan` / review queue；TodayPlanPage 改为任务面板；HomeReviewButton 消费到期复习 + 近期错词 + 薄弱词；BattlePage today/review 共用计划来源；学习报告解释计划完成度 |
| **V0.9.4** | 听音题型 + 魔法书图鉴 | 把 `audioUrl` 资产用进玩法 + 收集成就感 | 新 `QuestionKind.ListenAndPick`；新 `SpellbookPage`；HomePage 增 `HomeSpellbookButton` |
| **V0.9.5** | 个性化今日推荐 + 区域章节感 | 提升次日 / 多日留存 | HomePage 增 `HomeRecommendedAdventureCard`（基于 V0.9.3 计划模型）；区域剧情卡、章节标题过场、章节完成庆祝从原计划顺延到此阶段 |
...
- 优先**复用已有字段**：V0.9.1 用现成 `example`、V0.9.4 用现成 `audioUrl`、V0.9.5 优先复用 V0.9.3 计划模型与现有 `WordStat`，章节文案如需扩 `Pack.scene` 则走 schema_version 单调递增 + 老客户端 ignore。
...
V0.9.3 的产品目标是把目前分散的学习线索收拢成一个孩子能执行、家长能理解、三端能一致实现的学习任务模型。现状里 `TodayPlanPage` 偏只读，Home 复习按钮偏“最近错词快捷入口”，Battle 的 today / review 计划各自带有历史包袱；V0.9.3 要把这些入口统一到同一套 `LearningPlan` 语义下。

建议范围：

- **统一计划模型**：定义 `LearningPlan` / `LearningPlanTask` / `ReviewQueueReason` 一组跨端语义，至少覆盖 `due_review`、`recent_wrong`、`weak_word`、`learning`、`new_word` 五类来源；同一 word 可以有多个 reason，但 UI 展示一个主 reason。
- **今日计划可执行化**：`TodayPlanPage` 不再只是展示 10 个词，而是展示“今日任务”：到期复习、继续学习、新词探索三段，每段有完成状态、预计题数和进入战斗按钮。完成进度来自真实答题记录，不靠打开页面打勾。
- **复习入口升级**：Home 复习按钮优先消费 review queue：先到期复习，再近期错词，再薄弱词；完全没有可复习内容时给积极空态，引导去今日冒险或选择一个词包，而不是只提示“先答错几题”。
- **战斗计划同源**：today battle、review battle 和 result 写回都围绕同一个计划来源，避免“计划页看到 A 词，战斗实际出 B 词”。复习战斗保持短局（例如 3 怪 / 120s），但词池可以由完整词库补干扰项。
- **学习报告解释计划**：LearningReportPage 增加“今日计划完成度 / 到期复习完成度 / 薄弱词变化”的解释字段，家长能看懂系统为什么推荐这些词。
- **三端 SOP**：HarmonyOS 先冻结 `LearningPlan` 语义、测试夹具和截图；iOS / Android 复制时不得重新解释计划优先级。
...
| 学习系统  | 错词复习、轻量遗忘曲线、按词包本地报告；V0.9.3 统一计划模型规划中 | 个性化学习计划、长期掌握度模型、云端报告                 |
...
> **2026-05-26 刷新。** V0.3–**V0.8.2+**（含三端 bootstrap、家长词库、管理员后台、admin global pack delete、admin / parent family pack delete、draft pack split）已在主干交付；V0.9.1 句子填词与 V0.9.2 Boss 个性 / 登场对话均已三端对齐并合入 main。V0.8.6 怪物等级积分金币与 V0.8.8 每日打卡已完成三端对齐与模拟器验收，V0.8.7 复习入口近期错词策略已三端代码对齐，iOS / Android targeted UI 验证已通过。短期重点：进入 V0.9.3 学习计划与复习逻辑重构，同时继续 V0.8.3 / V0.8.4 verification closeout。
...
3. **V0.9.3 学习计划与复习逻辑重构**：V0.9.2 已完成，下一项先把 `TodayPlanPage`、Home 复习入口、review queue、today/review 战斗计划和学习报告解释统一；先做 Harmony design + plan，再走三端 SOP。
4. **V0.9.4–V0.9.6 内容与学习增强**：听音 + 魔法书 → 个性化今日推荐 + 区域章节感 → LLM 辅助内容生产。
...
V0.9 系列后续（docs/features/ 三端 SOP）
  → V0.9.4 听音题型 + 魔法书图鉴
  → V0.9.5 个性化今日推荐 + 区域章节感
  → V0.9.6 LLM 辅助内容生产闭环
  → V0.10 战斗音频混音（BattleAudioMixer + BGM）  ← 在 V0.9.3 之后插入
  → V0.11 Cocos2D 战斗表现层
```

V0.3–V0.9.2 已构成当前产品的 **三端可玩、可绑、可运营词库、可后台排查、可日常留存、可语境练习、可故事化 Boss 入场** 底座。V0.8.3–V0.8.4 的剩余事项主要是验证与 release gate hygiene，可与 V0.9.3 学习计划设计推进并行处理。
