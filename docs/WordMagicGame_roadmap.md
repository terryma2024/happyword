...
- **三端原生客户端（V0.7.1 bootstrap parity，已交付）**：
  - **HarmonyOS**（`harmonyos/`，`app.json5` 0.8.8 / 1008008）：17 页闭环；三层 `Pack` + `PackSelectionService`；ohosTest UI 套件覆盖主路径；V0.8.3 polish、V0.8.4 scheduler core、V0.8.6 怪物等级积分金币、V0.8.7 复习入口语义与 V0.8.8 daily check-in 已落地。
  ...
  - **对齐边界**：三端共享 `shared/fixtures` 解码契约；bootstrap 不含 V0.9 句子填词、V0.10 战斗 BGM、V0.11 Cocos；学习报告仍为 **device-local**（未上云按 pack 聚合）。
...
| V0.9 系列（6 子版本）/ V0.10 / V0.11 | V0.9.1 Stage 1 design written / review pending | V0.9.1–V0.9.6（句子填词→Boss 个性→区域剧情→听音+魔法书→个性化今日→LLM 辅助内容）、V0.10 战斗 BGM 混音、V0.11 Cocos（顺序见 §23） |
...
| V0.9.1 | Stage 1 design written | 句子填词题型（AI 语境第一步）  | 新 `QuestionKind.SentenceCloze`：例句留空目标词，3 选 1 填回；复用现有 `example.{en,zh}`；默认开启并纳入题型开关；内置 5 词包补齐 example；缺例句的词自动回退。设计：[`docs/features/2026-05-24-sentence-cloze-v0-9-1/00-design.md`](features/2026-05-24-sentence-cloze-v0-9-1/00-design.md) | 复用既有例句字段 |
...
> **2026-05-24 刷新。** V0.3–**V0.8.2+**（含三端 bootstrap、家长词库、管理员后台、admin global pack delete、admin / parent family pack delete、draft pack split）已在主干交付；HarmonyOS / iOS / Android app metadata 均为 **0.8.8 / 1008008**。V0.8.6 怪物等级积分金币与 V0.8.8 每日打卡已完成三端对齐与模拟器验收，V0.8.7 复习入口近期错词策略已三端代码对齐，iOS / Android targeted UI 验证已通过，HarmonyOS ohosTest 需稳定设备通道复跑。短期重点改为 V0.8.3 / V0.8.4 verification closeout；V0.9.1 已进入三端 SOP Stage 1 design review。
...
3. **V0.9.1 句子填词 Stage 1 review**：feature folder 与 cross-platform design 已建立；Harmony 实现仍需在设计获批后按 SOP 进入 `10-harmony-plan.md`，并继续把 V0.8.3 / V0.8.4 gate closeout 作为 release hygiene 并行处理。
...
```text
V0.8.3 verification closeout（三端代码已补；iOS targeted 与三端完整套件复核）
  ↓ 关闭 V0.8.3 前置 parity gate
V0.8.4 release gate closeout（三端 core 已实现）
  ↓ 补必要 full-suite / assemble 验证
...
V0.3–V0.8.8 已构成当前产品的 **三端可玩、可绑、可运营词库、可后台排查、可日常留存** 底座；V0.8.3–V0.8.4 的剩余事项主要是验证与 release gate hygiene。V0.9.1 可以先进入三端 SOP 的 Stage 0–1 设计，但 Harmony 实现、复制与发版仍要尊重 feature folder、签字 gate 和后续 parity checklist。
