...
> 最近更新：**2026-05-24** — V0.8.7 复习按钮 bugfix 已三端对齐：Home toolbar 复习入口读取近期回答错误词，若存在则以这些错词作为临时 focused word plan 开战（1 个错词也可启动，选择题干扰项仍可来自完整词库）；若近期无错词则显示 `先答错几题再来复习吧` toast。V0.8.6 怪物等级积分金币也已完成三端对齐（**0.8.6 / 1008006**）：战斗结算金币完全替换为 `Σ 击败数量 × 怪物等级值`，星级与 Bonus 不再参与金币；HarmonyOS 已通过用户验收，iOS `BattleEngineTests` 与 simulator build 已通过，Android `BattleEngineTest` 与 `assembleDebug` 已通过，iOS / Android 模拟器安装验收已完成；owner 明确跳过截图 artifact 刷新。V0.8.7 验证：iOS targeted XCUITest 通过，Android targeted connected UI test 通过，Android instrumentation Kotlin 编译通过；HarmonyOS ohosTest 断言已补强但本机 runner 本轮无报告码输出后中断，需后续在稳定设备通道复跑。最新主线还包括 **draft pack split**（家长 family 词包与管理员 global 词包均可从草稿中选词，以 copy / move 创建新词包）、**admin global pack hard delete**（审计删除官方词包）与 **phrase parity**（短语题保留空格、跳过冠词遮挡，三端一致），均已纳入当前评估。设计基线见 [`2026-05-23-draft-pack-split-design.md`](superpowers/specs/2026-05-23-draft-pack-split-design.md)、[`2026-05-08-v0.8-backoffice-and-vocabulary-management-design.md`](superpowers/specs/2026-05-08-v0.8-backoffice-and-vocabulary-management-design.md)、[`docs/features/2026-05-18-battle-polish-v0-8-3/00-design.md`](features/2026-05-18-battle-polish-v0-8-3/00-design.md)、[`docs/features/2026-05-18-battle-balance-v0-8-4/00-design.md`](features/2026-05-18-battle-balance-v0-8-4/00-design.md) 与 [`docs/features/2026-05-23-coin-reward-by-monster-level-v0-8-6/00-design.md`](features/2026-05-23-coin-reward-by-monster-level-v0-8-6/00-design.md)。
...
- **三端原生客户端（V0.7.1 bootstrap parity，已交付）**：
  - **HarmonyOS**（`harmonyos/`，`app.json5` 0.8.6 / 1008006）：17 页闭环；三层 `Pack` + `PackSelectionService`；ohosTest UI 套件覆盖主路径；V0.8.3 polish、V0.8.4 scheduler core、V0.8.6 怪物等级积分金币与 V0.8.7 复习入口语义已落地。
  - **iOS**（`ios/WordMagicGame/`，Swift / SwiftUI，0.8.6 / 1008006）：与 Harmony 同矩阵——核心环（Home / Battle / Result）、成长环（PackManager、愿望单、图鉴、今日计划、按词包学习报告）、绑定与家长 PIN/后台、Debug 路由；V0.8.4 scheduler core、V0.8.3 polish parity、V0.8.6 怪物等级积分金币与 V0.8.7 复习入口语义已落到代码层，release-pre 仍保留生产 backend 给 Apple review 的 P0 确认项。
  - **Android**（`android/`，Kotlin / Compose，0.8.6 / 1008006）：17 页均已实现；V0.8.4 scheduler core、V0.8.3 polish parity、V0.8.6 怪物等级积分金币与 V0.8.7 复习入口语义已落到代码层；Phase 5 发布硬化门禁见 [`docs/android-replica/07-release-readiness-checklist.md`](android-replica/07-release-readiness-checklist.md)。
  - **对齐边界**：三端共享 `shared/fixtures` 解码契约；bootstrap 不含 V0.9 句子填词、V0.10 战斗 BGM、V0.11 Cocos；学习报告仍为 **device-local**（未上云按 pack 聚合）。
- **服务端（V0.5.1–V0.8.2+，已交付）**：FastAPI + MongoDB + Vercel。内容管线（词条 CRUD、版本化词包、LLM 草稿、课本图 import、插画/发音 Blob）；账户与绑定（家长 OTP + OAuth、pair、child word-stats / wishlist）；**V0.8.1** 家长词库 HTML 工作台 + family pack JSON API + 子端 **global+family 合并** `packs/latest.json`（`parent_packs_pages.py`、`child_family_pack.py`）；**V0.8.2** 系统管理员 HTML 控制台 `/admin/`（`admin_pages.py`、`admin_audit_service.py`，会话 cookie + 高风险操作审计）；**2026-05-22 follow-up** 已补 `DELETE /api/v1/admin/global-packs/{pack_id}` 与 HTML 删除入口，硬删除官方词包并记录审计；**2026-05-23 follow-up** 已补 family/global 草稿词包拆分，只能从 source draft 选词创建新词包，支持 `copy` / `move`，并覆盖 JSON API、HTML 表单、契约与 e2e 场景。遗留 JSON admin API（V0.5.x）与 HTML 控制台并存，新运营操作走 V0.8.2 壳。

因此后续路线不应回到「从零搭原型」或「单端 replica / 后台追平」，而应围绕 **三端 feature SOP 新能力、V0.6 体验增强（可选）、战斗音频（V0.10）、AI 语境（V0.9）** 演进。

### 2.1 路线图与现状差距（2026-05-23）
...
| **V0.8.6 怪物等级积分金币** | **Done / PR pending** | [`docs/features/2026-05-23-coin-reward-by-monster-level-v0-8-6/`](features/2026-05-23-coin-reward-by-monster-level-v0-8-6/) — 战斗结束金币改为 `Σ 数量 × 怪物等级值`（初/中/高/Super = 1/2/3/4），完全替代星星金币与 Bonus `×1.3`；模拟器验收完成，截图 artifact 明确跳过 |
...
| V0.8.2+ | 已完成 | 草稿词包拆分新建包 | Family 与 global 草稿详情页支持勾选部分 draft words，选择 copy / move 后创建新词包；API 为 `POST /api/v1/family/{family_scope}/family-packs/{pack_id}/draft/split` 与 `POST /api/v1/admin/global-packs/{pack_id}/draft/split`，并同步 OpenAPI / error code contracts | 必需 |
...
| 子版本 | 主题 | 趣味 Δ | 关键改动 |
| --- | --- | --- | --- |
| **V0.9.1** | 句子填词题型 | 把"识别"升级为"语境理解" | 新 `QuestionKind.SentenceCloze`；复用 `example.{en,zh}`；Boss 题型轮换加入 |
...
> **2026-05-24 刷新。** V0.3–**V0.8.2+**（含三端 bootstrap、家长词库、管理员后台、admin global pack delete、draft pack split）已在主干交付；HarmonyOS / iOS / Android app metadata 均为 **0.8.6 / 1008006**。V0.8.6 怪物等级积分金币已完成三端对齐与模拟器验收，V0.8.7 复习入口近期错词策略已三端代码对齐，iOS / Android targeted UI 验证已通过，HarmonyOS ohosTest 需稳定设备通道复跑。短期重点改为 PR 合入、V0.8.3 / V0.8.4 verification closeout。V0.9 已拆为 6 个子版本。
...
3. **V0.9.1 句子填词**：V0.8.3 / V0.8.4 gate 决议后启动；最大的题型趣味 Δ，且不依赖 V0.10 音频。
...
**V0.7.1 与 V0.8.1 / V0.8.2+ 已完成**：三端客户端 bootstrap 对齐；家长词库工作台与子端合并拉取、系统管理员控制台与审计均已交付；草稿词包拆分新建包已同时覆盖 family / global 管理。

下一步最适合推进的主题（2026-05-24）：

```text
V0.8.3 replica gap closeout（Harmony 已落地；iOS / Android 待补或延期）
...
V0.3–V0.8.2 已构成当前产品的 **三端可玩、可绑、可运营词库、可后台排查** 底座；V0.8.3–V0.8.4 已把方向推到战斗与学习体验升级，但 gate 上必须先把 **V0.8.3 前置 polish parity** 说清楚、补齐或延期，再开始 V0.9.1。
