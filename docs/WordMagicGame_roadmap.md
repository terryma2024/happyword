...
| V0.7.1 | 已完成 | 三端原生客户端 bootstrap 功能对齐 | **HarmonyOS / iOS / Android** 按同一 17 页路由矩阵复刻 V0.6.7.8 产品语义：核心战斗环、三层词包（builtin / global / family）、PackManager 激活与 pin、愿望单与兑换记录、扫码/短码绑定、按词包本地学习报告、家长 PIN/后台、Debug 预览路由；契约对齐 `shared/contracts` + `shared/fixtures`。iOS：Swift / SwiftUI（[`ios/release-pre.md`](../ios/release-pre.md)）。Android：Kotlin / Compose（[`docs/android-replica/00-index.md`](android-replica/00-index.md)）。**不含** V0.9 剧情、V1.0.0 BGM、V1.1.0 Cocos | 无 |
...
- 不碰战斗页（BattlePage）任何 SVG / 图像：战斗美术化已顺延到 V1.1.0 Cocos2D 重构范围。
...
**7. 战斗背景音乐（暂时下线，回退到 SFX-only；推迟到 V1.0.0 战斗音频混音与 BGM 版）**
...
- 留下的开放问题（推迟到 **V1.0.0 战斗音频混音与 BGM 版** 专门处理，设计基线见 [`2026-04-30-battle-audio-mixer-design.md`](superpowers/specs/2026-04-30-battle-audio-mixer-design.md)）：
...
- 不包含 V0.9 句子填词 / AI 剧情、V1.0.0 战斗 BGM 混音、V1.1.0 Cocos 战斗层。
...
- 不动 V1.0.0 战斗 BGM / 混音边界（保持现有 `AudioService` / `PronunciationService`）。
...
- 不与 V1.0.0 / V1.1.0 抢边界：V0.9.5 不新增战斗题型或音频交互；BGM / 混音留 V1.0.0，战斗特效升级留 V1.1.0。
...
## 17. V1.0.0 战斗音频混音与 BGM 版

V1.0.0 的目标是把 V0.4.8 暂时回退的战斗 BGM 重新纳入产品，但不再用“页面里直接调用播放器播一条循环音乐”的方式实现。该版本已建立三端战斗音频总控 `BattleAudioMixer`，让 BGM、短音效和单词朗读按优先级协作，保证学习发音清晰，同时保留轻松、有节奏的战斗氛围。

设计基线：

- [`2026-05-30-v1-0-0-audio-lab-design.md`](superpowers/specs/2026-05-30-v1-0-0-audio-lab-design.md)
- [`docs/features/2026-06-01-pcm-audio-v1-0-0/00-design.md`](features/2026-06-01-pcm-audio-v1-0-0/00-design.md)
- 早期方案记录：[`2026-04-30-battle-audio-mixer-design.md`](superpowers/specs/2026-04-30-battle-audio-mixer-design.md)
...
- **不做 stop/resume 抢救**：生产路径不再用 `stopBgm + resumeBgm` 伪混音；speak 时降低 BGM，语音结束后恢复。
...
- 新增 `BattleAudioMixer`，收敛 BattlePage 内的 BGM / SFX / pronunciation 调用。
- 新增独立 BGM lane，播放 `bgm_battle_loop` 循环资源。
- SFX lane 覆盖普通攻击、combo、答错、受伤、胜利、失败、怪物击败等 cue。
- speak 时 BGM duck 到 50%，语音结束后恢复到 32%；胜利 / 失败 / win sequence 不再停止 BGM。
- 设置页发音播放改为三个 switch：自动发音默认开、播放 BGM 默认关、动作特效音默认开。
- 题型选择改为 switch button，并沉淀全局 switch color、配置项间距与 alignment guideline。
- DevMenu 重组为 Domain Switch / PcmAudioLab / MessageBubbleLab 并列入口，Domain Switch 单独成页。
- `PcmAudioLab` 保留为 PCM-only 测试页，三端补齐 Mix / Transport / Voice / SFX During Voice / Status 控制项，不再暴露 System TTS 切换。
