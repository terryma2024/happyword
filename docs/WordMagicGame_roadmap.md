...
> 最近更新：2026-04-30（V0.4.7 自定义愿望单条目：`MagicWish.isCustom` + 快照 v2；`WishlistStore.addCustomWish` / `removeCustomWish` 带 1-12 字名称 / 5-200 币 / 1-2 字 emoji 校验；`WishlistPage` 头部 + 添加按钮、自定义卡尾部 ✕ 删除按钮；两路径都过家长 PIN 闸；`AddCustomWishDialog` 三段表单；UI 测 38 / 38 通过。V0.4 全部子版本完成，进入 V0.5 内容后台与 LLM 题库版。）
