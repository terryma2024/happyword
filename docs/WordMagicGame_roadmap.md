...
> 最近更新：**2026-05-17** — **V0.7.1** 三端 bootstrap parity 与 **V0.8.1 / V0.8.2** 后台体系均已标为已完成。V0.8.1：家长 Web 词库工作台（`/family/{family_id}/packs/*`）+ 子端合并拉取 `GET /api/v1/family/{family_id}/packs/latest.json`。V0.8.2：系统管理员 HTML 控制台（`/admin/*`，会话登录 + 审计日志 + 全局/家庭/设备排查）。**当前未启动大版本**：V0.9（AI 剧情）、V0.10（战斗 BGM 混音）、V0.11（Cocos）。设计基线见 [`2026-05-08-v0.8-backoffice-and-vocabulary-management-design.md`](superpowers/specs/2026-05-08-v0.8-backoffice-and-vocabulary-management-design.md)。  
...
| V0.9 / V0.10 / V0.11 | 计划中 | AI 语境、战斗 BGM 混音、Cocos 表现层（顺序见 §23） |
...
| V0.9   | 计划中 | AI 剧情与语境学习版      | 句子填词、主题剧情、LLM 生成剧情草稿、个性化冒险                                | 必需       |
...
> **2026-05-17 刷新。** V0.3–**V0.8.2**（含三端 bootstrap、家长词库、管理员后台）已在主干交付；下列为 **当前** 短期优先级。

短期优先级建议如下：

1. **新功能走三端 SOP**：`docs/features/<feature-id>/` — Harmony 设计实现 → `20-replication-trigger.md` 签字 → iOS / Android 复制。
2. **V0.10 战斗音频**：三端战斗层落地 `BattleAudioMixer`（Harmony 先稳，`SpellQuestionFlow` 连跑 5 次全绿后再合入 BGM，再复制 iOS / Android）。
3. **V0.9 AI 剧情与语境学习**：句子填词、主题剧情、LLM 草稿审核链路（在 V0.10 音频策略稳定后启动更稳妥）。
4. **V0.11 Cocos 战斗表现层**：玩法与音频稳定后再迁移表现层。
5. **体验增强（可选）**：云端学习报告按 pack、兑换推送通知、运维 runbook 补全——不阻塞版本号推进，可按 feature 单独立项。
...
下一步最适合推进的主题（2026-05-17）：

```text
docs/features/ 三端 SOP（新能力）
  -> V0.10 战斗音频混音（BattleAudioMixer + BGM）
  -> V0.9 AI 剧情与句子填词
  -> V0.11 Cocos2D 战斗表现层
```

V0.3–V0.8.2 已构成当前产品的 **三端可玩、可绑、可运营词库、可后台排查** 底座；后续重点是 **跨端新功能、音频与 AI 体验升级**，而不是重复建设 replica 或后台骨架。
