...
- **HarmonyOS 工程整体迁移**：`AppScope/`、`entry/`、`hvigor/`、`hvigorfile.ts`、`build-profile.json5`、`oh-package*.json5`、`code-linter.json5` 一起进入 `harmonyos/`，让 `build-profile.json5` 的 `srcPath: "./entry"` 继续成立。
...
- 迁移后验证 `cd harmonyos && ohpm install && hvigorw assembleHap`，再运行 CodeLinter、no-device unit tests 和必要的 UI 测试。
