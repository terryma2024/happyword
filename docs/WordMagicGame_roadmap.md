...
> 最近更新：**2026-05-29** — 主干已推进到 **0.9.4 / 1009004**。V0.9.3 学习计划与复习逻辑重构已完成三端对齐：本地 `YYYYMMDD` daily state、稳定 daily review snapshot、A/B 完成语义、Home 复习计数与 `TodayPlanPage` 进度均已落地，验收为 HarmonyOS 81/81、iOS UI 30/30、Android 37/37。V0.9.4 实际落地为 **词包故事与提审版本硬化**：三端 `SceneMetadata.storyEn/storyZh` 解析与首页故事行、内置 5 词包故事文案、global/family pack story 保留合并、家长 / 管理员后台 story 编辑与 LLM 生成、OpenAPI 同步、三端版本号 0.9.4，以及 iOS `0.9.4 (1009004)` App Store Connect 上传并于 2026-05-29 提审。V0.9.5 不再新增听音题型，直接聚焦 **魔法书图鉴**：把已掌握词按 pack 沉淀成可收集、可回看的词卡进度。设计基线见 [`docs/features/2026-05-26-learning-plan-review-v0-9-3/00-design.md`](features/2026-05-26-learning-plan-review-v0-9-3/00-design.md)、[`docs/features/2026-05-24-sentence-cloze-v0-9-1/00-design.md`](features/2026-05-24-sentence-cloze-v0-9-1/00-design.md)、[`docs/features/2026-05-25-boss-dialogue-v0-9-2/00-design.md`](features/2026-05-25-boss-dialogue-v0-9-2/00-design.md)、[`docs/features/2026-05-18-battle-polish-v0-8-3/00-design.md`](features/2026-05-18-battle-polish-v0-8-3/00-design.md)、[`docs/features/2026-05-18-battle-balance-v0-8-4/00-design.md`](features/2026-05-18-battle-balance-v0-8-4/00-design.md)、[`docs/features/2026-05-23-coin-reward-by-monster-level-v0-8-6/00-design.md`](features/2026-05-23-coin-reward-by-monster-level-v0-8-6/00-design.md) 与 [`docs/features/2026-05-23-daily-checkin-v0-8-8/00-design.md`](features/2026-05-23-daily-checkin-v0-8-8/00-design.md)。
...
- **三端客户端当前版本：0.9.4 / 1009004。** HarmonyOS、iOS、Android 已完成 bootstrap 17 页矩阵、三层词包、家长绑定 / PIN / 后台入口、愿望单、怪物图鉴、今日计划、本地学习报告、V0.9.1 句子填词、V0.9.2 Boss 个性 / 登场对话、V0.9.3 daily learning / review state 与 V0.9.4 词包故事展示。iOS `0.9.4 (1009004)` 已于 2026-05-29 上传并提交 App Review。
...
- **服务端当前能力。** `server/` 是 FastAPI + MongoDB + Vercel 后端，已覆盖内容管线、词条 CRUD、版本化词包、LLM 草稿、课本图 import、插画 / 发音 Blob、家长账户 / 设备绑定、child word-stats / wishlist、V0.8.1 家长词库工作台、V0.8.2 系统管理员 HTML 控制台与审计。后台高风险操作已补齐：global pack 硬删除、family pack 管理员 / 家长硬删除、family/global 草稿词包 split copy/move；V0.9.4 新增 pack story 编辑与 LLM 生成路径。
- **仍未实现的边界。** 学习报告仍是 **device-local**，未做云端按 pack 聚合；魔法书图鉴、个性化推荐、战斗 BGM / `BattleAudioMixer` 与 Cocos2D 战斗表现层仍未实现。

因此后续路线不应回到「从零搭原型」或「单端 replica / 后台追平」，而应围绕 **V0.9.5 魔法书图鉴、V0.9.6 个性化推荐、V0.10 战斗音频、V0.11 战斗表现层** 继续演进。
...
| V0.9.5 | 计划中 | 魔法书图鉴 | 新 `SpellbookPage`：按 pack 分组展示掌握词收集进度、词卡状态与可回看内容；HomePage 增 `HomeSpellbookButton` | 无 |
...
| **V0.9.5** | 魔法书图鉴 | 把掌握词沉淀成可收集、可回看的成长记录 | 新 `SpellbookPage`；按 pack 分组展示掌握词卡、未解锁词卡和收集进度；HomePage 增 `HomeSpellbookButton` |
...
| 学习系统  | 错词复习、轻量遗忘曲线、按词包本地报告；V0.9.3 已统一本地 daily learning / review state | 个性化学习计划、长期掌握度模型、云端报告                 |
| 内容系统  | 后台 CMS、家长/管理员词库工作台、三层 Pack、V0.9.4 词包故事  | LLM 题库增强、审核流优化            |
...
> **2026-05-29 刷新。** V0.3–**V0.8.2+**（含三端 bootstrap、家长词库、管理员后台、admin global pack delete、admin / parent family pack delete、draft pack split）已在主干交付；V0.9.1 句子填词、V0.9.2 Boss 个性 / 登场对话、V0.9.3 学习计划与复习逻辑重构、V0.9.4 词包故事与提审版本硬化均已三端对齐并合入 main。V0.8.6 怪物等级积分金币与 V0.8.8 每日打卡已完成三端对齐与模拟器验收，V0.8.7 复习入口近期错词策略已被 V0.9.3 daily review state 吸收。短期重点：继续 V0.8.3 / V0.8.4 verification closeout，随后推进 V0.9.5 魔法书图鉴。
...
3. **V0.9.5 魔法书图鉴**：取消新增听音题型，直接把掌握词沉淀到 `SpellbookPage`，按 pack 展示收集进度、已掌握词卡和未解锁词卡。
...
6. **V0.9.1–V0.9.4 release notes hygiene**：已完成，不再作为短期阻塞项；后续仅随发版清单同步引用。
...
V0.3–V0.9.4 已构成当前产品的 **三端可玩、可绑、可运营词库、可后台排查、可日常留存、可语境练习、可故事化 Boss 入场、可解释今日复习、可展示词包故事** 底座。V0.8.3–V0.8.4 的剩余事项主要是验证与 release gate hygiene，可与 V0.9.5 魔法书图鉴设计推进并行处理。
