...
| V0.8   | AI 剧情与语境学习版      | 句子填词、主题剧情、LLM 生成剧情草稿、个性化冒险                                | 必需       |
| V0.9   | 战斗音频混音与 BGM 版 | 新增 `BattleAudioMixer`，让战斗 BGM、combo/攻击/受伤音效、单词朗读按优先级共存；用 duck + 单次恢复策略解决 TTS 抢焦点问题 | 无 |
| V0.10  | Cocos2D 战斗美术化重构版 | 用 Cocos Creator 重写战斗表现层，支持更完整的角色、怪物、动画、特效和多美术主题           | 可选       |
...
- 不碰战斗页（BattlePage）任何 SVG / 图像：战斗美术化已顺延到 V0.10 Cocos2D 重构范围。
...
**7. 战斗背景音乐（暂时下线，回退到 SFX-only；推迟到 V0.9 战斗音频混音与 BGM 版）**
...
- 留下的开放问题（推迟到 **V0.9 战斗音频混音与 BGM 版** 专门处理，设计基线见 [`2026-04-30-battle-audio-mixer-design.md`](superpowers/specs/2026-04-30-battle-audio-mixer-design.md)）：
...
- 受伤音效本地化变体（按区域 / 角色不同）：当前一个统一短促 grunt 足够；后续 V0.8 AI 剧情版可考虑按怪物配本。
...
V0.8 的目标是让学习从单词识别进入语境理解，并让战斗更有故事感。
...
## 16. V0.9 战斗音频混音与 BGM 版

V0.9 的目标是把 V0.4.8 暂时回退的战斗 BGM 重新纳入产品，但不再用“页面里直接调用 AVPlayer 播一条循环音乐”的方式实现。该版本要先建立战斗音频总控 `BattleAudioMixer`，让 BGM、短音效和单词朗读按优先级协作，保证学习发音清晰，同时保留轻松、有节奏的战斗氛围。
...
V0.9 不建议直接做 PCM 级真实混音。第一版采用“逻辑混音”：BGM、SFX、TTS 各自使用合适的系统播放能力，由 Mixer 统一处理音量、优先级、生命周期和降级。只有当 HarmonyOS 音频焦点仍然无法稳定共存时，才进入 AudioRenderer / PCM 混音路线。

## 17. V0.10 Cocos2D 战斗美术化重构版

V0.10 的目标是在玩法、学习系统和战斗音频策略都打磨稳定后，把战斗场景从 ArkUI 元素拼装升级为更完整的 2D 游戏表现层。ArkUI 继续承担应用外壳和业务页面，Cocos Creator / Cocos2D 负责战斗场景、角色、怪物、动画、特效和美术主题。
...
- **数据接口先行**：在 V0.3-V0.8 中逐步形成可序列化的 `BattleSession`、`Question`、`AnswerOutcome`、`SessionResult` 等边界对象。
...
V0.10 不建议重写全部 App。它是战斗表现层重构，不是产品业务层重构。等 V0.3-V0.9 的题型、学习状态、奖励、后台模型和音频策略稳定后再做，可以减少跨引擎迁移返工。
...
7. **先完成 V0.9 战斗音频策略，再启动 Cocos2D 重构**：BGM、SFX 与单词朗读的优先级边界应先在 ArkUI 战斗原型里跑稳，再进入 V0.10 表现层迁移。
...
| 音频策略过早复杂化 | BGM/TTS 互抢焦点导致 UI flake | V0.9 专门做 BattleAudioMixer，先逻辑混音，SpellQuestionFlow 连跑通过后合入 |
| Cocos 迁移过早 | 玩法未定导致表现层返工 | V0.10 再做战斗美术化重构，V0.3 起保持逻辑与表现分离 |
