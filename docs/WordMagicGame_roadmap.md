...
- **三端原生客户端（V0.7.1 bootstrap parity，已交付）**：
  ...
  - **iOS**（`ios/WordMagicGame/`，Swift / SwiftUI，0.8.8 / 1008008）：与 Harmony 同矩阵——核心环（Home / Battle / Result）、成长环（PackManager、愿望单、图鉴、今日计划、按词包学习报告）、绑定与家长 PIN/后台、Debug 路由；V0.8.4 scheduler core、V0.8.3 polish parity、V0.8.6 怪物等级积分金币、V0.8.7 复习入口语义与 V0.8.8 daily check-in 已落到代码层，release-pre 仍保留生产 backend 给 Apple review 的 P0 确认项。
  - **Android**（`android/`，Kotlin / Compose，0.8.8 / 1008008）：17 页均已实现；V0.8.4 scheduler core、V0.8.3 polish parity、V0.8.6 怪物等级积分金币、V0.8.7 复习入口语义与 V0.8.8 daily check-in 已落到代码层；Phase 5 发布硬化门禁见 [`docs/android-replica/07-release-readiness-checklist.md`](android-replica/07-release-readiness-checklist.md)。
  - **对齐边界**：三端共享 `shared/fixtures` 解码契约；iOS / Android 仍不含 V0.9.1 句子填词（等待 Harmony full gate + 人工签字）、V0.10 战斗 BGM、V0.11 Cocos；学习报告仍为 **device-local**（未上云按 pack 聚合）。
...
| V0.9 系列（6 子版本）/ V0.10 / V0.11 | V0.9.1 Harmony implemented / replication blocked | V0.9.1 Harmony 已实现并部分过 gate；full ohosTest / screenshots / main merge / 人工签字仍未完成。V0.9.2–V0.9.6（Boss 个性→区域剧情→听音+魔法书→个性化今日→LLM 辅助内容）、V0.10 战斗 BGM 混音、V0.11 Cocos（顺序见 §23） |
...
| V0.9.1 | Harmony implemented / soft gate partial | 句子填词题型（AI 语境第一步）  | HarmonyOS 已新增 `QuestionKind.SentenceCloze`：例句留空目标词，3 选 1 填回；复用现有 `example.{en,zh}`；默认开启并纳入题型开关与 Challenge pool；内置 5 词包补齐 example；缺例句的词自动回退；默认不自动朗读答案词，只有主动点击喇叭才发音。targeted BattleFlow ohosTest 已通过；full suite / screenshots / 人工签字仍待完成。设计：[`docs/features/2026-05-24-sentence-cloze-v0-9-1/00-design.md`](features/2026-05-24-sentence-cloze-v0-9-1/00-design.md) | 复用既有例句字段 |
