...
V0.3 可以先实现一个区域，再把数据结构设计成可扩展的 `AdventureRegion` 或类似模型，为后续地图和剧情做准备。
...
- 初级：缺 1 个字母，例如 `a _ p l e`。
- 中级：缺 2 个字母，但分两步选择，例如 `s _ h _ o l`。
- 高级：连续隐藏字母或完整拼写，延后到 V0.4。

V0.3 暂不做系统键盘输入。原因是横屏键盘容易遮挡战斗 UI，儿童误输入成本高，并且会引入焦点、撤销、大小写和输入法兼容问题。V0.3 先做点选式补字母，V0.4 再进入完整写单词。
...
4. **最后整理 V0.3 验收与测试**：确保补字母、出题、奖励和复习不会破坏现有战斗。
5. **V0.3 稳定后再启动服务端**：避免客户端模型未稳定时过早设计后台字段。
6. **玩法和业务稳定后再启动 Cocos2D 重构**：先保留 ArkUI 战斗原型，等战斗规则、题型和学习数据边界稳定后再迁移表现层。
