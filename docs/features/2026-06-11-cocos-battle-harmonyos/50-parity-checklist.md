...
| Per-answer side effects (learning record / review marking / codex encounter+defeat / cloud-sync inputs) | **was missing in v1.1.0** — Cocos battles recorded nothing per answer ([60-followups FU-1](60-followups.md)). **Fixed in v1.1.1**: `CocosBattleBridgeCallbacks.onAnswerOutcome` + shared `BattleAnswerRecorder` (same body the native BattlePage delegates to, Android `4a0bea8` blueprint); 8 bridge unit tests; emulator persistence check of `wordmagic_learning` / `wordmagic_monster_progress` see FU-1 | bridge tests + FU-1 evidence |
