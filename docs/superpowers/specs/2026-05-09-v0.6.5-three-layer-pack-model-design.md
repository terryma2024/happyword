...
| ConfigPage **词包同步** button | `WordPackSyncService.syncOnce()` fetches `/api/v1/packs/latest.json` (one mega-pack of every global word) and writes it verbatim into `WordPackCache`. | `wordmagic_word_pack` preferences slot. Cold start uses this if `>= 3` valid words. |
...
The server has a clean, discrete multi-pack model for **family** packs (`FamilyPackDefinition`, `FamilyPackDraft`, `FamilyWordPack`, `FamilyPackPointer`, with parent CRUD at `/api/v1/parent/family-packs/*` and child merged JSON at `/api/v1/child/family-packs/latest.json`). For **global** content the server still serves a single mega-pack at `/api/v1/packs/latest.json` produced by `pack_service.publish_pack(...)` — every published `Word` lands inside the one global snapshot regardless of category.
...
- **G7.** Backward-compatible for v0.6.x clients in the wild — the legacy `/api/v1/packs/latest.json` keeps working.
...
- **NG3.** Migrating any v0.6.x clients off the legacy `/api/v1/packs/latest.json` endpoint.
...
- **Layer C (family)** is parent-managed via the existing `/api/v1/parent/family-packs/*` API. Only visible to the device's bound family. Can be deleted from ConfigPage by the parent (existing server `DELETE` route is reused).
...
- Row controls: `Active` switch (capped at 5 — toggling a 6th shows an inline hint and refuses), `📌 固定` toggle (only enabled when the row is Active), `🗑 删除` (Family rows only, hits `DELETE /api/v1/parent/family-packs/{pack_id}`).
...
- `/family/{family_id}/**` — parent web shell scoped to a family (will eventually replace today's `/parent/**` once we wire family-id into the parent session).
...
`GET /api/v1/packs/latest.json` (and its admin publish path) is left untouched. It continues to serve a single global mega-pack of every published `Word`. v0.6.x clients in the wild keep working unchanged.
...
- `/api/v1/parent/family-packs/*` — parent-managed family packs. Should move to `/api/v1/family/{family_id}/packs/*` (parent-side actions) when the parent session learns to inject `family_id` automatically.
- `/api/v1/child/family-packs/*` — child-side merged read. Should move to `/api/v1/family/{family_id}/packs/latest.json` once Bearer auth is reframed around `family_id`.
- `/api/v1/parent/*` HTML shells (`/parent/login`, `/parent/admin/...`). Should move to `/family/{family_id}/...` (and the operator-only stuff to `/admin/...`).
- `/api/v1/packs/latest.json` (legacy mega-pack). Should be aliased at `/api/v1/public/packs/latest.json` once we're confident every v0.6.x client has upgraded.
...
sequenceDiagram
  ...
  and
    ...
    FPS->>SrvF: GET /api/v1/child/family-packs/latest.json (Bearer)
...
- E2E hygiene: `_create_word` in [server/tests/e2e/test_admin_packs_e2e.py](server/tests/e2e/test_admin_packs_e2e.py) keeps inserting test words with `category="test"` (no behaviour change), but `family_pack_service.publish` gains a guard that drops `category=="test"` entries when `family_id == GLOBAL_PACK_FAMILY_ID` so they cannot enter `/api/v1/public/global-packs/latest.json`. The legacy `/api/v1/packs/latest.json` keeps the old, contaminated behaviour for v0.6.x compatibility. Family packs (real `family_id`) are unaffected by the guard.
...
All suites assume the standard `scripts/run_ui_tests.sh` orchestration: mock server bound to `127.0.0.1:8123`, `hdc rport`, AppStorage `serverBaseUrlOverride` set in `List.test.ets` `beforeAll`. Every suite resets device prefs in `beforeAll` (clear `wordmagic_pack_selection` + `wordmagic_global_packs`) so first-launch seeding paths are exercised deterministically.
...
The matching `latest.json` shape for the v0.6.5 client (`GET /api/v1/child/family-packs/latest.json`) is added if it isn't already there:
...
```python
@app.delete("/api/v1/parent/family-packs/{pack_id}")
