...
#   1. /api/v1/public/health           -> 200 {"ok": true, ...}
#   2. /api/v1/admin/auth/login       -> 200 access_token
#   3. /api/v1/admin/auth/me          -> 200 {"username":"admin",...}
#   4. /api/v1/public/packs/latest.json-> 200 word pack
#
...
# for unauthenticated /public/health + /public/packs/latest.json).

set -u

PROD_URL="${1:-https://happyword.com.cn}"
...
probe "GET /api/v1/public/health" "$PROD_URL/api/v1/public/health" || exit 1
...
if PASS="$(resolve_password)"; then
    ...
    probe "GET /api/v1/public/packs/latest.json" "$PROD_URL/api/v1/public/packs/latest.json" || exit 1
...
if [[ "$LOGIN_CODE" != "200" ]]; then
    echo "  [FAIL $LOGIN_CODE] POST /api/v1/admin/auth/login"
    echo "  $LOGIN_JSON"
    exit 1
fi
echo "  [OK 200]   POST /api/v1/admin/auth/login"
TOKEN="$(echo "$LOGIN_JSON" | jq -er .access_token)"

probe "GET /api/v1/admin/auth/me" "$PROD_URL/api/v1/admin/auth/me" "Authorization: Bearer $TOKEN" || exit 1
probe "GET /api/v1/public/packs/latest.json" "$PROD_URL/api/v1/public/packs/latest.json" || exit 1

WORD_COUNT="$(curl -sS "$PROD_URL/api/v1/public/packs/latest.json" | jq -r '.words | length')"
PACK_VERSION="$(curl -sS "$PROD_URL/api/v1/public/packs/latest.json" | jq -r '.version')"
