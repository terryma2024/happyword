...
The `:CompileArkTS` step must emit **zero** disallowed `ArkTS:WARN` lines before a Harmony-side change is considered merge-ready. Typical causes: deprecated module-level `router` / `getContext`, legacy `@kit.CoreFileKit` picker types, `ImagePacker.packing`, etc. Migrate to `this.getUIContext().getRouter()`, `this.getUIContext().getHostContext()`, `@kit.MediaLibraryKit` / `photoAccessHelper`, `ImagePacker#packToData`, and related SDK replacements.

**Verify:** capture the build log and run the gate script from the repo root:

```sh
cd harmonyos && hvigorw assembleHap --no-daemon 2>&1 | tee /tmp/hap-build.log
cd .. && scripts/check_arkts_warnings.sh /tmp/hap-build.log   # must exit 0
```

Agents fix warnings at the source; do not silence the compiler for convenience.

**Sole exception (vendored Cocos adapter):** files under `harmonyos/entry/src/main/ets/cocosvendor/` (regenerated by `tools/cocos/build-harmonyos.sh`; never hand-edited) are allowed exactly two warning messages — `"globalThis" is not supported (arkts-no-globalthis)` (the native engine reads those functions off the worker global via napi; ArkTS offers no compliant alternative) and `Function may throw exceptions. Special handling is required.` (advisory on `MessagePort.postMessage` in the vendored message pump). `scripts/check_arkts_warnings.sh` encodes this allowlist; everything else there — including missing-permission hints — is fixed by scripted fix-ups in `build-harmonyos.sh` (see `cocos/README.md` → Known quirks, HarmonyOS side).

**Cold-build caveat:** ArkTS warnings only surface for files that actually recompile — an incremental `assembleHap` can report a falsely clean log. When touching `cocosvendor/`, the vendor script, or the gate itself, force recompilation first: `find harmonyos/entry/src/main/ets -name '*.ets' -o -name '*.ts' | xargs touch`.
